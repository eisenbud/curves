...
The central result of this chapter is the  \emph{uniform position lemma}, which deals with the monodromy group of the points of a general hyperplane section of a curve $C \subset \PP^r$. To say what the monodromy group is, and to prove the  uniform position lemma, we will use the classical topology (an equivalent definition is described in Cheerful Fact~\ref{Galois equals monodromy} below). 
...
\begin{exercise}
Recall that if $E \subset \PP^2$ is a smooth plane cubic curve, a point $p \in E$ is called a \emph{flex} if $\cO_E(3p) \cong \cO_E(1)$; by Corollary~\ref{torsion points} in case $g=1$, there are nine of them. Now let $\PP^9$ be the space of plane cubics, and let
$$
