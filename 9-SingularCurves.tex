...
$$
and $f$ is the defining equation of $C_0$ in an affine open containing $q$. As we saw in the nodal case, $\omega_0$ will have poles at the points $r_i$; let $m_i$ be the order of the pole of $\omega_0$ at $r_i$. At this point, we can define the \emph{adjoint ideal} of $C_0$ at $q$ to be the ideal
$$
...
At this point it may seem that we have merely slapped a label on our ignorance. But in fact, it's a useful construction, for two reasons: first, the adjoint ideal can in practice be determined readily; and second, there is a remarkable alternative characterization of the adjoint ideal.

Let's start with the first of these. To begin with, let's do some simple examples:

\begin{example}[nodes and cusps]
We have already seen that in case $q$ is a node of $C_0$, there are two point of $C$ lying over it, and $m_1=m_2=1$; the adjoint ideal is thus just the maximal ideal $\cI_q$ at $q$. In the case of a cusp, for example the zero locus of $y^2-x^3$, there is only one point $r=r_1$ of $C$ lying over $q$, and the differential $\omega_0$ vanishes to order $m_1=2$; since the pullback to $C$ of any polynomial $g$ vanishing at $q$ will vanish to order at least two at $r$, and so again the adjoint ideal is simply the maximal ideal at $q$.
\end{example}
...
There is in fact a general formula for the adjoint ideal of any isolated plane curve singularity:
