...
Algebraic geometry is almost unique among geometric theories in
\marginpar{filler}
\redden{the sense}
that
...
collection of all varieties with specified numerical invariants, or
...
right (Figure~\ref{cubic family}).
\marginpar{\hskip-30pt $\leftarrow$ It's not so clear how this figure relates; maybe hang it on the word ``family'' somewhere?}
...
\marginpar{removed quotes around special}
special properties of objects parametrized by closed subvarieties
of the moduli space. We have already used this
...
\marginpar{removed quotes around $g+3$}
...
\marginpar{removed quotes around ``existence''}
...
\marginpar{reordered to allow line break}
maps of schemes,
...
\marginpar{changed \{enumerate\} to examples}
\index{moduli functor}%
...
the source
of $f'$ making the
\redden{expected diagram commute:}
\marginpar{moved ``commute'' to avoid short line}
$$
\begin{diagram}[small]
\cC && \rTo^\cong && \cC'\\
&\rdTo_f&&\ldTo_{f'}\\
...
\marginpar{changed ``the following'' to number for the sake of link}
Chapter~\label{CurvesModuliChapter}. Nonetheless,
...
\marginpar{changed to plural to agree with title}
discuss
\redden{Severi varieties}
...
\index{flat family}%
are all equal
\marginpar{removed unmatched left parentheses, Section $\to$ Chapter+\S}
...
constancy of the Hilbert polynomial \cite{Hartshorne1977}.
\marginpar{\hskip-100pt $\leftarrow$ removed empty argument of \string\cite, but it's better to provide the passage}

In Chapter~\ref{HilbertSchemesChapter} we
\marginpar{dropped ``will'' to avoid bad break}
study the
...
\redden{the cardinality of the continuum},
\marginpar{The 2 of ``$2^{\aleph_0}$'' made it harder (for me) to see why}
...
\marginpar{made Hom upright (of course kept it distinct from $\sHom$)}
...
In this section
\redden{we}
\marginpar{we'll $\to$ we (to allow line break), then dropped ``we'll'' for consistency}
compute
...
\CC[\eps]/(\eps^2)$. Recall
\redden{from}
\marginpar{instead of parentheses}
...
\marginpar{\hskip -40pt changed $I$ to $I'$ in the first inclusion}
...
\marginpar{please check if these are the right places}
\redden{the example of plane curves introduced in the second paragraph of this chapter (page~\pageref{ModuliChapter}; see also Examples \ref{Hilb for plane curves} and \ref{Hilb for plane curves-continued}):}
...
\marginpar{Lemma $\to$ Theorem since later it's referred to as M's theorem}
...
\marginpar{\hskip-20pt repunctuated with colon here and full sentences below}
such that:
...
\marginpar{indexed usage of Serre--Grothendieck (overkill?)}
...
\marginpar{lowercased ``scheme'', removed comma before ``above'', ok?}
\unif
\end{theorem}
...
\mbinom{m+0 -3}{0},
\marginparhere{removed parentheses around 3 in the last summand}
 $$
...
The
\redden{simplest and most fundamental}
Hilbert schemes are the
Grassmannians\emdash including the projective spaces themselves.
\marginpar{replaced parens by dash and started new sentence}
\index{Grassmannian}%
\redden{They}
\null
...
Concretely,
\marginpar{dropped ``if we'' (to avoid bad break) and repunctuated}
...
%(
\marginpar{to avoid awkward break in your name :-)}
\redden{the reader may consult}
Chapters 3--5 of
\cite{3264}.
...
\marginpar{either this formula or the next have to be displayed; I chose this one}
...
\marginpar{added period at end of sentence; or was something truncated here?}
...
\marginpar{subset, of $\to$ subset of (please check)}
\redden{subset of}
...
\marginpar{set Isom upright, indexed}
...
\marginpar{Lemma $\to$ Theorem; also gave the name of the theorem for better layout, and index it}
\index{Matsusaka's theorem}%
 is 4.
\end{enumerate}

Hint: Any cubic vanishing on $X$ vanishes identically on $H$.
\marginpar{moved the hint outside the enumerate, ok?}
\end{exercise}
