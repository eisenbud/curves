...
More generally, if $0\in D\rTo^v \CC^{r}$ is the germ of a curve defined on a disk $D\subset \CC$, and every point $v(t)$ is a flex point, then $v(D)$ lies in a hyperplane.
\end{lemma}

The proof applies to any linear series on a curve.

\begin{proof}
To say that $v(t)$ is an inflection point is to say that the vectors $v(t), v'(t), \dots, v^{(r)}(t)$ are linearly dependent. If this holds for all $t \in D$ then
$$
v(t) \wedge v'(t) \wedge \dots \wedge v^{(r)}(t) \; \equiv \; 0.
$$
Now let $k$ be the smallest integer such that
$$
v(t) \wedge v'(t) \wedge \dots \wedge v^{(k)}(t) \; \equiv \; 0,
$$
so that the vectors $v(t), v'(t), \dots, v^{(k-1)}(t)$ span a $k$-plane $\Lambda \subset \CC^{r+1}$, and $v^{(k)}(t) \in \Lambda$.

When we take the derivative of the wedge product $v(t) \wedge v'(t) \wedge \dots \wedge v^{(k)}(t)$ by applying the product rule we see that the first $k$ terms are zero because they contain a repeated factor; it follows that
$$
v(t) \wedge v'(t) \wedge \dots \wedge v^{(k-1)}(t)\wedge v^{(k+1)}(t) \equiv 0,
$$
so that $v^{(k+1)}(t) \in \Lambda$ as well. Indeed, as we continue to take derivatives, we see in each case that all but one term is zero, and we deduce that $v^{(l)}(p) \in \Lambda$ for all $l$. Thus $v(D)$  lies in $\Lambda$.
\end{proof}
 ...
 \begin{proof} Again the result is local, this time with a pair of germs $D\rTo^v \CC^r, \  D\rTo^w \CC^r,$ neither of which is contained in a line. Let $C_1, C_2$ be the images of $v,w$ respectively
 Let  
 $$
 \Sigma := \{ (p,q) \in D \times D \mid \text{ and }\TT_{v(p)}(C_1) = \TT_{w(q}(C) \}
 $$
 be the variety parametrizing bitangents to the two germs. Our goal is to say that the projection maps $\Sigma \to D$ are not surjective.
 Suppose instead that they are, and let $(p,q) \in \Sigma$ be a general point. 
 ...
 The statement that the tangent lines to $C$ at the points $v(t)$ and $w(t)$ are equal then says that the vectors $v(t), v'(t), w(t)$ and $w'(t)$ all lie in a 2-dimensional subspace of $\CC^{r+1}$; in particular,
 $$
 v(t) \wedge v'(t) \wedge w(t) \equiv 0 \quad \text{and likewise} \quad v(t) \wedge w(t) \wedge w'(t) \equiv 0
 $$
We proceed now exactly as in the proof of Lemma~\ref{finite inflections}: taking derivatives, we see that \emph{all derivatives of $v(t)$ at $t=0$ lie in a subspace $\Lambda$}, and hence $C_1$ is a straight line.
 \end{proof}
...
(In the applications, we will want to think of it in the projective setting, but for now it will be clearer to work in the context  of vector spaces.) We start with a basic definition:
...
b_j \; := \; \#\{ i \mid a_i \geq j \};
$$
we will write ${\bf a}^*$ for the transpose Schubert index of $\bf a$.
...
along $C$ to order $m$ as is done in \cite[pp. 84--85 in the German edition]{Kunz}:
