...
suppose that ${\bf a} = (a_1,\dots, a_{k+1})$ is a Schubert index in $G(k+1, V)$; that is, we have $n-k \geq a_1 \geq a_2 \geq \dots \geq a_{k+1} \geq 0$. We define the \emph{transpose} Schubert index to be the sequence ${\bf b} = (b_1,\dots, b_{n-k})$ by setting
$$
b_j \; := \; \#\{ i \mid a_i \geq j \};
$$
...
\begin{theorem}
Let $p_1,\dots, p_\delta \in \PP^1$ be any (distinct) points, and for each $j = 1,\dots, \delta$ let $0 \leq a^j_1 \leq a^j_2 \leq \dots \leq a^j_r \leq d-r$ be any  decreasing sequence. There exists a $g^r_d$ on $\PP^1$ with ramification sequence $a^j$ at $p_j$ if and only if 
\end{theorem}
