...
We will be concerned here with  a smooth, nondegenerate curve $C \subset \PP^3$ of degree 4 and genus 0 in $\PP^3$. To describe the geometry of $C$, the first thing to determine is what surfaces it lies on---that is, what degree polynomials on $\PP^3$ vanish on $C$. This will serve as a template for our analyses of space curves in general, so rather than state the end result in advance, we will treat this as an exploration.

To describe the geometry of $C$, the first thing to determine is what surfaces $C$ lies on.
...
Now, having found that $C$ lies on a quadric surface $Q$, we should pause and review what we know about curves on a quadric. To start, 
