...
\fix{To put in: Rational quartic in $\PP^3$ as curve of type 1,3 on quadric do dimension count. Branch points can be chosen. $g^3_4$ is sum of $g^1_1$ and a $g^1_3$. Mention open questions: Set theor comp int problem,  "secant conjecture"?}
...
In fact, the minors of this matrix generate the ideal of forms on $\PP^d$ vanishing on $C$. For this result see for example \cite[****]{E}. \fix{We can prove this later, from the Eagon-Northcott resolution and homological considerations. Put in a ref when there is one}.
...
$$
h^0(\cI_{C/\PP^d}(2))  = \binom{d}{2};
$$
and if equality holds if and only if $C$ is a rational normal curve
\end{proposition}
...
\end{proof}
\fix{will ``linearly independent points impose independent conditions on quadrics" be obvious to our reader?}
...
\begin{exercise}
Prove directly \fix{clarify what ``directly'' means? Otherwise we're playing ``read my mind''.} the special case $r=3$: that the twisted cubic is the unique irreducible, nondegenerate space curve lying on three quadrics.
\end{exercise}
