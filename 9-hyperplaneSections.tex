...
We will say that a curve achieving the bound is a \emph{Castelnuovo curve}. We will see in Chapter~\ref{ScrollsChapter} that the bound is sharp: for every $r$ and $d \geq r$, there exist curves with genus exactly $\pi(d,r)$. Moreover, when
$d\geq 2r+1$, any Castelnuovo curve lies on a surface of  degree $r$, the minimum possible.
...
\begin{example}
Every canonical curve is a Castelnuovo curve. These are curves of degree $2g-2$ in $\PP^g-1$
(and any smooth, irreducible curve with these characteristics is embedded by the canonical
series---see Exercise~\ref{canonical characterization}). We have 
$M = \lfloor (2g-3)/(g-2) = 2$, so $\pi(2g-2, g-1) = 1\cdot (g-2) + 2\cdot 1 = g$.
\end{example}
...
A curve $C\subset \PP^r$ (or more generally a purely 1-dimensional subscheme) is
said to be \emph{arithmetically Cohen-Macaulay}, or \emph{ACM}, if 
all the homomorphisms
$$
H^0(\sO_{\PP^r}(d)) \to H^0(\sO_C(d))
$$
are surjective; or equivalently $H^1(\sI_C(d)) = 0$ for all $d$.

\begin{corollary}
Castelnuovo curves are arithmetically Cohen-Macaulay. In particular this is true for
canonical curves.
\end{corollary}

\begin{proof}
If $C\subset \PP^r$ is a Castelnuovo curve, then the inequalities in the proof of
Theorem~\ref{Castelnuovo's bound} must be equalities, and it follows that
the linear series of hypersurfaces sections of degree $d$ are all complete. \fix{this needs
a little more argument.}
\fix{give the relevant inequality an equation number and refer to it directly}
\end{proof}

\begin{fact}
Though we have stated and proved that smooth canonical curves are ACM,
only a few additional arguments are needed to treat a more  general case. In \cite{Schreyer} 
...
$$ 
such that there is a $g-3$ plane meeting $C$ in exactly and having a simple $(g-2)$-secant $(g-3)$-plane.
\end{fact}

We will return to the ACM condition in Chapter~\ref{linkage}.
...
Exercise~\label{canonical characterization} If $C\subset \PP^g-1$ is a nondegenerate smooth
curve of genus $g$ and degree $2g-2$, then $\sO_C(1) = \omega_C$.

Hint: by the Riemann-Roch theorem t $\sO_C(1)$ is special, and the case of equality in Clifford's theorem applies.
\input footer.tex