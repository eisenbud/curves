...
We will say that a curve achieving the bound is a \emph{Castelnuovo curve}.  For example, 
In the case $r=2$ we have $\pi(d,r) = \binom{d-1}{2}$, so in particular every plane curve (smooth or not) is a Castelnuovo curve.

\begin{theorem}
Suppose that $C\subset \PP^r$ is a nondegrenerate curve embedded by a complete linear series. If $d := \deg C \leq 2r$
...
It is easy to describe the Castelnuovo
