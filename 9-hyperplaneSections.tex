...
Given this language, we can state our first result. Recall that if $C\subset \PP^r$ is a reduced curve then, by Bertini's theorem, a general hyperplane meets $C$ transversely.
...
One way to understand the bound $mn+1$ is to realize that if $\Gamma$ is any finite subscheme of a rational normal curve $C\subset\PP^r$ of degree $r$, 
...
\begin{proof}
Suppose first that $d \geq mn+1$, and let $p_1,\dots,p_{mr+1} \in \Gamma$ be any subset of $mn+1$ points. It suffices to show that $\Gamma' = \{p_1,\dots,p_{mr+1}\}$ imposes independent conditions of $H^0(\cO_{\PP^r}(m))$, that is, for any $p_i \in \Gamma'$ there is a hypersurface $X \subset \PP^r$ of degree $m$ containing all the points $p_1,\dots, \hat{p_i},\dots,p_{mr+1}$ but not containing $p_i$.
...
Now suppose that  $p_{a}(C) = \pi(d,r)$. To show that $C$ is arithmetically Cohen-Macaulay we must show that $V_{m} = H^0(\cO_C(m))$ for all $m$.
...
\end{proof}
 ...
 First of all, in the case $r=2$ we have $\pi(d,r) = \binom{d-1}{2}$, so in particular every plane curve (smooth or not) is a Castelnuovo curve.
...
\begin{theorem}\label{list of Castelnuovo curves}
Suppose that $C\subset \PP^r$ is a nondegenerate curve embedded by a complete linear series. If $d := \deg C \leq 2r$
then $C$ is  a Castelnuovo curve (and thus arithmetically Cohen-Macaulay) if and only if $C$ if and only if one of the following holds:
\begin{enumerate}
\item $C$ is embedded by a linear series of degree $\geq 2g+1$; in this case $d<2r$.
\item $C$ is embedded by the canonical series; in this case $d = 2r$.
\item $r=3$ and $C$ is a divisor on a (smooth or singular) irreducible quadric in the class $mH$ or $mH+L$,
where $H$ is the class of a hyperplane and $L$ is the class of a line; if $r>3$ and and $d>2r$ then
$C$ lies on a rational normal scroll in a certain range of divisor classes; see Theorem~\ref{Castelnuovo examples}.
\end{enumerate}
\end{theorem}
...
Show that the line bundle $\cO_C(M)$ is nonspecial. (This is sharp: for example, canonical curves have $M=2$).
\end{exercise}
