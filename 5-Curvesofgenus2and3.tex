...
\fix{Hi! I think I figured out how to get the count of even and odd theta characteristics on a hyperelliptic curve. The calculation below is a work in progress; I hope to have it finished by the weekend}
...
\item 
$$
\sum_{k=0}^n \binom{4n}{4k} = 2^{4n-2} - 2^{2n-1} \quad \text{and} \quad \sum_{k=0}^{n-1} \binom{4n}{4k+2} = 2^{4n-2} + 2^{2n-1}
$$
\item 
$$
\sum_{k=0}^n \binom{4n+2}{4k+1} = 2^{4n} - 2^{2n} \quad \text{and} \quad \sum_{k=0}^{n} \binom{4n}{4k+2} = 2^{4n} + 2^{2n}
$$
\end{enumerate}
\end{lemma}
...
The second and third follow similarly by applying the binomial theorem to the expression $(1 + i)^{4n} = (-1)^n2^{2n}$ and taking the real and imaginary parts
\end{proof}

Now, let $C$ be the hyperelliptic curve of genus $g$ expressed as a 2-sheeted cover of $\PP^1$, with ramification points $p_1,\dots,p_{2g+2}$. We can count the number of theta characteristics on $C$ explicitly as follows:
...
D \sim mE + F
$$
with $-1 \leq m \leq \frac{g-1}{2}$ and $F$ the sum of $g-1-2m$ distinct points $p_i$. Moreover, this representation is unique, except in case $m=-1$ \fix{It seems you mean the case of the sum $F$ of $g+1$ branch points, not $F-E$.} in that case, we note that the sum of $g+1$ of the branch points of $C$ is linearly equivalent to the sum of the other $g+1$ \fix{explain why}. Thus the total number of theta characteristics is a sum of binomial coefficients; if $g$ is odd, it is
$$
...
\section{Exercises}
 \begin{exercise}
 An affine curve $C^\circ$ is defined in the proof of Theorem~\ref{hyperelliptic existence}.  There are natural embeddings of $\AA^2$ in $\PP^2$ and also in $\PP^1\times \PP^1$. In the case $g=1$, show that the closure $\overline{C^\circ}$ of $C^\circ \subset \AA^2$ in each of $\PP^2$ or $\PP^1 \times \PP^1$ consists of the union of $C^\circ$ with one additional point, with that point a tacnode of $\overline{C^\circ}$.
  \end{exercise}
