...
To give a smooth projective model of the hyperelliptic curve $C$ with given branch divisor, we divide the $2g+2$ branch points  into two sets of $g+1$---say, for example, $q_1,\dots,q_{g+1}$ and $q_{g+2}, \dots, q_{2g+2}$. We can then take $C$ to be the closure in $\PP^1 \times \PP^1$ of the  locus
  ...
  $$
To see that $C \subset \PP^1 \times \PP^1$ is smooth we note that it is a curve of bidegree $(2,g+1)$ in $\PP^1 \times \PP^1$, and the formula for the genus of a curve in $\PP^1 \times \PP^1$ derived in Section~\ref{rational quartics section} \fix{adjunction?} tells us that such a curve has arithmetic genus $g$, and thus no singular points.
...
$$
\end{enumerate} 
See~\cite[Chapter 6]{GH}.
%\fix{if we say "elementary", why don't we prove it? The examples are nice anyway. Joe will write. Pf given in GH ch 6.}
\end{fact}
...
\fix{I think this is the first occurrence of the residue in this book. We might need to say more somewhere. insert in Ch 2-RR; with exercise
giving the differentials proof of the RR formula.}
...
As for Theorem~\ref{number of theta characteristics}, it is possible to describe the configurations of odd and even theta characteristics as subsets of the set $S$ of all theta characteristics, which as we've seen is a principle homogeneous space for the group $\Jac(C)_2 \cong (\ZZ/2\ZZ)^{2g}$ of points of order 2 on the Jacobian \fix{add citation of Harris paper}. However, we can instead
...
% \fix{we will have theta on hyperelliptic directly?}
...
\begin{exercise}\label{theta char on genus 2} \fix{will do the case of arb g}
