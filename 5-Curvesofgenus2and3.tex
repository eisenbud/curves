...
Summarizing we have proven:   
   ...
   \end{enumerate}
   \end{lemma}
\fix{why don't we have to mod out the cyclic permuations of the list $\tau_1, \dots, \tau_b \in S_d$?}
...
$\tau_1\cdot \cdots,\tau_{b}$ is also a
 ...
 \fix{why not cyclic sequences? Joe had "collections", which suggests unordered.} $\tau_1,\dots,\tau_b$. In sum, the number of simply branched three-sheeted covers of $\PP^1$ with specified branch points $q_1,\dots,q_b \in \PP^1$ is
$$
...
We say that a theta-characteristic $\cL$ is \emph{even} or \emph{odd} according to the parity of $h^0(\cL)$. Given the irreducibility of the moduli space $M_g$,  Theorem~\ref{locally constant sign} suggests that all curves of genus $g$ have the same number of even theta characteristics. \fix{I changed "shows" to "suggests". Too timid?}
...
The proof of Theorem~\ref{locally constant sign} follows, via an ingenious construction of Mumford's, from an elementary fact about quadratic forms in an even number of variables \fix{if we say "elementary", why don't we prove it? The examples are nice anyway.}. To state this, suppose that $V$ is a complex vector space of dimension $2n$ and $Q$ a nondegenerate symmetric bilinear form on $V$. By an \emph{isotropic plane} we mean a linear space $\Lambda \subset V$ such that $Q(\Lambda, \Lambda) = 0$. 
...
correspond to linear subspaces of dimension 3 in $\CC^6$, which are the isotropic planes in the two distinct components. \fix{say why.} Two of these families of lines intersect in either a point, or coincide; that is, the linear subspaces either intersect  in a line or a 3-plane. See Exercise~\ref{G13}.
...
\fix{I think this is the first occurrence of the residue in this book. We might need to say more somewhere.}

We now introduce two isotropic subspaces for $Q$: first, we set
$$
\Lambda := H^0( \cL / \cL(-D) ).
$$
\fix{This is a priori a quotient, not a subspace. I don't think it's the image of $H^0(\cL)$, either.} 

This is isotropic because the product $\sigma\tau$ of two elements of $\Lambda$ corresponds to a regular differential, and so has no residues. \fix{this would be true if $\Lambda$ was the image of 
$H^0(\cL)$, but why in general?} Second, we set
$$
