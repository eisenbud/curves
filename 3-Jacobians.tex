...
A priori, this is a map of sets. But it is a fundamental fact that both sets may  be given the structures of  algebraic varieties in a natural way, so that the map between them is regular and respects the natural group structures on the two sets. The geometry of this map governs the geometry of the curve in many ways.
We start with the divisors, which in our case are parametrized by the \emph{symmetric powers} $C^{(d)}$ of the curve $C$, described in Section~\ref{symmetric section}. 

The parametrization of the set of invertible sheaves on $C$ of a given degree $d$ by the variety $\Pic_d(C)$ requires different techniques. We will define it by a universal property in the category of schemes, but construct it only as an analytic variety, actually a complex torus, whose group structure reflects the product structure of
sheaves in $\Jac(C) := \Pic_0(C)$, the family of line bundles of degree 0.
...
The existence of these spaces has striking consequences. For example, we show in Theorem~\ref{g+3 theorem} that every curve can be embedded in projective space as a curve of degree $g+3$.
...
\def\Cd{{C^{(d)}}}
\begin{definition}
A \emph{family} of divisors over $C$ is a flat map $X\to C\times B$ whose fibers over points of $B$ are divisors of degree $d$ on $C$ (more precisely:
project isomorphically to divisors of degree $d$ on $C$.)

The universal family of divisors is the pair $(\Cd,\ \sD \rTo^\alpha C \times \Cd)$ such that for any family of divisors  $X\beta C\times B$ there is a unique map
...
From the universal property it is clear that the universal divisor is unique if it exists; we will now construct it, using symmetric products.

If $G$ is a finite group acting by automorphisms on an affine scheme $X:=\Spec A$ then $X/G$ is by definition $\Spec(A^G)$, the spectrum of the ring $A^G$ of invariant elements of $A$. It is a basic theorem of commutative algebra that the map $X\to X/G$ induced by the inclusion of rings is finite, and the fibers of the map $X\to X/G$ are actually the orbits of $G$ (see for example \cite[Theorem ***]{E}).  Since the map $X\to X/G$ is finite, $\dim X/G = \dim X$. The construction commutes with the passage to $G$-invariant open affine sets, and thus passes to more general schemes---and in particular to projective schemes (see exercise~\ref{quotient of projective})---as well.
When the group $G$ is infinite, the situation becomes much more complex---see Section *** (in the Moduli chapter) for some  idea of what can be done.
...
For any quasi-projective variety $X$ we define the $d$-th symmetric power of $X$ to be the quotient of the Cartesian product $X^d$ of $d$ copies of $X$ by the action of the group of all permutations of the factors. The resulting variety $X^d/S_d$ is called the \emph{$d$-th symmetric power}, or \emph{$d$-th symmetric product}, of $X$, denoted  $X^{(d)}$. Since an effective divisor of degree $d$ on a curve $C$ is an unordered $d$-tuple of points on $C$, with repetitions allowed, it corresponds to a point in the \emph{$d$th symmetric power} $C^{(d)}$. For this reason we will write the points of $\Cd$ as $d$-tuples.
...
Finally we come to the construction of the universal family:

\begin{theorem}
 There is a universal family of divisors $\sD\rTo^\alpha \Cd$ where
 $\sD \subset C \times \Cd$ is the incidence correspondence 
$$
\sD := \{(x,(x_1,\dots x_d)) \mid x = x_i\hbox{ for some }i\}.
$$
and $\alpha: \sD \subset C \times \Cd$ is the inclusion.
\end{theorem}
...
gives us the diagram in the Theorem, and we need only prove that the map $\phi$ is a map of schemes. \fix{how to finish?|}
\end{proof}


One aspect of the symmetric powers that is special to the case of curves:  
...
\begin{definition}
 ...
 is equivalent to one whose restriction to $\{p\}\times B$ is trivial. 
 
 Since the Euler characteristic of the sheaves in such a family must be constant, the degree of the restriction of 
...
The goal of the 19th century mathematicians who first described abelian integrals was  to make sense of integrals of algebraic functions. In the early development of calculus, mathematicians figured out how to evaluate explicitly integrals such as
$$
...
$$
When there is no ambiguity about $d$, we will denote them  by $\mu$.  and we 
...
The functorial description of $\Pic_{X/\CC}$ makes it easy to prove a number of properties:

\begin{theorem}
If $X$ is a smooth curve, then $\Pic_{X/\CC}$ is smooth and the tangent space to $\Pic_{X/\CC}$ at any closed point is isomorphic to $H^1(\cO_X)$.
\end{theorem}
...
$B$ and a subscheme $B'$ such that $\cI_{B'/B}^2 = 0$, we must show that any map $B'\to X$ extends to a map $B\to X$.
But a map $B'\to X$ corresponds to an invertible sheaf on $X \times B'$, and similarly for $B$, so we must show that every
invertible sheaf on $X\times B'$ extends to an invertible sheaf on $X \times B$. We will use the identification of the group of invertible sheaves  on a space
with the first cohomology of the multiplicative group of invertible functions on the space.

Note that  $\cI_{B'/B}$ is supported on $B'$, and Let $N$ be the pullback of $\cI_{B'/B}$ to $X \times B'$. From the exact sequence
$$
0\to N \to \cO_{X\times B} \to \cO_{X \times B'} \to 0
$$
we deduce an exact sequence of multiplicative groups 
$$
1 \to (1+N)\to \cO_{X\times B}^* \to \cO_{X\times B'}^* \to 1.
$$
...
$$
H^0 \cO_{X\times B}^* \to H^0 \cO_{X\times B'}^* \to H^1 \cO_X \to H^1(\cO_{X\times T}^*) \to H^1(\cO_{X^*\times B'}^*) \to H^2(N) 
$$
Since $B$ is affine and $X$ is 1-dimensional,  $H^2(N) = 0$, proving that we can extend invertible sheaves. 
...
From the exact sequence above we see that the set of extensions is $H^1(\cO_X)$.
\end{proof}

We can also use the universal property to prove properness:

\begin{corollary}
 $X$ is a nonsingular projective curve, then $\Pic_{X/\CC}$ is proper over $\CC$. 
 \end{corollary}
 
\begin{proof}
We use the valuative criterion of properness.
Thus we consider $D := \Spec R$, where $R$ is a discrete valuation ring, and an invertible sheaf $\cL$ on $X\times U$, where $U$ is the generic point of $D$, 
and we must show that $\cL$ extends to $X\times D$.

Choose a rational section of $\cL$, and let $\ell$ be the associated  Cartier divisor of zeros and poles. Because $X$ is smooth, the closure $\overline \ell$ of $\ell$ in $X\times D$
is again a Cartier divisor, and the invertible sheaf associated to $\overline \ell$ is an extension of $\cL$.
\end{proof}

Since the points of $\Pic_{X/\CC}$ correspond to invertible sheaves, it is not surprising that $\Pic_{X/\CC}$ is an abelian algebraic group in a natural way:

\begin{exercise}
Write $\pi_{1,2}$ and $\pi_{1,3}$ for the projections 
$$
X \times \Pic_{X/\CC} \times \Pic_{X/\CC}  \to X \times \Pic_{X/\CC}
$$ 
onto the $(1,2)$ and $(1,3)$ factors, respectively. 
The map $\Pic_{X/\CC} \times \Pic_{X/\CC} \to \Pic_{X/\CC}$ corresponding to the family $\pi_{1,2}^*\cP \otimes \pi_{1,3}^*\cP$
makes $\Pic_{X/\CC}$ into an abelian algebraic group, with inverse operation $\Pic_{X/\CC} \to \Pic_{X/\CC}$ corresponding
to the sheaf $\cP^{-1}$.
\end{exercise}

\begin{exercise}
Show that if $X$ is an irreducible curve with a smooth closed point $p$ over a field $k$, then tensoring with $cO_X(p)$ induces
isomorphisms $\Pic_{d, X/k} \to \Pic_{d+1, X/k}$ for all $d$. Without assuming the existence of a smooth closed point, show that
$\Pic_{d, X/k} \to \Pic_{d+2g-2, X/k}$, where $g$ is the genus of $X$.
\end{exercise}

\begin{fact}
There are smooth curves over certain fields such that
$\Pic_{d, X/k}$ is not isomorphic to $ \Pic_{e, X/k}$, unless $d\equiv
 \pm e$ (mod $2g-2$) \cite{Kouvidakis}.
\end{fact}

In parallel with the functor $B \mapsto Pic(X/B)$ we define the functor of relative divisors: 
$$
Div(X/B) :=
\hbox{Divisors in $X\times B$, flat over $B$}\}
$$
\fix{DE rev to here 3/13/22}
...
To start with, we can define $W^r_d(C) \subset \Pic^d(C)$ to be  the set of invertible sheaves $L \in \Pic^d(C)$ such that $h^0(L) \geq r+1$. We can see readily that this is a Zariski closed subset of $\Pic^d(C)$, for example by pointing out that it is exactly the locus where the fiber dimension of the Abel-Jacobi map $\mu : C_d \to \Pic^d(C)$ is at least $r$; this is closed by upper-semicontinuity of fiber dimension. 

Note that among the subvarieties $W^r_d$ are the images $W^0_d$ of the Abel-Jacobi maps; in other words, the locus of \emph{effective} divisor classes of degree $d$. The superscript is often omitted, meaning $W^0_d$ is usually written  $W_d$.

Moreover, the subsets $W^r_d \subset \Pic^d(C)$ can be given the structure of a scheme, in a natural way. To set this up, suppose we fix an effective divisor $D$  of degree $m$, with $m$ large enough that $h^0(L(D))$ is the same for all line bundles $L$ of degree $d$---that is, with $m+d > 2g-2$. Now consider the Poincar\'e sheaf $\cL = \cL_{m+d}$ on $\Pic^{d+m}(C) \times C$. The pushforward $(\pi_1)_*(\cL_{m+d})$ is then a vector bundle on $\Pic^{d+m}(C)$ of rank $m+d-g+1$.

Now take the restriction map
%$$
%\cL_{m+d} \to \cL_{m+d}|_{\Pic^{d+m}(C) \times D}
%$$
and push it forward via the projection map to the first factor $\Pic^{d+m}(C)$. We get a map between two vector bundles of ranks $m+d-g+1$ and $m$; and if we identify $\Pic^{d+m}(C)$ with $\Pic^d(C)$ via tensoring with $\cO_C(D)$, the locus $W^r_d(C)$ is exactly the locus where the rank of this map is less than or equal to $m+d-g-r$. We can thus define the scheme $W^r_d$ as the determinantal scheme associated to this bundle map.


One way to characterize this scheme structure is to say that the scheme $W^r_d$ represents the functor of families of invertible sheaves $L \in \Pic^d(C)$ on $C$ with  $h^0(L) \geq r+1$.
