...
$$
\mu : \big\{ \text{effective divisors } on C\big\} \rTo \big\{ \text{invertible sheaves } on C \big\}.
$$
...
We start with the effective divisors. Since $C$ is smooth, effective divisors on $C$ may be thought of as finite subschemes, and thus
...
\def\Cd{{C^{(d)}}}
\begin{definition}
A \emph{family} of degree $d$ divisors on $C$ is a flat map $X\to C\times B$ whose fibers over points of $B$ are divisors of degree $d$ on $C$ (more precisely:
...
\begin{fact}
If $C$ is a smooth curve then $Pic_d(C)$, with its universal bundle, exists as a projective variety, and may be constructed purely algebraically---so that, for example, if the curve $C$ is defined over a given field $K$ then $J(C)$ will be defined over $K$ as well.  
\end{fact}

We will now trace some of the history leading to the analytic construction of the Jacobian. A goal of the 19th century mathematicians was  to make sense of integrals of algebraic functions. In the early development of calculus, mathematicians figured out how to evaluate explicitly integrals such as
$$
...
%a sheaf of degree $d$.
%
\fix{I commented out a page or so of material. We need to decide between notations: $\Pic(C)$ and  $\Pic_{X/\CC}$l}
...
\fix{I commented out the section on the derivative of Abel-Jacobi.}\fix{revised to here}
\section{Further consequences}

The set of $g^r_d$s on $C$---that is, linear systems ofdegree $d$ and dimension $r$ can be given the structure of a scheme in its own right.
We define $W^r_d(C) \subset \Pic^d(C)$ to be  the set of invertible sheaves $L \in \Pic^d(C)$ such that $h^0(L) \geq r+1$. This is  the locus where the fiber dimension of the Abel-Jacobi map $\mu : C_d \to \Pic^d(C)$ is at least $r$, which is closed by upper-semicontinuity of fiber dimension. 
