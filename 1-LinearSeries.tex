...
\centerline {\includegraphics[height=2in]{"main/Fig01-1"}}
 ...
 {Silvio: please show the two divisors, perhaps by color or bracket : p,q,r is one group and p, q', r' the other. The dashed lines could be the corresponding colors too.}}
\end{figure}
...
A simple example will serve to illustrate this. Suppose $C \subset \AA^2$ is a plane curve with a node at a point $p = (0,0) \in C$. The point $p$ itself (that is, the reduced subscheme) is \emph{not} a Cartier divisor, since its ideal $(x,y)$ is not generated by a single element. Likewise, the \emph{fat point}---that is, the subscheme $\Gamma = V(x^2, xy, y^2) \subset C$---is not Cartier. But a subscheme of degree 2 supported at $p$---that is, a scheme of the form
$$
...
Moreover, if $X$ is a \emph{normal} scheme (so that its local rings at the generic points of codimension 1 subvarieties are discrete valuation rings) then the Cartier divisors are the same as locally principal Weil divisors. See~\cite[Section II.6]{Hartshorne1977} for more information. 
...
Here \emph{nondegenerate} means the image of the morphism $\phi$ is not contained in any hyperplane. The phrase ``modulo $PGL_{r+1}$'' is needed because the notation $\PP^r$ supposes a choice of projective coordinates, and
...
by the Veronese morphism of degree 2 now simply called the \emph{Veronese surface}.
\end{example}
...
and we will therefore often speak of \emph{the} rational normal curve.
...
\centerline {\includegraphics[height=3in]{"main/Fig01-2"}}
 ...
 {Silvio: p needs a dot. perhaps add a line from p to the double point, meeting the space curve in two points? figure out a standard way to show projections}}
\end{figure}
...
\begin{figure}
\vbox to 0pt{\centerline {\color{red}IN PROGRESS}\vss\hrule height0pt}
\centerline {\includegraphics[height=2in]{"main/Fig01-3"}}
...
\begin{exercise}\label{here there be basepoints}
 ...
 series of dimension $<r$ on $\PP^r$ has a non-empty base locus. Hint: use B\'ezout's theorem.
\end{exercise}
