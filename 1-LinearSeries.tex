...
\fix{do Serre vanishing and the easy version of Riemann-Roch BEFORE the section on the canonical series; that gives
...
\begin{theorem} (Easy Riemann-Roch)
...
If $\sL$ is an invertible sheaf on a smooth curve, then $\chi(\sO_C) = 1-g$ and
$$
h^0(\sL) - h^1(\sL) = \deg \sL - g(C) +1.
$$
\end{theorem}
...
$$
H^1(C; \CC) = H^0(\omega_C)\oplus H^1(\sO_C) = H^0(\omega_C)\oplus (H^1(\sO_C))H^0(\omega_C))^\vee, 
$$
so $ h^1(\omega_C)$ is half the rank of the middle singular cohomology group, justifying the name ``genus''.
\end{fact}
