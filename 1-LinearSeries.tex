...
The relevant generalization to more general schemes $X$ is that of a Cartier divisor. To simplify matters we will assume that $X$ is a projective variety: a reduced irreducible projective scheme. Write $K(X)$ for the field of rational functions on $X$. 
A Cartier divisor on $X$ is by definition an element of the group $K(X)^*/\CC^*$.

If $C$ is a smooth curve then this corresponds exactly to the definition above:
there is a natural map $\div$
from the group of Cartier divisors on $C$ to the free group on the points of $C$:
 for any rational function $f$ on $C$
and any 
point $p\in C$ the local ring $\sO_{C,p}$ is a discrete valuation
...
$$
Exactly the same argument works on any variety that is regular in codimension 1,
and yields a map from the group of Cartier divisors to the free group on
codimension 1 subvarieties, called the group of Weil divisors. See~\cite{Hartshorne1977} for this and further generalizations. When
