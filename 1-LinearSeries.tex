...
\redden{familiar with this material,}
...
\redden{two-(real-)dimensional} 
manifold with a complex structure; 
\redden{but its}
global complex structure
\redden{can be hard to visualize.} 
\index{Riemann, Georg}%
...
\emdash the number 
of disjoint 
\marginparhere{m $\to$ mm}
closed curves 
\redden{(one-}
\unskip
dimensional real submanifolds) that can be drawn on $X$ such that 
\redden{the complement of their union}
...
were 
subtly different
\marginparhere{ct $\to$ cht;\\ indexed Clebsch}
\redden{from their modern counterparts: whereas} 
...
\redden{nonisomorphic} 
\marginpar{removed hyphen from ``non'' words where no confusion can arise}%
...
\redden{The algebraic and complex analytic theories are equivalent,} 
\marginpar{\redden{The red sentence is ungood.  Let's discuss.}}
and in this book we generally take the algebraic path.

\redden{On a projective variety,} 
the only globally defined regular functions (that is, functions locally 
\index{regular function}%
equal to a polynomial in affine coordinates) are the constant functions. 
\redden{Interesting maps, therefore, must be described and studied using}
...
\marginpar{added ``In...'' to avoid double colon}
...
\marginpar{I'm removing parens around arguments of multi-letter operators when superfluous, ok? (cf.\,$\sin x$).}
...
differ by the 
\marginpar{pull-back $\to$ pullback (for consistency)}
...
\marginpar{\redden{nonzerodivisor without hyphen, ok?}}
...
\marginpar{\vskip 8pt moved the emdash from display to next line, and replaced next pair of emdashes by commas}
$$
...
\redden{For all these assertions see}
\marginpar{streamlined}%
Exercise~\ref{Cartier examples} 
\redden{at the end of the chapter;} 
the reader
...
Moreover, if $X$ is a 
\blue{\emph{normal} scheme} 
\marginpar{is the ``so that'' clause a definition?}%
(so that its local rings at
\index{normal!scheme}{scheme!normal}%
...
\blue{\emph{degree of the morphism}.}
...
$\cO_C(E) \to \cO_C(D)$. 
\marginpar{dropped $\cong$ for better layout; I'm working on a solution for the arrows.}
%$\cO_C(E) \rTo^{\cong} \cO_C(D)$; 
Thus the isomorphism class of the invertible sheaf $\cO_C(D)$
corresponds to the linear equivalence class of $D$. We will see 
\redden{in Corollary~\ref{invertible sheaves and divisors}} 
...
\blue{\emph{value $\sigma(p)$ of $\sigma$ at $p$}} 
...
\footnote{\redden{Note that}
when we  say that 
\def\marginpar{\marginparhere[200pt]}%
\blue{$\sigma$ vanishes at $p$,} 
...
\index{pullback!of invertible sheaf}%
\index{pullback!of line bundle}%
...
\marginpar{to avoid too many dashes}%
...
function, 
\marginpar{dropped ``it follows that'' (you have ``because'' already)}
...
\marginpar{added ``we have''; is this what you mean?}
 $$
 H^0(\sO_{\PP^r}(m)) = 
...
of freedom.%
\footnote{
\redden{This ``$g$'' is unrelated to the genus}; 
