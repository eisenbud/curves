...
that $L_{i} \cong \sO_X(U_{i})$, the free module on one generator. Note that the tensor product of two invertible sheaves is thus again an invertible sheaf, as is the dual invertible sheaf $\hom(\cF, \cO_C)$ of an invertible sheaf $\cF$. Thus the set of isomorphism classes of invertible sheaves on $C$ forms a group with law of composition given by tensor product.
...
differ by a unit in $\sO_{X,p}$, the vanishing locus, denoted $(\sigma)_0$ of $\sigma$ \emph{is} a well-defined subscheme of $X$. Moreover, if $X$ is reduced and irreducible, then the ratio of two global sections is a well-defined rational function, so the divisor class of 
...
If $D\subset X$ is an effective divisor, then we define $\sO_{X}(-D)$ to be the ideal sheaf of $D$. If $D$ is locally defined by the vanishing of a (locally defined) non-zerodivisor in $\sO_{X}$, (that is, $D$ is a Cartier divisor), then
