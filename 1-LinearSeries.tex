...
Morphisms of a smooth curve $C$ (or indeed of any scheme) to a projective space are conveniently studied using the closely related notions of Divisors, linear systems, invertible sheaves, and line bundles.
...
of the covering. \fix{this sounds more advanced than the rest of this section; but it helps make the "linearly equivalent" concept in the next section sensible.}
...
Our goal in the next sections is to explain and prove:
...
$$
\phi^*: \frac{\sO_{D,\phi(p)}}{\mm_{D,\phi(p)}^2} \to \frac{\sO_{C,p}}{\mm_{C,p)^2}
$$
...
$$\sO_{D,\phi(p)}$ is generated as a module by the element 1, and then the fact that
The required finiteness is a consequence of Theorem~\ref{FTET}, below.
\end{proof}

The following result is sometimes called the Fundamental Theorem of
Elimination Theory since it implies that the image of a projective variety is closed; in analytic geometry
it corresponds to the fact that projective space is compact, and thus all projective varieties are compact too.

\begin{theorem}\label{FTET}
If $\phi:X\to Y$ is a morphism of varieties, and $X$ is projective, then $\phi_*(\sO_{X})$ is a coherent sheaf
on $Y$, and in  particular $\ph^*$ makes the local ring $\sO_{X,p}$ into a finitely generated module
over $\sO_{Y, \phi(p)}$ for any $p\in X$.
\end{theorem}
...
$$
Applying \trr we see this holds if and only if $h^0(\cO_C(p+q)) = 2$ for some $p,q \in C$. By Lemma~\ref{deg 2 morphism}, this implies that $C$ is hyperelliptic.
\end{proof}
...
$|\cL|$ defines a morphism of degree 2 to $\PP^{1}$, and $C$ is hyperelliptic. In particular, if $g(C) = 2$ then the canonical series $|K_{C}|$ defines a 2 to 1 morphism to $\PP^{1}$.
\end{lemma}
...
intersection form
Pic , K, adjunction,
description of int pairing on blowup of a smooth surface at a point. can div via rational 2-forms
proper transform of a curve with m-fold point has arith genus reduced by m choose 2.
del Pezzo surfaces (we use Pic of smooth cubic). trans of monodromy is at best a cheerqful fact -- in the Mumford ch.
\input footer.tex