...
The \emph{degree} of  $D = \sum m_p\cdot p$ is by definition the sum $\sum m_p$ of its coefficients. Because the curve is smooth each local ring is a discrete valuation ring, so every effective divisor $D$ is defined locally
on an affine open set by the vanishing of one non-zerodivisor. If effective divisors $D, E$ are defined on
some afiine open set by equations $f$ and $g$, then  $D - E$ corresponds to the rational function $f/g$ on that open set.
...
\end{definition}
 ...
 \subsubsection{What is a rational function?}
These considerations can be extended to Cartier divisors and linear equivalence on any projective scheme $X$. To do so, we define a rational function on a projective scheme $X$ to be a function regular on a dense open affiine subscheme -- that is, the ring of rational functions $K(X)$ is the union of the rings
$A_{\rm tot}$, where $A$ is the coordinate ring of an open  dense affine subscheme and $A_{\rm tot}$ denotes
the ring obtained by inverting all the non-zerodivisors in $A$\footnote{See~\cite{MR570309}
to understand the care needed in dealing with more general schemes.}.
...
Here ``nondegenerate" means the image of the morphism $\phi$ is not contained in any hyperplane, and base-point free means that the scheme $\cap_{\sigma\in V}(\sigma)_0$, called the  \emph{base locus} of $\sV$, is empty; in other words, there is no point of $C$ where all the sections in $V$
...
the vanishing of sections of $\cL$, and which are thus of degree 1. Because $C$ is projective, the map is finite. Thus if $p\in C$ is the preimage of $q\in \PP^1$, the induced map of local rings
...
Via the correspondence of Theorem~\ref{morphisms and linear series}, the statements about the geometry of a morphism $\phi : C \to \PP^r$ can be formulated as statements about the relevant linear series. In the case of complete series, these are statements about the vector space $H^{0}(\sL)$ of global sections of $\sL$. We write $h^{0}(\sL)$ for the vector space dimension of $H^0(\sL)$ (and similarly for other cohomology groups). It is useful to have criteria
