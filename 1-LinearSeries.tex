...
$H^i(X, \sF$ or  $H^i(\sF)$ (our preferred form) without blushing. If $D$ is a divisor on a scheme $X$ we will often
...
or equivalently h^1(\sI_{C/\PP^n}(d))=0$.
\end{definition}
...
\end{proof}
 ...
 We will often analyze curves  on a smooth surface; here are a few results that will be useful. Suppose for this section that $X$ is a smooth projective surface.
...
For proofs and discussion see~\ref[Section V.1]{H} and \ref[Theorem I.4]{Beauville}. 
...
\begin{theorem}
If $\pi: X \to Y$ is a generically finite map of smooth surfaces, then the pullback map on divisors
preserves
\end{theorem}
