...
We remarked at the beginning of Chapter~\ref{linear series} that a smooth projective curve over $\CC$ is the same thing as a compact Riemann surface. In this section, we'll adopt the complex analytic viewpoint, and give an explanation of the Riemann-Roch theorem  (Theorem~\ref{RR theorem}). We will use the fact that meromorphic functions on a Riemann surface are exactly the rational functions on the corresponding
algebraic curve. Thus the Riemann-Roch theorem is equivalent to the statement:

\begin{theorem}
Let $X$ be a compact Riemann surface of genus $g$, and let $D = \sum a_i p_i$ be a divisor ($=$ formal sum of points $p_i$ with integral coefficients $a_i$) of degree $d:= \sum a_i$ on $X$.
The dimension of the space $L(D)$ of meromorphic functions on $X$ with poles of order $a\leq a_i$ at $p_i$ and no other poles, is
$d-g+1$ plus the number of holomorphic differential forms vanishing on each $p_i$ to order at least $a_i$.
\end{theorem}

We will prove this using residues, supposing for simplicity that both $D$ and $K-D$ are reduced ( $a_i=\pm 1$ for all $i$), for some
canonical divisor $K$.
...
Given a reduced, effective divisor  $D = p_1 + \dots + p_d$ on $X$ we will prove the Riemann-Roch formula for the dimension of the space of meromorphic functions on $X$ that are holomorphic on $X \setminus \{p_1,\dots, p_d\}$ and have at most a simple pole at each $p_i$. 

Locally, a function with a simple pole at $p$ may be written in terms of a local coordinate $z$ at $p$ as $c_{-1}z^{-1} +\sum_{i=0}^\infty c_iz^i$;
the term $c_{-1}$ is called its \emph{polar part}.
By the maximum principle, a meromorphic function with poles only at the $p_i$ is determined, up to the addition of a constant, by its polar parts at the points $p_i$; thus we have $\dim L(D) \leq d+1$.
...
This is a priori only an inequality. But we can apply the same logic to a divisor $K-D$, where we assume for simplicity
that this is again reduced, and we see that
\begin{align*}
