...
\section{The canonical morphism}
 ...
 $\omega_C = \sO_C$, and $K_C = 0$. Thus we restrict our attention to smooth curves $C$ of genus $g\geq 2$. 
 
 
 that the canonical series is then base-point free, so $|K|$ defines a morphism.

We will generally begin to study the invertible sheaves on a smooth curve $C$ by studying the complete linear series associated to the canonical divisor $|K|$ and, when $g\geq 2$, the \emph{canonical map}
 $\phi_K : C \to \PP^{g-1}$. 
...
Theorem~\ref{characterization of P1} implies that $d=0$, proving the first statement. 
\end{proof}

\begin{theorem}\label{canonical series is very ample} Let $C$ be a smooth curve of genus $\geq 2$.
\begin{enumerate}
 \item $|K_C|$ is base-point free.
 \item $|K_C|$ is very ample if and only if $C$ is not hyperelliptic.
\end{enumerate}
\end{theorem}
