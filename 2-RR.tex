...
%\fix{I added the word ``projective" in the above. If you had something else in mind, you can change it.}
...
%\fix{Does the reader know what a syzygy is at this point, or the Koszul complex? (We specify Hartshorne as a prerequisite, but Robin doesn't mention syzygies.) I know it's a cheerful fact, where different rules apply, but this could be daunting to a reader unfamiliar with the set-up.}
...
integral, see Exercise~\ref{pa example}.
 ...
 Although the Riemann-Roch theorem for linear series on a smooth curve will suffice for most of this book, we will make
 use of a somewhat more general version in Chapter~\ref{uniformChapter} and, in a more serious way, in Chapter~\ref{linkage}. 
 For example if $\sF$ is a coherent sheaf on an integral projective curve $C_0$, then there is a (generally non-unique) short exact sequence
 $$
...
\begin{theorem} If ${C_0}$ is a projective curve and $\sF$ is a coherent sheaf of rank $r$ and  degree $d$ then
$$ 
...
%\fix{Does the reader know what we mean by ``divisor class" on a singular surface?}
