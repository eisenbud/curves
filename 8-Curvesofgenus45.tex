...
\caption{Canonical curve of genus 4 on a quadric cone, with just one $g^{1}_{3}$.\fix{Silvio: Nice picture!. Maybe make the two $g^{1}_{3}$ arrows different colors? and add their target lines}
}
\label{Fig8.2}
\end{figure}
...
In either case, the one $g^1_3$ on $C$ is the projection from the unique singular point of $\pi(C)$.
...
\begin{proposition}\label{general gonality exclusion chapter}
...
{Silvio: Not clear why one triple of points is distinguished---red. The lines could be a different color than the curve. The surface looks unreasonably flat, too.}}
\end{figure}
...
Let $C$ again be a smooth projective nonhyperelliptic curve of genus 4 whose canonical model lies on a smooth quadric. We have seen that $C$ is birational to a quintic plane curve $C_0$ with two nodes $p, q \in C_0$. Show that the canonical series of $C$ is cut out by the system of plane conic curves passing through $p$ and $q$ in the sense
...
Hint: Try blowing up the plane at the nodes. Look at Section~\ref{canonical series on nodal plane curves} if you get stuck.
\end{exercise}
...
Hint: consider separately the cases where $\Gamma$ contains a fat point (that is, the scheme defined by the square of the maximal ideal at a point) or is curvilinear (that is, has Zariski tangent space of dimension at most 1 everywhere).
\end{exercise}
