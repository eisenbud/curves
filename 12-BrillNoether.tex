...
For the
\marginpar{dropped emdashes}
perhaps more interesting question of when  there
exists a curve of genus $g$ with a 
\blue{birationally very ample $g^r_d$, Castelnuovo's theorem}
...
\marginpar{dropped ``of all'' (``all'' appears next)}
...
\marginparhere{\hsize=2\hsize\redden{This figure said In Progress, but I couldn't find my notes (if there were any) describing its flaws. A \string\fix\ note asked for axes labels (which had already been added) and for color ``to make the regions clearer''; is this what you mean?}}
}
...
\marginpar{\redden{it said ``\string\fix\{char 0?\}}}
...
\marginpar{dropped ``by Brill and Noether'', ``1874''}
...
\marginpar{dropped dots in the parentheses (we could add ``e.g.'' but I don't think we need it)}
...
and set
\marginpar{reworded; adjusted punctuation in items}
$$\rho = g - (r+1)(g-d+r).$$ 
...
\marginpar{removed parentheses around cites, including an unmatched one here} 
$W^{r+1}_d(C)$
...
\marginpar{used multiple remarks instead of enumerate, partly to avoid indentation which was complicating the layout, and partly because the theorem already has items}
...
\redden{E. Larson}
\index{Larson, Eric}%
\marginpar{and edited theorem title}%
gives a
...
\marginpar{\redden{Your note said: This will come after the plane curves: we get the 5-ic del Pezzo for free, given the ``conditions of adjunction'', and then the special cases will give us exercise in what the conditions of adjunction are. But we're running out of time}}
...
\begin{figure}[b]
\hskip0pt
\raise3pt\hbox{\includegraphics[width=0.36\hsize]{main/Fig11-2-new}\quad}
\includegraphics[width=0.34\hsize]{main/Fig11-3}
\caption{Left: A $g^1_4$ as (left)
the projection from a node of a plane sextic 
\marginparhere[-20pt]{\vskip-100pt Left diagram, regarding your note: I didn't add another line since it would either be close to the existing ones or not have 6 real intersections.  I did make the dashed line a tangent as you suggested.  Regarding ``they are all on the same footing'': should I replace dashed by solid?}
and (right)
...
\marginpar{Combined two figures for better layout}
...
\blue{complete canonical series}
...
\marginparhere{\vskip-80pt\hsize=1.9\hsize \redden{``use color for the three lines''} -- done. \redden{``Is the sextic mathematically correct?''} I think so -- It's four lines with a perturbation that grows sextically with the distance to the circle of the nodes, so the branches fold onto themselves. Something like $ (x^2-1)(y^2-1)-\epsilon(x^2+y^2-2)^3 =0$. I forget the exact values.}}
...
\marginpar{didn't index Manin or Beauville, per previous convention; but I will if you confirm}
...
\blue{anticanonical bundle}
\index{anticanonical bundle}%
 is 
\blue{nef but not necessarily ample.}
...
of a quadric
and the 
\blue{cone over the elliptic quintic}
...
\blue{three to one onto a conic}
 or birationally onto a 
\blue{sextic with a triple point}.
...
Hint: Take the image of $C$ under the map associated to the $g^2_6$
...
Hint: the curve in question is the normalization of a plane sextic curve
...
containing $S$. 
\marginpar{replaced hanging left parenthesis by period}
