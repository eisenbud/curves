...
A priori, the Hilbert scheme parametrizes subschemes of projective space. But the construction is adaptable to many other situations. In this section we'll sketch a proof of the following theorem that depends on the existence
of the Hilbert scheme. As we have seen, there can be infinitely many maps from a curve to $\PP^{1}$, and
this is also the case for maps to a curve of genus 1, even modulo the automorphisms of the target. But this is not
the case in higher genus:
...
from a smooth projective curve of genus $g$ to one of genus $h$.
\end{theorem}

A special case of this result is a bound on the size of the group of automorphisms of a curve of genus $g\geq 2$. We will give a complete proof of that result as Theorem~\ref{finite autos}.
...
in $\PP^{g+1}_{S}$
and similarly for $S_{h}$ and curves of genus $h$, and write $([C],[D])$ for the corresponding point in the fiber. From the universal families of curves over $S_{g}$ and
$S_{h}$ we may construct a family $\sC \to S= S_{g}\times S_{h}$ whose fiber over $s$ includes all products of 
...
one fixed curve to another.  By Theorem~\ref{tangent to Hilb} it suffices for this to show that if we fix the curves $C,D$ and a map $\phi$
with graph $\Gamma_{\phi}$
then the normal bundle 
$$
\sN_{\Gamma_{\phi}/(C\times D} = \sT_{C\times D}/\sT_{\Sigma_{\phi}}
$$
has no sections. The projection onto the first factor identifies $\sT_{\Sigma_{\phi}}$ with $\sT_{C}$,
and the quotient is thus $\phi^{*} \sT_{C}$, which has degree $2-2h<0$. Thus, fiber by fiber,
