...
$X\subset B\times \PP^n$  is flat). The points $b\in B$ such that the fiber $X_b$ is smooth and irreducible is an open set.
\end{lemma}


\begin{lemma}\label{smooth is open}
Suppose that $X \to B$ is a flat family of projective schemes (that is, the projection to the second factor of 
$X\subset B\times \PP^r$  is flat) and $B$ is smooth. The points $b\in B$ such that the fiber $X_b$ is smooth and irreducible is an open set.
\end{lemma}

\begin{proof}
To prove the Lemma we may assume that $B$ is affine. Let $x_{0}\dots, x_{r}$ be homogeneous coordinates
on $\PP^{r}$, and suppose that 
$X\subset \PP^{r}_{\CC}$ is defined by the ideal $I_{X} = (F_{1}, \dots, F_{n}) \in B[x_{0}, \dots x_{r}$.
Since $X$ is flat over $B$ the fiber dimension is constant, and the singular locus 
of a given fiber $X_{b}$ is  the scheme defined by the ideal $J$ 
$(r-d)\times (r-d)$ minors of the Jacobian matrix $\partial F_{i}/\partial x_{j}$ intersected with $X_{b}$.
Let $Y\subset B\times \PP^{r}$ be scheme defined by the ideal of minors alone. 
Thus the locus in $B$ over which the fibers are singular is the locus where the fibers of $X\cap Y$
are empty, which is closed by the semi-continuity of fiber dimension.
