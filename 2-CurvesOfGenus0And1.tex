...
As we saw in more generality in Example~\ref{linear series on Pr}, there is for each $d \in \ZZ$  a unique invertible sheaf $\cO_{\PP1} (d)$
...
\begin{proof}
 By \trr, any linear series $\cL$ of degree $d$ on $C$ has $h^0\cL \geq d+1$.
\end{proof}
...
$$
corresponding to by the complete linear series $|\cO_{\PP^1}(d)|$ is called the \emph{rational normal curve} of degree $d$. Rational normal curves arise often in the literature because of their they have many extremal properties. 
...
\fix{where was this Corollary supposed to be? It doesn't seem to exist. We could argue that even the automorphism group of curves of genus $\geq 2$ is finite; and in genus 1 there are no invertible sheaves
fixed by all translations; thus arithmetic genus 0 and thus $\PP^1$. There should be  a simpler proof.}
...
$$
We now use L\"uroth's Theorem: any subfield of $\CC(x)$ having transcendence degree 1 over $\CC$ is equal to $\CC(j)$ for some rational function $j$ .\fix{citation needed. It seems to me that Hartshorne Example 2.5.5 cheats a little.}
