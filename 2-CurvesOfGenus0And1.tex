...
$$
\rho_e: H^0(\O3e) \to H^0(\cO_C(e)).
$$
Since we assume that $C$ is smooth, thus $C\cong \PP^1$,
we may identify $H^0(\cO_C(e))$ with $H^0(\O1{4e})$.
 Since we assume that $C$ is nondegenerate, it does not lie on a hyperplane,
 so for $\rho_1$ is a monomorphism. 
 
The source $H^0(\O32)$ of $\pho_2$ is 10-dimensional, and the target $H^0(\O18)$ is
9-dimensional, so $\pho_2$ has a kernel of dimension at least 1; that is, $C$ lies on
