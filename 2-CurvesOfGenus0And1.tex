...
$h^0( \sL) = \deg \sL$ for any $\sL$ of positive degree. 


We will describe the maps given by
the complete linear systems in the lowest degree cases of interest, 2,3,and 4 and 5. Along the
way we will see evidence for the assertions that there is a 1-parameter family of non-isomorphic
curves of genus 1, and that on each such curve there is a 1-parameter family of distinct
invertible sheaves, assertions that we will make precise in later chapters.

For the rest of the chapter, we fix a smooth irreducible projective curve $E$ of genus 1.

\subsection{Plane cubics}

Let $\sL$ be an invertible sheaf of degree 3 on $E$. We see from Corollary~\ref{degree 2g+1 embedding} that the sections of $\sL$ give an embedding of $E$ as a smooth plane cubic curve; conversely, the genus formula tells us that a smooth plane cubic curve indeed has genus 1. 

We claim that once we fix a point as the origin, the points of $E$ form a group in a natural way that mimics
the : to add two points, we
...
Let $E$ be a smooth projective curve of genus 1 and let  $\sL$ be an invertible sheaf of degree 2 on $E$. By \trr{},\kern -3pt $h^0(\sL) = 2$ and the linear series $|\sL|$ is base point free, so we get a map $\phi : E \to \PP^1$ of degree 2. By \trh the map $\phi$ will have 4 branch points; by the remark above, these four points are determined, up to automorphisms of $\PP^1$ by the curve $E$, and are independent of the choice of $\sL$.
