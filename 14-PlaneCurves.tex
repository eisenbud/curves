...
%\fix{if this chapter comes before ch 3, we need to say more.}
...
\pict{illustrate this (actually, we do have a picture in Chapter 3)}
\end{example}
...
Given a divisor $D = D_{+}-D_{-}$ on $C$, we will compute the complete linear series $|D|$, using a birational map $\nu$ from 
...
\pict{illustration of a case where $D_{+}$ contains one point of $\Delta$.}
