...
From the point of view of Brill and Noether the existence of the form $H$ in the proof above comes about
in a different, though closely related way: Since $F$ is irreducible, 
...
formula(Proposition~\ref{adjunction} and the examples in Sectioin~\ref{pa and pg}, the genus $g$ of the normalization $C$ of $C_{0}$ is
...
\fix{Could we prove the completeness by doing one blowup and an induction, assuming that iterated blowups
will give a resolution for the curve singularity?}
...
each $r_i$. It follows that for $e(x,y)dx/f_y$ to be regular, $e$ must vanish to order $n-1$ at each $r_i$. 
 \fix{why does this imply $e\in \gm_q^{n-1}$?}
