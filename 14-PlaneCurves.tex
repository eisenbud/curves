...
%%\end{enumerate}
%
%In Proposition~\ref{nodal projection}  we  showed that every smooth curve has a nodal plane model---that is,
...
%
%To explain the constructions  we start with the simplest case, where $C = C_0$ is a smooth plane curve.  
%
...
%
%\begin{figure}
%\centerline {\includegraphics[width=2.6in]{"main/Fig14-1"}}
%\caption{If $H$ and $G$ have the same degree, then $r+s+t-u\sim p+q$ on $C$.
%{Silivo, Silvio: C should be red; G,H green and blue. G,H represent curves of the same degree; maybe use
%mirror images so they look similar. C is different -- any (plane curve). }}
%\label{Fig14.3}
%\end{figure}
%
...
on $C$, then $D$ is linearly equivalent to the pullback of a divisor supported on the smooth locus of  $C_{0}$. More precisely, and more every divisor on $C$ can be written as the pullback
of a Cartier divisor on $C_{0}$ minus the divisor $\Delta$ which is the preimage of the union of
the nodes.
\end{lemma}
...
so there is a divisor $D'\sim D+m\nu^{-1}(p)$ such that $D'$ is contained in the preimage of the 
smooth locus of $C_{0}$. It follows that $\nu^{-1}(\nu(D')-mp) \sim D$, and $\nu(D')-mp$ is contained
in the smooth locus of $C_{0}$.

For the last statement, see the more general version, Theorem \ref{Cartier on C}.
\end{proof}
...
If $D$ is a (Cartier) divisor on $C$, then $D+\Delta$ is the pullback of a Cartier divisor on $C_{0}$. 
\end{theorem}

\begin{proof}
We first establish the result for $\Delta$ itself. Since our ground field $\CC$ is infnite, 
the conductor ideal $\ff_{C/C_{0}}$ is the integral closure of a general generator $s\in \ff_{C/C_{0}}$
(see for example \cite[Chapter 8]{Swanson-Huneke}).
This means that for sufficiently large $n we have $x$\ff_{C/C_{0}}^{n} = $\ff_{C/C_{0}}^{n+1}$.
Let $\Delta_{0}$ be the Cartier divisor on $C_{0}$ defined by $x$,
Considering everything as ideals of $\sO_{C}$, we get
$$
\nu^*(\Delta_{0})+n\Delta = (n+1)\Delta
$$
whence $\nu^*(\Delta_{0}) = \Delta$.

 The result is local on $C$, and obvious for divisors supported away from the singular locus $\Delta$,
 so it suffices to prove that if $D$  is  an effective divisor on $C$ then
 $\Delta+D$ is the pullback of a Cartier divisor on $C_{0}$.
 If a local equation defining $D$ is $g$ then $\Delta + D$ is defined by $\ff_{C/C_{0}}g$,
 which is integral over $xg$, and this defines a Cartier divisor on $C_{0}$
 that pulls back to $\Delta+D$.
 \end{proof}

\fix{clarify here and perhaps in the nodal curve section what the following theorem has
to do with the previous theorem: if we didn't include the conductor, there might not be
a form cutting out $D$ locally.}
