...
$e(x,y)$ is a rational function of degree $\leq d-3$.
...
$$
Since $\sO_{C_{0}}\subset \sO_{C}$, This sheaf is naturally contained
in 
$$
...
$\kappa(C) = \kappa(C_{0}) $ it is a well-known result from Galois theory that 
$$
...
with the map $(1\mapsto dx) \in Hom_{\CC(x)}(\kappa, \CC(x))$. Thus by Theorem~\ref{Kunz}
...
\begin{exercise} \label{computing delta}
Suppose that a plane curve $C_{0}$ has multiplicity $m$ at a point $p$. Let $\pi: B \to \PP^{2}$
be the blowup of the plane at $p$, and let $E = \pi^{-1}(p)$ be the exceptional fiber.
 Show that the \emph{total transform} of $C$, which is the preimage $\pi^{-1}(C_{0})$, is a divisor of the form
$C_{1}+mE$, where $C_{1}$ is the closure of $\pi^{-1}(C_{0}\setminus p)$, called the \emph{strict} or \emph{proper} transform of $C$.

Using the adjunction formula on $B$, conclude that 
$$
p_{a}(C_{1}) = p_{a}(C) - \binom{m}{2}.
$$ 
It is a fact that any plane curve can be desingularized by
iteratively blowing up of singular points of $C$, then of the strict transform, and so on.
...
where the sum runs over all \emph{infinitely near} singular points $q$--that is, singular points of strict transforms
of $C$ lying over $p$.
\end{exercise}
\input{footer}