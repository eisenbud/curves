...
Consider the
regular 1-form $dx$ on $\AA^2$, which we may regard as $\pi^{*}(dx)$, pulled back from $\AA^{1}$.
...
It is possible to carry out an analysis of linear series on the normalization of an arbitrary plane curve in a manner  analogous to what we did in the preceding section for nodal curves, replacing the the set of nodes by the scheme $\adj(C_{0})\subset C_{0}$ called the \emph{adjoint scheme}, which is defned in this case by the conductor ideal $\ff_{C/C_0}$.
...
$$
Since $\psi(1) = \sum e_i$, the module $R'/R$ is generated by $e_1,\dots, e_n-1$. We can write some of the relations of $R'/R$ as
...
%\fix{I think this can't be right: a sheaf of length 2}
...
%In this case, in contrast to case of the planar triple point, the condition that $f(r)=f(s)=f(t)$ is both necessary and sufficient for $f$ to descend, and thus  $\delta_p=2$. However $\ff_{C/C_{0}} = \fm_{p}^{2}$, so for a function
%to lie in 
%$\ff_{C/C_{0}}$
%is 3 conditions.
