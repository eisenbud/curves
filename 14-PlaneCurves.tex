...
on $C$, then $D$ is linearly equivalent to the pullback of a divisor supported on the smooth locus of  $C_{0}$.
\end{lemma}
...
\fix{I'm sorry to leave a fix comment behind, but I wasn't able to parse this proof. Do we know that $\pi_{*}\pi^{*}(\sL)$ is invertible? If not, how do we deduce that the map $\pi_{*}\pi^{*}(\sL) \to \sL$ is an isomorphism? Maybe we can talk about this tomorrow. }
...
away from the codimension 2 set of nodes, and is thus an isomorphism. 
\fix{Proof: $H^{0}_*(\pi_{*}\pi^{*}(\sL))$ has depth $\geq 2$ so the long exact sequence
$$
0\to H^{0}_{*}(\pi_{*}\pi^{*}(\sL))\to H^{0}_{*}(\sL) \to H^{0}_{*}(points) \to\cdots
$$
must have the first map surjective.}
