...
\fix{Having written this out, I have to ask: do we need this here?}
...
general and show that if $\nu: C\ to C_0$ is the normalization of a reduced, connected
...
\fix{we talk about $\delta(C)$ in an early chapter. Should the following ``deficiency'' description be there instead of here?}

For plane curves the number $\delta(C_0)$,  called the deficiency, has a long history.

For an ordinary node its value is 1, and it may be regarded as the ``number of nodes equivalent to the singularities of $C_0$'' as
...
defined an invariant called the \emph{deficiency} that is equal to the genus \fix{I think this should be 
$\binom{d}{2} -g$.} When Clebsch, Brill and Noether tried to 
...
\fix{I have a suggestion. You've pointed out that this chapter is over-long, and beyond that the material from here on feels very different in style from what's come so far. So I'd propose declaring the chapter over at this point, and moving this last section (The canonical sheaf of a singular curve) to a later chapter.}
...
use: $n\leq 2m+1$ points impose indep cond on curves of degree $m$ in the plane unless they contain m+2 colinear points. Why no case where $\rho>0$?}


\fix{Does the reader need to use ``conductor equals adjoint'' to do the next exercises? If so, we should say that before this series.}

\begin{example}[tacnodes]

\fix{What's an example doing in the midst of the exercises? -- probably should move back into
the text someplace or be changed to an exercise ``compute the adjoint ideal of a tacnode''.}
