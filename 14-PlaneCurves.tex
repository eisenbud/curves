...
$\AA^2$ has a pole of order $d$ along the line $L$ at infinity. Thus if $h$ has degree at least 2 then $dx/h$ will have no poles at infinity. However, $h(x,y)$ will vanish at points of $C \cap U$, and this may create new poles of $dx/h$. Of course if $h$ vanishes only at  points of $C \cap U$ where $dx$ already has a zero, the zeroes of $h$ may cancel the zeroes of $dx$ rather than creating new poles.
 ...
 $$
Because $C$ is smooth, the functions $\frac{\partial f}{\partial x}$ and $\frac{\partial f}{\partial y}$ have no common zeroes on $C^\circ$. Moreover, the differentials $dx$ and $dy$ have no common zeros on $C^\circ$. It follows that at for all $p \in C^\circ$ we  have
$$
...
the restriction to $C$ of $Q/H'P$ is a regular function, a section of $\sO_C(m)$.
Since $C\subset \PP^2$ is arithmetically Cohen-Macaulay (see Proposition~\ref{ACM basics}), this implies that
$Q/H'P$ is the restriction of a form $H$ of degree $m$ on $\PP^2$; that is, if $G$ is
the defining equation of $C$, then $H'P = QH+LG$ for some form $L$ on $\PP^2$.
...
There is one important additional fact about the adjoint ideal that we should mention here: it coincides with the \emph{conductor ideal} of $C_0$, which is to say the annihilator of the sheaf $\nu_*\cO_C / \cO_{C_0}$. This is a reflection of the fact that plane curve singularities are \emph{Gorenstein}; for more details, see~\cite{}.
