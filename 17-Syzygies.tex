...
\fix{revision idea: Use WMACE to prove ACM $=$ short resolution.
}
...
The first examples of a minimal free resolution are the Koszul complexes, which resolve $/I$ when
 $I = (f_1,\dots, f_t)$
...
$$
0\lTo S/I \lTo^\epsilon S\lTo \oplus_{j=1^3}S(-d_j)^{
 \begin{pmatrix}
f_1&f_2&f_3
\end{pmatrix}} \lTo\cdots
 \lTo^{\phi_t}\oplus_jS(-j)^{\beta_{t,j}}\lTo 0,
$$
...
Minimal free resolutions of a given module $M$ are all isomorphic, and the \emph{projective dimension}  $\pd(M) = \pd_(M)$ is their length, the number of nonzero
maps. Equivalently, 
$$
...
If $N$ is a finitely generated graded module over $S = \CC[x_0,\dots, x_r]$, then, taking account of the grading, there is a minimal graded free resolution $\FF$
...
%Here is the connection between these notions: 
%
