...
We can use the uniquenss of minimal free resolutions to give a simple proof of the classification of 1-generic $2\times f$
 matrices announced in Chapter~\ref{Scrolls} first done by Kronecker~\cite{Gantmacher}.
 
\begin{theorem}\label{matrix pencils}
 Every
 1-generic $2 \times f$ matrix of linear forms can be transformed by row and column transformations and a linear change
...
is projective (= locally free) if and only if $I(\phi) = R$. If $I(\phi)$ contains a nonzerodivisor, then
...
a regular local ring of dimension $r$, and suppose that the complex $\FF$ is acyclic. The hypothesis $\grade I(\phi_{d+1}) \geq d+1$ can only be satisfied if $I(\phi_{d+1} = R$ (so that its grades is $\infty$ by convention). This  is equivalent to the cokernel of $\phi_{d+1}$ being free. Thus the theorem ``explains'' why a minimal free resolution
has length $\leq r+1$.
...
of the the resolution $(\FF, \phi)$ is again acyclic, so it is the minimal free resolution of $\omega_{R_C}$. Thus 
...
$$
so $\coker \phi_{r-2}^* = R_C(r)$. Thus $(\FF^*(-r), \phi^*)$ is a minimal free resolution of $R_C$, and is thus isomorphic to
...
In this subsection we let $\phi:F = R^{f}\to R$ is a homomorphism from a free module to a ring $R$, and we
we will annalyze the Koszul complex $K(\phi)$. 
...
We now assume only that $\grade I\geq f$.  Theorem~\ref{WMACE}: From what we just proved we see that if we localize
$R$ by inverting any element of $I$ the complex $K(\phi)$ becomes split exact. Since $\grade f\geq 1$,
we can find such an element that is a nonzerodivisor, so inverting it does not change the ranks of the 
...
a nonzerodivisor in $I\delta_{1})$ we have $I(\delta_{i}) = R$; equivalently, 
$$
I  \subset \sqrt {I(\delta_{i})}.
$$
(In fact $I(\delta_{i}) = I^{\binom{i-1}{f-1}}$, though this requires a separate argument.)Thus if $\grade I = f$ then  $\grade I(\delta_{i})$ for all $i$, so $K(\phi)$ is acyclic.
\end{proof}
...
directly, but in any case easy to see that its $a\times a$ minors of $\phi_{a}$ generate this ideal, which has grade 2, so the Eagon-Northcott resolution $EN(\phi)$
has the form
$$
...
$$
and we have proven in Theorem~\ref{rankG1} that it is a free resolution of $R/(G)=S$, which appears
...
\fix{include all for the following:}
