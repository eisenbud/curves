...
it is tautological: its existence depends only of the definition of a commutative ring; and like the Koszul complex it is exact or not depending on the properties of the matrix $\phi$. 
...
section of this chapter. We will also use the complex to give a proof of the classification of matrix pencils.

To provide intuition for Eagon-Northcott complex and its properties, we first revisit the Koszul complex ($\rank G = 1$) and then the special case $\rank G = \rank F -1$.
...
$S:=\ZZ[x_{1}\dots, x_{f}]$, since when $K(\phi_{1},\dots, \phi_{f}) = R \otimes_{S}K(x_{1},\dots, x_{f})$
...
$$
(where the 0 on the right can be interpreted as $\wedge^{f+1}F = 0$.)
...
ring, the image of a basis vector of $\wedge^{i}F^{*} $ is a sum of linear forms times basis vectors
...
$\phi$ sending $V\subset F$ by the identity to $V = S_{1}\subset S$. -- the ``tautological map''. 
...
$$
to be multiplcation by $t$.

Having defined the complex $K(\phi) = EN(phi)$ in the case $\rank G = 1$, we next ask what conditions on $\phi$
make it acyclic (that is a free resolution). 

\begin{theorem}
 Suppose that $\phi_{1}, dots, \phi_{f}\in \CC[x_{0,\dots, x_{r}}$ are homogeneous elements of positive degree.
 The complex $K(\phi)$ is acyclic if and only if the ideal $(\phi_{1}, dots, \phi_{f})$ has codimension $f$, the maximum
 possible.
\end{theorem}

\begin{proof}
Theorem~\ref{WMACE} immediately implies that if $K(\phi)$ is acyclic then
$\rank \delta_{f} = 1$ and $\codim I_{1}(\delta)_{f} \geq f$, proving one implication.

To prove that if the codimension condition is satisfied, we first prove that 
the complex is split exact when $\phi: F\to S$ is a surjection,
and therefore split, or equivalently $S\to F^{*}$ is a split injection. We may then write $F^{I} = S\oplus F'^{*}$ in such a way that $\phi*$ is the injection into the first summand , and we may
choose a basis $\{e_{i}\}$ of $F^{*}$ so that the last $f-1$ basis elements are a basis for $F'*$. 
Specializing the sequence $x_{1},x_{2}, \dots, x_{f}$ to the sequence $1, 0,\dots, 0$, the differential,
becomes the multiplication by  $1\otimes e_{1}$.

The module
$\wedge^{i}F$ naturally decomposes as 
$$
\wedge^{i}F = Se_{1}\otimes_{S} \wedge^{i-1}F^{*} \oplus \wedge^{i}F'^{*}
$$
and because $e_{1}\wedge e_{1} = 0$ the differential
$$
\partial: Se_{1}\otimes_{S} \wedge^{i-1}F^{*} \oplus \wedge^{i}F'^{*} \rTo Se_{1}\otimes_{S} \wedge^{i}F^{*} \oplus \wedge^{i+1}F'^{*}
$$
$$
\begin{diagram}
Se_{1}\otimes \wedge^{i}F'^{*} &\rTo^{0}&  Se_{1}\otimes \wedge^{i}F'^{*}\\
 \bigoplus&\ruTo^{\cong}&\bigoplus\\
 \wedge^{i}F'^{*}&\rTo^{0}& \wedge^{i}F'^{*}
\end{diagram}.
$$
...
\begin{theorem}\label{Eagon-Northcott}\label{E-N}
 ...
 $\phi: F  \to G= S$ be a homomorphism of degree 0, and let $n := \rank F, m:= \rank G$. If $I_m(M)$ has codimension $\geq n--m+1$, then the minimal free resolution of $S/I_m(M)$ has the form:
\begin{align*}
EN(M) := 
S \lTo{\bigwedge^m \phi} 
 \bigwedge^2 F&
 \lTo^{\delta_{2}}
...
Note that if $m = 1$ this looks like the Koszul complex -- it is!
