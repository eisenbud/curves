...
However, as we shall show in Section~\ref{good projections}, we can embed $C$ as a curve $ C \subset \PP^r$ in a higher-dimensional projective space and find a projection $\PP^r \to \PP^2$ that carries $C$ birationally onto its image $C_0$, called a plane model of $C$. The curve $C_0$ will have singularities, and $C$ is the normalization of $C_0$. Brill and Noether wanted to prove the Riemann-Roch theorem for $C$ by formulating and proving a related theorem for $C_0$. In particular, they tried to characterize linear equivalence of divisors on $C$ in terms of certain ``clusters'' of points---we would say 0-dimensional subschemes---of $C_0$. 
...
$E := D'-D$ can be defined with properties such as that $D'-E = D$; Brill and Noether seem simply to have assumed that this is so. Frances Sowerby Macaulay proved that this is possible a bit later, and also understood that this would not generally be possible if $D$ were the intersection of three or more curves. 

Macaulay exploited this theory of residualtion  to prove what he called the ``Generalized Riemann-Roch Theorem'' (now erroneously called the ``Cayley Bacharach Theorem''.) This early work of Macaulay led directly to his definitions of  ``perfection'' (a homogeneous ideal
...
\begin{exercise}
 Show that no hyperelliptic curve of genus $\geq 2$ can be embedded as a smooth curve in $\PP^2$.
\end{exercise}
...
\fix{move the theorem of existence to the Jacobian ch as a corollolary of $g+3$ - but maybe do this more elementary argument too?}
...
This follows immediately from the uniform position lemma: this says that if $H \subset \PP^3$ is a general plane, then the points of intersection $p_1,\dots,p_d \in C \cap H$ are in linear general position in $H$; that is, no three are collinear. The line joining any two is thus a secant line to $C$ but not a trisecant line, which suffices to prove our assertion.

\begin{exercise}
Show that if every secant is a trisecant, then every pair of tangent lines is coplanar, and thus projection from a tangent line is inseparable, which cannot happen in characteristic 0.
\end{exercise}
...
\section{Using plane models to compute linear series} \label{good projections}

To summarize: given the equations of a smooth curve $C$ embedded in some projective space, and the coordinates of a collection of points $p_i$,  we can project it birationally to a (possibly singular) plane curve $C_0$, and use this ``plane model'' to explicitly and algorithmically determine the complete the linear series 
$|\sum_i p_i|$. 
...
We start toward our first goal by writing down a single rational 1-form on $C$: just take a regular 1-form on $\AA^2$, such as $dx$, and restrict/pull back to $C$. Though $dx$ is regular on $\widetilde C$, writing
$$
...
\section{Reduced irreducible plane curves}\label{arbitrary plane curves}

In this section, we'll deal with a plane curve $C_0$ that is assumed to be reduced and irreducible, with normalization $\nu : C \to C_0$. 
...
And that is how we extend the notion of genus to arbitrary singular curves $C_0$: for any 1-dimensional scheme $C_0$ over a field, we define the \emph{arithmetic genus} of $C_0$ to be $p_a(C) := 1-\chi(\cO_{C_0})$. By contrast, the genus of the normalization $\nu : C \to C_0$ is called the \emph{geometric genus} and often denoted $g(C_0)$ or $p_g(C_0)$.
...
Suppose again that $C_0 \subset \PP^2$ is reduced and irreducible, with normalization $\nu : C \to C_0$, and we want to extend our solution to the keynote problems---finding all regular differentials on $C$, and finding all divisors on $C$ linearly equivalent to a given $D \in \Div(C)$---to this more general setting.

In this setting, if we simply try to mimic the analysis above in the nodal case we're led to introduce the \emph{adjoint ideal} of each singularity; in the case of a node, this is the maximal ideal of the point), in terms of which we have theorems analogous to the results obtained above in the nodal case.
...
(H) = F + A + \Delta + D'
$$
we arrive at an effective divisor $D'$ on $C$ linearly equivalent to $D$. Indeed,  the analog of the theorem of completeness of the adjoint series tells us that we arrive at \emph{every} effective divisor $D'$ on $C$ linearly equivalent to $D$ in this way. \fix{What do we need  about the embedded resolution of the curve singularity to prove the vanishing of cohomology as in the nodal case? I think we should do embedded resolution somewhere.}
...
\fix{needs a proof! Also, admit that this is only an algorithm if one has an algorithm for normalization;
we never treated embedded resolution, but probably we should somewhere.}
...
To see how this goes, let's start with a smooth plane curve $C \subset \PP^2$ of degree $n$, and suppose that $D = p_1+\dots + p_d$ is a divisor of degree $d$ ()assumed effective for simplicity). As we described in Section~\ref{}, to find the complete linear system $|D|$ we start by choosing a plane curve $B$ of degree $m$ containing $D$ but not containing $C$; again for simplicity of notation we'll assume $m \geq n$. The intersection $B \cap C$ will then consist of $D$ plus another divisor $E$ on $C$, of degree $mn-d$; and what we saw is that the complete linear series $|D|$ will be cut on $C$ by plane curves of degree $m$ containing $E$, modulo those containing $C$.

How large a linear system is this? Well, if the $(mn-d)$ points of $E$ impose independent conditions on curves of degree $m$, the space of curves of degree $m$ containing $E$ will have dimension
$$
...
$$
so the expected dimension of $H^0(\cO_C(D))$ is
$$
h^0(\cO_C(D)) = \binom{m+2}{2} - (mn-d) -  \binom{m-n+2}{2} = d + 1 - \binom{n-1}{2}.
$$
As our distant forebears put it, a smooth plane curve of degree $n$ has deficiency $\binom{n-1}{2}$.

Now suppose the curve $C$, instead of being smooth, has a node at a point $p$; let $D$ again be a divisor of degree $d$, whose support we assume does not contain $p$. If we take our curve $E$ to pass through the point $p$, then the residual intersection $E$ will consist of $mn-d-2$ points rather than $mn-d$. To construct the complete linear system $|D|$, then, we take curves of degree $m$ passing through this residual divisor $E$ and passing through $p$.
...
$$
so the deficiency of the curve is $\binom{n-1}{2}-1$, or one less. More generally, if $C$ has $\delta$ nodes, we can get this two-for-one deal $\delta$ times, resulting in a deficiency of $\binom{n-1}{2} - \delta$, giving us an early version of the genus formula for plane curves with nodes.

What about other singularities? For example, what if $C$ has a triple point $p$? In this case, we can get a ``three-for-one" deal by requiring our curve of degree $m$ to pass through $p$; as in the nodal case, this means the residual divisor $E$ will have degree 3 less; again, the curves of degree $m$ that cut out the linear series $|D|$ only have to satisfy the one additional condition of passing through $p$, so the apparent deficiency will be 2 less than for a smooth plane curve of degree $n$.

But we can do better! If we require our initial curve of degree $m$ to be double at $p$, that reduces the degree of the residual divisor $E$  by 6. Of course, now the curves of degree $m$ containing $E$ that cut our the linear system $|D|$ have to double at $p$ as well, but this is only 3 linear conditions; in effect, we have got a ``six-for-three" deal, suggesting (correctly, as we know) that the presence of a triple point should reduce the deficiency by 3. 
...
In this way, our ancestors were able to both define the genus (a.k.a. deficiency) of a plane curve, and also arrive at the genus formula, long before the introduction of modern definitions of genus.
...
The family of reduced irreducible plane curves of degree $d$ is an open subset of the space 
$\PP^{{d+2\choose 2}}$, the space of forms of degree $d$ up to scalars---it is the complement of the union of the images of the maps 
$$
\PP^{{d_1+2\choose 2}}\PP^{{d_2+2\choose 2}} \to \PP^{{d+2\choose 2}}
$$ 
with $d_1+d+2 = d$ given by multiplication of forms.
All plane curves $C$ of degree $d$ have Hilbert polynomial 
$$
\chi(\sO_C(m)) = md+(1-{d-1\choose 2})
$$
as follows from a computation of the Hilbert function, and thus arithmetic genus 
$p_a = {d-1\choose 2}$. But depending on the singulariities of the $C$, the geometric genus can vary from 0 up to $p_a$. \fix{ the discussion of arithmetic vs geometric genus should go somewhere else.}.

Our first goal in this section
is to describe an open subset of those curves of degree $d$ with given geometric genus $g$, and compute its dimension. We will also prove the existence of specializations from a smooth projective curve of genus $g$ to a general $g$-nodal curve, a result used in Chapter~\ref{InflectionsChapter}.

%Our main goal will be to give a description of at least an open subset of the Severi variety parametrizing plane curves of degree $d$ and geometric genus $g$; 
%we'll be able to deduce the fact that the Severi variety always has the expected dimension; and we'll be able to prove 

%\subsection{Definitions, and the statement of our main theorem}

Fix a degree $d$, and let $\PP^N := \PP^{{d+2\choose 2}}$ be the projective space parametrizing plane curves of degree $d$.
\begin{propdef}

\begin{enumerate}
\item The \emph{Severi variety} $V_{d,g}$ is the locus of reduced and irreducible curves of degree $d$ and geometric genus $g$. This is is a locally closed subvariety of $\PP^N$. 

\item The \emph{small Severi variety} $\widetilde V_{d,g} \subset V_{d,g}$ is the locus of plane curves of degree $d$ with $\delta = \binom{d-1}{2} - g$ nodes and no other singularities. This is an open subset of $V_{d,g}$.

\item The \emph{large Severi variety} $\overline V_{d,g}$ is the closure of $V_{d,g}$ in the projective space $\PP^N$.
\end{enumerate}

\end{propdef}

\fix{THE Severi variety is actually never mentioned again; only the small and the large. Are its singularities well-understood? Say something about this?}

\begin{proof}
  \fix{ Insert some proof of something!}
\end{proof}

We will see that in a neighborhood of $\widetilde {V}_{d,g}$,  the large Severi variety is well behaved; but away from this,
...
\fix{We should at least mention the irreducibility result as a "fact"!}
...
$$
and the universal singular point is given (in characteristic 0!) as the common zero locus of the three partial derivatives $\partial F/\partial X$, $\partial F/\partial Y$ and  $\partial F/\partial Z$. \fix{char p?}
...
This in turn gives us a beautiful picture of $\overline V_{d,g}$ in a neighborhood of a nodal curve:
...
\end{lemma}
\begin{proof}
 Let $C$ be a rational nodal curve of degree $d$, with nodes $p_1,\dots,p_\delta$ (so that $\delta = \binom{d-1}{2}$). In a neighborhood of the point $C \in \PP^N$, the discriminant hypersurface $\Delta$ will consist of $\delta$ smooth sheets intersecting transversely. What's more, if we set $\delta' = \binom{d-1}{2} - g$, we see that in that neighborhood the variety $V_{d,g}$ will consist of the $\delta'$-fold intersections of these sheets. In other words, we have established the
\end{proof}
