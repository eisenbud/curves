...
\fix{The next two sections are applications of the linearly general position theorem. Maybe state Noether's theorem just in the reduced irreducible case,
(we still need to justify RR and Serre for this case) and move the next two sections back to Ch 8}
...
If $C\subset \PP^r$ is a reduced irreducible projective curve, embedded by a complete linear series $|D|$ of degree $d\geq 2p_a(C)+1$, then
...
This implies that
the restriction map $H^0(\sO_{\PP^{r}}(2))\to H^0(\sO_H(2)) \to H^0(\sO_\Gamma(2))$ is surjective, whence $H^1(\sI_\Gamma(2)) = 0.$
...
Since $V\sO_C = \sO_C$, it follows that the Koszul complex
$$
...
canonical curves are arithmetically Cohen-Macaulay, a result (in the smooth case) of Max Noether~\cite{Max Noether} that we have been using ever since Section~\ref{Noether theorem section}.
The necessary modification is to replace the base-point-free pencil with one having many base points.
We follow the treatment in \cite{Schreyer}, which allows us to treat a rather general case:
...
$C$ is (locally) Gorenstein---that is, the dualizing sheaf $\omega|_C$ is invertible. Thus the Riemann-Roch theorem and duality work as expected. 
In particular, since $h^0(\sO_C) = 1$ and $h^0(\omega_C) = g$,  duality gives
$\chi(\sO_C) = 1 - h^0(\omega_C)= 1-g$ and thus $\chi(\omega_C) = g-1$.  Using this we get
...
For $m = 0,1$ the conclusion is immediate.

For $m>0$ we must show that $H^0(\sO_C(-m)=0$. Let $D$ be the intersection of $C$
with a hypersurface of degree $m$, so that there is a left exact sequence
$$
0\to H^0(\sO_C(-m)) \to H^0(\sO_C) \to H^0(\sO_D).
 $$ 
 By hypothesis, the vector space $\HH^{0}\sO_{C}$ is spanned by the constant functions, and these
restrict non-trivially to $\sO_{D}$, so $\HH^{0}(\sO_{C}(-m))$, is 0 as claimed.
...
Let $\Gamma = \{p_0,\dots, p_{g-3}$ be the $g-2$ points in which a
simple $(g-1)$-secant $(g-3)$-plane $\Lambda$ meets $C$, and let $V$ be the span of the two sections of $\omega_C = \sO_C(1)$
vanishing on $\Gamma$. Note that since $\Gamma$ is a union of components of a hyperplane section of $C$, it is a Cartier divisor on $C$.
...
$$
for some invertible sheaf $\sL$, and we must show that the right-hand map is surjective on global sections for $m\geq 2$.
...
Thus it suffices to show that $H^1(\sO_C(m-2)H+\Gamma) = 0$ for $m\geq 2$.
...
$p(0) = 1-g$ and $p(1) = g-1$, so $p(m) = (2g-2)m+1-g$. Since the points of $\Gamma$ impose
independent conditions on forms of every degree we have 
$$
...
Thus
$$
0\to H^0(\sO_{C}(\Gamma)  \to V\otimes H^0(\sO_C(H)) \to H^0( \sO_{C}(2H-\Gamma))) 
$$
is right exact, as required for the case $m=2$.

For $m\geq 3$ the divisor $(m-1)H+\Gamma$ has degree $>2g-2$, so
$H^1(\sO_{C}((m-1)H+\Gamma)) = 0$ as required.
\end{proof}
