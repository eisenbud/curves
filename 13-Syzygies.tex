...
\fix{consider changing ``canonical'' to ``dualizing''}
...
$$
 ...
 $$
\omega_Z =  \sExt^2_X(\sO_Z, \omega_{\PP^3}) =\sExt^2(\sO_Z, \sO_{\PP^3}(-4)) = \sO_Z(s+b-4).
 $$
...
$$
\omega_X = \Hom_Z(\sO_X, \omega_Z) = \Hom_Z(\sO_X, \sO_Z)(s+t-4) = (\sI_Z:\sI_X)(s+t-4),
$$
...
by duality we have 
$$
h^0(\omega_X(m)) = \dim_\CC Hom_X(\sO_X(-m), \omega_X) = h^{1}(\sO_X(-m)).
$$ 
But for $m>0$
$h^0(\sO_X(-m) = 0$ since $X$ is purely 1-dimensional, and thus
the value of the Hilbert polynomial of $X$ at $-m$ is
$$
\chi(\sO_X(-m)) = (\deg X)(-m)+1-p_a(X) = h^1(\sO_X(-m)).
$$
...
\fix{need to add the proof of the second half of the theorem -- the genus formula}
\end{proof}
...
\fix{state H-R theorem as a fact}.
