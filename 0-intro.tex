...
\index{Gilbert, W. S.}%
\index{Sullivan @Sullivan, Arthur}%
\index{Pirates@{\it Pirates of Penzance}}
\end{quote}
...
Osserman
\citeyear{Poetry}
\index{Osserman @Osserman, Robert}%
\end{quote}

In Chapters~\ref{linear series} and~\ref{RiemannRochChapter} we lay out the central objects of algebraic curve theory: invertible sheaves, linear seriess, canonical sheaves and the Riemann--Roch theorem, as well as
Hurwitz's theorem,
...
Thereafter, we alternate between chapters focused on special cases and
...
\index{Grothendieck, Alexander@Grothendieck!convention (for projective space)}%
\index{projective!space!definition}%
\index{Grassmannian}%
\index{P@$\PP(V)$, $\PP^r$|defi}%
\index{Sym@$\Sym(V)$|defi}%
...
\index{G@$G(k,V)$, $G(1,V) = \PP(V^*)$, $\GG(k,r)$|defi}%
...
\index{H@$H^i_*(\sF)$|defi}%
$\bigoplus_{m\in \ZZ} H^i(\sF(m))$. 
