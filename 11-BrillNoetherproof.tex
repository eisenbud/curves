...
Thus given one invertible sheaf $\sF_1$ on $C$ there are pairwise non-isomorphic invertible sheaves $\sF_\lambda$ on $C$ for $\lambda\in \CC^*$ that become
isomorphic when restricted to the complement of $p$. 

\def\wL{{\widetilde\sL}}

Similarly, suppose that $p\in C$ is a cusp and $\pi: \widetilde C \to C$ is the partial normalization of $C$ at $p$. Suppose that
...
\fix{should we introduce the word conductor in Ch 2 where this is done?}
...
%\fix{we essentially proved this, without using the word conductor, in \subsection{Arithmetic genus and geometric genus} of Ch 2}
%
...
To summarize the situation so far: if we assume that the general curve $C_t$ of our family has a $g^r_d$, we may conclude that the $g$-nodal curve $C_0$ has a torsion-free sheaf $\cL_0$ of degree $d$ with at least $r+1$ sections. \fix{this requires an argument we haven't given, which will use the properness of the relative Picard scheme.} Moreover, at each node $r_i$ of $C_0$, $\cL_0$ is either locally free or isomorphic to the maximal ideal. Thus, if we denote by $p_1,\dots, p_k$ the cusps of $C_0$ at which $\cL_0$ is isomorphic to the maximal ideal, and let $\widetilde C_0$ be the normalization of $C_0$ at those cusps, then $\widetilde C_0$ will be a curve of arithmetic genus $g-k$ having $g-k$ cusps, and
...
\begin{proof}
To start, let $\cC \to B$ be a family of curves as in the proof of Lemma~\ref{cusp smoothing lemma} \fix{this doesn't exist}. Let $\sigma_1, \dots, \sigma_n : B \to \cC$ be sections of $\cC \to B$ with $\sigma_k(0)$ a smooth point of $C_0$ for all $k$ (such sections can always be found after passing to an \'etale open neighborhood of $0 \in B$ \fix{but we generally
stick with the analytic category}. Exactly as in the proof of Lemma~\ref{BN in family}, if the general curve $C_b$ in our family admits a $g^r_d$ $\cD$ with
$$
\rho(\cD;\sigma_1(b),\dots,\sigma_n(b)) < 0
$$
we can choose a family $\{\cD_b\}$ of such linear series \fix{needs justification} on the fibers $C_b$ for $b \neq 0$ and, taking limits, we arrive at a $g^r_d$ $\cD_0$ on $\PP^1$ with
$$
w(\cD_0, q_i) \geq r
$$
...
Note that applying this in case $d=2g-2$ and $r = g-1$, we arrive at the statement made earlier \fix{where?}: that a general curve $C$ of genus $g$ has only normal Weierstrass points!
