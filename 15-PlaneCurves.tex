...
\redden{In that form},
the technique 
\marginpar{and reordered}
\redden{has limited applicability}. 
...
\redden{We open this chapter by showing}
...
\marginpar{dropped ``More'' (there is ``More precisely'' just below)}
...
\index{differential!on nodal plane curve}%
...
\index{regular differential form}%
\index{differential form!regular}%
...
\redden{so that at each node of $C_0$ (necessarily contained in the affine plane $U = \PP^2 \setminus L$) the tangents to $C_0$ have finite slope.}
...
\marginpar{dropped repeated ``that''}
...
\marginpar{$dx \to (dx)$ for consistency}
...
\marginpar{removed parentheses and hyphen before ``times'' (or is this some special meaning?)}
one branch and
...
\marginpar{I didn't see an explicit comment on this reason; I hope the rephrasing is more or less what you had in mind}
...
\marginpar{replaced ``1989, 2008'' by ``1969'' since it's basically the same work}
...
\marginpar{\vskip-40pt \hskip- 60pt removed hyphen in ``$\delta$-invariant'' since that was the predominant usage after chapter 2 (and made the change retroactively); sorry I didn't spot the inconsistency sooner}
...
\marginpar{dropped ``where the sum runs''}
...
\redden{It follows that one can}
embed $C$ via the linear series
$|D|$:
...
\redden{which was known classically as the \emph{completeness of the adjoint series}.}
\index{completeness of the adjoint series}%
\marginpar{moved this bit of info here to avoid awkwardness; alternatively, you could place ``completeness of the adjoint series'' as the theorem's title and drop ``known classically''}
\end{proof}
...
\marginpar{\vskip10pt displayed def}
...
\marginpar{dropped parentheses around ``at least''}
at least 2.
...
\marginpar{should there be parentheses around $a\Hom...$?}
a nonzerodivisor
...
\blue{conductor ideal}
 $\ff_{C/C_{0}}$. \qed
\end{corollary}

\begin{proof}[Proof of Theorem~\ref{general differentials}]
\redden{We proceed in four steps.}
\marginpar{this said ``of four parts'' but only two were clearly marked.}%
...
way is called the 
\blue{\emph{Dedekind complementary module}.)}
...
\redden{For the last step we need a}
...
analytically
 ...
 $r$ of $C$ lying over 
\marginpar{dropped ``the cusp point''}
...
%\quad\hbox{ and }\quad \varphi_0 = \frac{-dt}{ 2t}
%\marginparhere{repeated $\varphi_0$}
 $$
\redden{and each of these differentials has a simple pole there.}
\index{adjoint ideal}%
...
\begin{example}[spatial triple points]
...
\index{nodal plane curve!gonality}
...
\begin{exercise}
Let $C_0$ be a 
\blue{plane quartic curve} 
\index{plane quartic curve}%
\index{quartic curve!plane}%
...
\begin{enumerate}
\item a 
\blue{\emph{triple tacnode},}
\redden{also known in classical language as}
\marginpar{removed from parentheses in view of the next entry where the same language is used}
a \emph{triple point with an}
\blue{\emph{infinitely near double point}:}
\index{infinitely near double point}
...
\marginpar{dropped ``to both'' (redundant, right?)}
\item a 
\blue{unibranch triple point,} 
such as the zero locus of $y^3-x^4$.
\index{unibranch triple point} 
\index{triple point!unibranch}
\end{enumerate}
