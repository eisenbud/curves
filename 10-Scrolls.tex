...
From the definition above it is easy to find the equations of a rational normal scroll. To warm up, we return to the case of the rational normal curve, $S(a)\subset \PP^{a}$, first treated in Section~\ref{rational normal curves section}. For $a=1$ there is of course no problem, so we assume $a>1$. Consider the expression of the hyperplane bundle $\cO_{\PP^{1}}(a)$ as the product
$$
...
\begin{exercise} \fix{sketch proof by hyperplane section. Need to know that the hyperplane section
is nondegenerate at each stage.}
...
The following result thus completes the proof  of Theorem~\ref{where are the curves?}.
\end{proof}
