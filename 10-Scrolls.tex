...
$|\cO_{\PP^1}(a)|$, is called a \emph{rational normal curve}; it is, up to linear transformations, the unique nondegenerate curve of degree $a$ in $\PP^a$ (\ref{rational normal curves section}).

``Scrolls'' is one answer to the question: ``What are the higher-dimensional analogues of rational normal curves?'' To construct a scroll of dimension 2, we choose integers $0\leq a_1, a_2$ (meaning that both $a_{1}$ and $a_{2}$ are non-negative) and consider  a projective space 
$$
\PP^{a_1+a_2+1} = \PP_B(\CC^{a_1+1}\oplus \CC^{a_2+1})
$$
with its two subspaces $\PP^{a_i} = \PP_B(\CC^{a_i+1})$. When $0< a_{1}, a_{2}$ we choose rational normal curves $C_i\subset \PP^{a_i}$, and we choose an isomorphism $\phi: C_1\to C_2$. We define the scroll $S(a_1, a_2)$ to be the union of the lines
$$
S(a_1,a_2) := \bigcup_{p\in C_1} \overline{p, \phi(p)}.
$$
We call the curves $C_{a_{1}}$ and $C_{a_{2}}$ the \emph{directrices} (singular: directrix) of the scroll, and we call the lines $\overline{p, \phi(p)}$ the \emph{rulings} of the scroll. To deal with the degenerate case $a_{i}= 0$ we think of $C_{i}$ as the image of $\PP^{1}$ under the map corresponding to the linear series $|\sO_{\PP^{1}}(a_{i})$, so that $C_{0}= \PP^{0}$ is a point. Thus if $a_{2}>0$ then we define $S(0,a_{2})$ to be the cone
in $\PP^{a_{2+1}}$ over a rational normal curve of degree $a_{2}$. We will never have occasion to use the case $a_{1}=a_{2} = 0$, though we might consider that to be a single line.
...
Note the behavior of some boundary cases: We may think of the ``rational normal curve of degree 0 as a point, so we take $S(0, a_2)$ to be the cone over the rational normal curve of degree $a_2$. A still less typical case occurs with $S(0,1) = \PP^2$, the ``cone over a line in the plane''. Since these cases are atypical, we will usually suppose $1\leq a_1\leq a_2$. 
...
If $0< a_{1}, a_{2}$ we also see from this argument that, given any point  $p\in S(a_{1},s_{2})$, there is
a hyperplane section that is non-singular at $p$, and thus $S(a_{1},s_{2})$ is nonsingular at $p$.
\end{proof}
...
A proof is given in an appendix to this book. \fix{do we want to keep this??}
...
Suppose $1\leq a_1\leq a_2$. Show that the projection of $S(a_1,a_2)$ from any point on $C_{a_1}$ is 
$S(a_1-1, a_2)$, and that the projection from any point on $C_{a_2}$ is $S(a_1, a_2-1)$.

Conclude, in particular, that the blowup of the cone $S(0,a_2)$ over the rational normal curve of degree $a_2$ is $S(1,a_2)$. Note 
\end{exercise}
...
$\sO_X(1)$ can be ``factored'' as a tensor product $\sL\otimes \sM$ of invertible sheaves.
sheaf. If we pick sets of $p$ independent elements $\{\ell_i\}\subset H^0(\sL)$ and  $q$ independent elements $\{m_i\} \subset H^0(\sM)$ then the multiplication map 
$$
...
where we have written $s,t$ for the basis of $\sO_{\PP^1}(1)$, and bordered the matrix
with the corresponding bases of $\sO_{\PP^1}(1)$ and $\sO_{\PP^1}(a-1)$, and it is obvious
...
$A\otimes B \to C \to C/<x>$, or equivalently to the addition of 1 element to $\ker \mu$, and thus
...
\item{The degree 2 part of the ideal of $2\times 2$ minors of $M$ has dimension ${a\choose 2}$; that is, the minors of $M$ are linearly independent.
\end{enumerate}
\end{theorem}

\begin{remark}
In general the $p\times p$ minors of a 1-generic $p\times q$ matrix generate ap
\end{remark}
\begin{proof} 
...
the result for the matrix $M_a$. If we regrade the polynomial ring, giviing the variable $x_i$ degree $i^2$
then the anti-diagonal term of each $2\times 2$ minor has strictly lower degree then the diagonal term.
Thus if there were a linear relation among the $2\times 2$ minors, then there would be a linear relation among the anti-diagonal terms. However these are precisely the ${a\choose 2$ 
\end{proof}
...
\begin{exercise}\label{many quadrics}
 If $X$ is a nondegenerate projective variety of codimension $c$ whose homogeneous ideal
...
We can easily compute the degrees and genera of curves that lie on smooth scrolls:
...
Moreover, the singular scroll $S(0,a_{2})$, with $a_2>0$ which is the cone over a rational normal curve of degree $a_{2}$, is the image of
