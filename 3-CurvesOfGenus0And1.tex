...
The \emph{grade} of an ideal $I$ in a ring  $R$ (sometimes confusingly called the depth of $I$ on $R$) is the maximal length of a regular sequence contained in $I$, or $\infty$ if $I = R$. 
...
\begin{itemize}
 ...
 In fact every regular local ring is Cohen-Macaulay.
 
 \item It follows from the definition that any complete intersection in a Cohen-Macaulay scheme is again Cohen-Macaulay.

\item Zero-dimensional rings are all Cohen-Macaulay. For a 1-dimensional ring, the Cohen-Macaulay property is the same has being pure-dimensional---that is, 
...
then $\grade I =\codim I$ for every ideal $I$ of $R$. In this sense the grade is a sort of arithmetic
...
of $X$ are essentially localizations of $R_{X}$.

\item Regular rings are Cohen-Macaulay. Thus every smooth scheme is Cohen-Macaulay, and in particular
...
The sequence of elements $x_{0},\dots, x_{r}\subset S:= \CC[x_{0}\dots, x_{r}]$
\end{itemize}
 \end{fact}
