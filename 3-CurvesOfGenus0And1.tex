...
then $x$ is algebraic over $\CC(z)$. Thus $\CC(x)$ is finite over $\CC(z)$, so $K$ is finite over $\CC(y)$
...
%\fix{Joe: I wrote a different proof for part 2; the original is commented out but still there if you want to go back.
%David: The algebraic
%closure of $\CC(x) does NOT correspond to a curve, so I felt we should prove that $K$ is finitely generated before claiming that it corresponds to a curve. }
