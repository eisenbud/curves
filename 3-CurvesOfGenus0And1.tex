...
a purely one-dimensional scheme, we say that $C$ is arithmetically Cohen-Macaulay
(usually abbreviated ACM)
 when the natural maps
$$
\alpha_m: H^0(\sO_{\PP^r}(m)) \to H^0(\sO_{C}(m)
$$
are surjective for every $m\in \ZZ$. This is related to the usual  Cohen-Macaulay property as follows:
...
\item There are linear forms $h,h'$ on $\PP^r$ such that $h, h'$ is a regular sequence
on $S_C$; in particular, $C$ has no 0-dimensional primary components.
\item $H^1_*(\sI_{C/\PP^r}) = 0.$
 \item For every hyperplane $H\subset \PP^r$ that meets $C$ in a zero-dimensional scheme,
...
and let $R =H^0_*(\sO_{C}(1)$.
If $H$ is a 
general hyperplane, with equation $h=0$, then $h$ does not vanish on any primary component of $C$, and thus the sequence
$$
0\rTo \sO_C(-1) \rTo^{h}\rTo \sO_C\rTo \sO_{C\cap H}\rTo 0
$$
is exact. Applying $H^0_*$, we see that $h$ is a nonzerodivisor on $R$, and that $R/hR$ is
a subring of $H^0_*(\sO_{C\cap H}$.  A general linear form $h'$ doesn't vanish on
any point of component of $C\cap H$, so $h'$ is a unit on $H^0_*(\sO_{C\cap H})$
...
and is in particular unmixed; that is, $C$ has no 0-dimensional primary components. This proves the equivalence of conditions {\sl 1} and {\sl 2.}

{\sl 1} $\Leftrightarrow {\sl 3}$: From the exact sequence 
$$
...
{\sl 2} $\Leftrightarrow$ {\sl 4}: The condition implies that  $h$ does not vanish on any component of $C$, and
this implies that $h$ is a nonzerodivisor on $S_C$. The ideal $I_{C\cap H}$ is in any case the saturation of $I_C+(h)$. 
Thus if $I_{C\cap H}=I_C+(h)$, then any linear form $h'$ not containing a point of $C\cap H$ is a nonzerodivisor
on $I_C+(h)$, so $C$ satisfies condition {\sl 1}. Conversely, in a 2-dimensional positively graded Cohen-Macaulay ring, any 
system of parameters is a regular sequence \cite[]{Eisenbud1997}, so $I_C+(h)$ is unmixed, and in particular, saturated
\end{proof}
...
$$ 
Since $C$ is nondegenerate, $h^0(\cI_{C/\PP^d}(1)) = 0$, and since $\deg C \geq d$, the hyperplane section $\Gamma$ of $C$ must contain at least $d$ linearly independent points.
\fix{A a later point of the book where this comes up, my students didn't find it obvious, so I added Proposition~\ref{arbitrary hyperplane}. Perhaps we should move that here, or at least refer to it.} Since linearly independent points impose independent conditions on quadrics, we have
$$
...
\fix{introduce $d$-normal, ACM, mention CM, normal"}
