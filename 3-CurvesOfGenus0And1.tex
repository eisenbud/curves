...
$$
and the map $\alpha_{\sV}$ is surjective. This is expressed by saying
...
(usually abbreviated ACM)
 when $\alpha_m$ is surjective for all $m$. We'll discuss the significance of this condition in Section~\ref{ACM} of this chapter, and we will prove in particular that it is equivalent to the condition that $R_{C}$ is a Cohen-Macaulay ring, justifying
 the name.
...
$$
\alpha_{m}: H^{0}(\op31) \otimes H^0(\sI_{C/Q}(m+3)) \to H^0(\sI_{C/Q}(m+1+3)
$$
...
\item For a curve $C\subset \PP^r$ to be ACM it is necessary, in particular, that $C$ be embedded by a complete linear
series.
...
We will prove that this is equivalent to $R_{C}$ being Cohen-Macaulay in Proposition~\ref{ACM basics}.
...
\begin{enumerate}
\item A scheme $X$ is Cohen-Macaulay if some (respectively every) finite map $f: X\to P$ to a smooth scheme $P$
