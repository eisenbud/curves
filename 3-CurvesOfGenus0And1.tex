...
each degree vanishing on the curve, and the condition has other important consequences that we will explore here and in Chapters~\ref{LinkageChapter} and \ref{SyzygiesChapter}. One of the characterizations of the condition is in terms of regular sequences:

\begin{definition}\label{gradeDef}
A sequence of elements $f_{1}, \dots, f_{c}$ in a ring $R$ is a \emph{regular sequence} if
$f_{1}$ is a nonzerodivisor, $f_{2}$ is a nonzerodivisor in $R/(f_{1})$\dots, and the ideal
$(f_{1}, \dots, f_{c})$ is not the unit ideal.

The \emph{grade} of an ideal $I$ in a ring  $R$ (sometimes confusingly called the depth of $I$ on $R$) is the maximal length of a regular sequence contained in $I$. 

If $(R,\gm)$ is a local ring and $\grade \gm = \dim R$ then $R$ is called Cohen-Macaulay. If $R$ is not necessarily local, then 
$R$ is called Cohen-Macaulay if every localization is Cohen-Macaulay. Similarly, an algebraic variety is Cohen-Macaulay if
all its local rings are Cohen-Macaulay.
\end{definition}

\begin{example}
For a 1-dimensional ring, the Cohen-Macaulay property is the same has being pure-dimensional---that is, 
every associated prime ideal being 1-dimensional (Proof: the zerodivisors in $R$ are the elements of the associated primes of 0.) Thus any purely 1-dimensional scheme is Cohen-Macaulay. 

 It follows from the definition that any complete intersection in a Cohen-Macaulay scheme is again Cohen-Macaulay.
\end{example}

\begin{fact}
If $(R,\gm)$ is a local ring then $\grade \gm \leq \codim \gm$. On the other hand, if $R$ is Cohen-Macaulay,
then $\grade I =\codim I$ for every ideal $I$ of $R$.
The same definitions and theorem apply to a positively graded ring, and homogeneous ideals.
 \end{fact}
...
\item The homogeneous coordinate ring $S_C$ of $C$ is a Cohen-Macaulay ring; that is, there are linear forms $h,h'$ on $\PP^r$ whose images in  $S_C$ form a regular sequence. In particular, $C$ has no 0-dimensional primary components.
...
{\sl 3} $\Leftrightarrow$ {\sl 4}: If  $h$ does not vanish on any component of $C$ then $h$ is a non-zerodivisor on $S_C$. The ideal $I_{C\cap H}$ is in any case the saturation of $I_C+(h)$. 
