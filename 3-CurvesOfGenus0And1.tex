...
The second part is called \emph{Luroth's theorem}.
...
\noindent 2. From the hypotheses, we see that $K$ has transcendence degree $1$ over $\CC$; that is, it is the function field of a curve $C$. The inclusion $K \subset \CC(x)$ then corresponds to a map $\PP^1 \to C$; from the first part, it follows that $C \cong \PP^1$ and hence $K = \CC(y)$ for some $y \in \CC(x)$.
\end{proof}

\fix{I wrote a different proof for part 2; the original is commented out but still there if you want to go back}
...
\fix{I couldn't figure out what was intended in the above}
