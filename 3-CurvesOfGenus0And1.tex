...
In Section~\ref{rational normal curves section} we said that a curve $C\subset \PP^{r}$ is Arithmetically Cohen-Macaulay (ACM) if the maps
$$
\alpha_m: H^0(\sO_{\PP^r}(m)) \to H^0(\sO_{C}(m))
$$
are surjective for all $m$. When this is the case we can accurately predict the number of independent forms of
each degree vanishing on the curve. In this section we will analyze the ACM condition further. One of the characterizations
is in terms of regular sequences:
...
The \emph{grade} of an ideal $I$ in a local ring $(R,\gm)$ (sometimes confusingly called the depth of $I$ on $R$) is the maximal length of a regular sequence contained in $I$. 

If $\grade \gm = \dim R$ then $R$ is called Cohen-Macaulay. If $R$ is not necessarily local, then 
$R$ is called Cohen-Macaulay if every localization is Cohen-Macaulay.
\end{definition}

For a 1-dimensional ring, the Cohen-Macaulay property is the same has being pure-dimensional---that is, 
every associated prime ideal being 1-dimensional. Similarly, an algebraic variety is Cohen-Macaulay if
...
The same definitions and theorem apply to any positively graded ring, and homogeneous ideals.
 \end{fact}
