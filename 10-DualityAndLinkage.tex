...
Consider the ideal
$$
I= (x_{0}, x_{1})^{2}+ (x_{0}F_{0}(x_{2},x_{3})+x_{1}F_{1}(x_{2},x_{3})).
$$
where $F_{0}, F_{1}$ are forms
of degree $d>1$ with no common factor. It is not hard to show that 
$I$ is unmixed of codimension 2, and is thus the ideal of a purely 1-dimensional subscheme $C$ of
$\PP^{3}$. Since the radical of $I$ is $(x_{0}, x_{1})$, the curve $C$ is supported on the line
$x_{0}=x_{1} = 0$. One can check that  the degree of $C$ is 2, so $C$ is minimal in its linkage class.
One can also show that the arithmetic genus is $-d$, and
$C$ has Rao module $S/(x_{0}, x_{1}, F_{0}, F_{1})$. \fix{proof?} Since a reduced curve of degree 2
can have arithmetic genus only 0 or 1, all the minimal curves in the linkage class of $C$ are
double lines and the ideal generated by $F_{0}, F_{1}$ determines and is determined by the linkage class.
\end{example}
