...
Let $A$ be a homogeneous $(n+1)\times n$ matrix of forms in $S := k[x_{0},\dots, x_{n}]$ and
Let  $I: = I_{n}(A)$ be the ideal generated by the $n\times n$ minors of $A$.
\begin{enumerate}
...
$$
0\rTo S^{n} \rTo^{A} S^{n+1}\rTo^{\scriptsize
\begin{pmatrix}
 \Delta_{0}& -\Delta_{1}&\dots&\pm \Delta_{n}
\end{pmatrix}}
S
$$
...
\begin{proof}
If we augment $A$ to an $(n+1)\times(n+1)$ matrix by repeating the $i$-th column, the determinant is zero.
...
\cite[Theorem *****]{E}. This shows that $S/I = S/(\Delta_{0}, \dots \Delta_{n})$ is Cohen-Macaulay.
...
$$
0\rTo S^{m} \rTo^{A} S^{n} \rTo^{B} S
$$
where $n$ is the minimal number of generators of $I$. Tensoring with the quotient field of $S$, we
get a complex of vector spaces that is exact, so $n = m+1$, and we see that $A$ is a homogeneous
$(n+1)\times n$ matrix. Again by \cite[Theorem *****]{E}, the ideal of $n\times n$ minors of $A$ has
codimension 2, and the dual of the resolution is a resolution of $\omega_{I}$. Write $I'$ for the ideal
of minors of $A$; by part 2, the ring $S/I'$ is also Cohen-Macaulay of codimension 2.
\end{proof}
