...
the difference $b_i- b_{g-i-1}$ are independent of the curve.
...
of the hyperplane section as the sum of two Cartier divisors
 $H = D+E$ where both $r(D) = 1$ and $r(E) = c\geq 1$ gives a 1-generic $h^0(\sO_X(D)) \times h^0(\sO_X(E))$
...
normal scroll, then the Eagon-Northcott comples that is  the minimal free resolution of $I_X$ is termwise a direct summand
...
\includepdf[pages=1]{"SyzygiesGupto8"}
...
of genus 3. 
%\fix{What's the best way to find a nonCM example? Does this need computation?}
\end{exercise}
