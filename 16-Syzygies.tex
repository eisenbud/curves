...
theory for a regular local ring is exactly parallel.
...
is Cohen-Macaulay or, equivalently, the depth of the homogeneous coordinate ring  coordinate ring is 2.
\end{proposition}
...
Moreover, for any purely 1-dimensional projective scheme $C$ the $S$-module $H^0_*(\sO_C)$ has depth 2; this follows because
...
the  minimal free resolution of $S/I$ is isomorphic to its own dual. We have seen an example already
...
\end{fact}
 ...
 As noted about, the homogeneous coordinate ring $S/I_C$ of an arithmetically Cohen-Macaulay curve $C\subset \PP^r$ is isomorphic
 ...
 so $\omega_C(-1) = \sO_C$ so $\omega_{S/I}$ is generated in degree $-1$, so the  the last
 free module in the minimal free resolution $\FF$ of $S/I$ is $S(-r)$. Since the resolution is minimal,
 the minimal  twist in $F_{i+1}$ must be $\leq$ the minimal twist in $F_i$; since the dual is also a minimal resolution,
 it follows that both the minimal and maximal twists of $F{i+1}$ are $\leq$ the minimal and maximal twists of $F_i$, respectively.
...
It is also true for plane curves, and in a number of other special cases. See~\cite{MR4022070} for the currently simplest
proof that the conjecture holds for generic curves of each Clifford index.
