...
%\subsection{Decompositions of the hyperplane divisor}
%When investigating the canonical models of a curve $C$ of genus 5 in $\PP^4$ in
%Section~\ref{non-trigonal genus 5} we remarked that when $C$
% is trigonal, so that there is a degree 3 divisor $D\subset C$ such that $|D|$ is 1-dimensional,
% the geometric Riemann-Roch theorem implies that in $\PP^4$ the divisors $D_\lambda\in |D|$
% consist of 3 colinear points, and the line they span is contained in all the quadrics containing the curve.
% In Section~\ref{trigonal genus 5} we showed that the three quadrics actually intersect in a surface.
% This surface obviously contains the union
%$$
%\bigcup_\lambda \overline D_\lambda
%$$ 
%and we will soon see that they are equal. We remarked on a similar construction
%in the case of trigonal curves of genus 6 in Chapter~\ref{Brill-NoetherChapter}. 
%These are special cases of a general phenomenon.
%
%In genus 5 we also wrote down the $5\times 5$ matrix
...
%\end{example}
%
%In case $X$ is reduced and irreducible the matrix above has a special property: $\kappa $ is a domain, so no product of a nonzero
...
\section{The Eagon-Northcott Complex}
...
We have seen in Chapter~\ref{ScrollsChapter} that an ideal of $2\times 2$ minors of
a 1-generic  $2\times n$, matrix of linear forms often occurs inside the ideal of an interesting curve, and defines a rational
normal scroll of codimension $n-1$ containing the curve. This has a strong implication for the minimal free resolution of the ideal of the curve, which, as we shall see, must contain the minimal free resolution of the ideal of the scroll as a termwise direct summand.

In this section we will describe the resolution of such an ideal of minors, a special case of the Eagon-Northcott complex.

\begin{theorem}\label{Eagon-Northcott}
 Let $S = k[x_0,\dots, x_r]$ be a polynomial ring,  and let $M: F\to G$ be a homomorphism with
 $F = S^n(-1), G= S^2$. If $I_2(M)$ has codimension $n-1$, then the minimal free resolution of $S/I_2(M)$ has the form:
\begin{align*}
EN(M) := 
S \lTo{\bigwedge^2 M} 
 \bigwedge^2 F&
...
\begin{lemma}\label{1-generic lemma}
