...
\marginpar{dropped ``Introduction''}
...
\marginparhere{replaced commas by semicolons}
\vspace*{-10pt}
}
\label{Fig16.1}
\end{figure}
...
{\it In this chapter 
\marginpar{italicized and enforced the shorthand (generally marked below)}
...
\marginpar{dropped ``rational normal''}
scrolls of
...
\marginpar{changed CF to theorem with \string\qed, per your treatment of such short results elsewhere; moved the proof citation}
...
\marginpar{\redden{It would be good to clarify whether this is just for $S(a_1,a_2)$ or arbitrary many $a_i$'s.}}
\redden{on page~\pageref{thescroll}, we're entitled to talk about}
\redden{{\it the\/} scroll $S(a_1,a_2)$}:

\begin{proposition}\label{uniqueness of scrolls}
Up to a linear automorphism of $\PP^{a_1+a_2+1}$,
\marginpar{reversed to allow line break}%
...
\marginpar{supplied missing right parens at end of paragraph; changed ``This matrix'' to $M$ to avoid breaking LHS of map)}
multiplication
...
\marginpar{dropped ``But note that'', made into separate sentence}
...
\redden{Theorem~\ref{Castelnuovo examples} below, which classifies}
...
$p_i,p_j$, thus at 
\redden{$r-3$ additional} 
\marginpar{supplied \$, fixed typo (a few other typos unmarked)}
...
\marginpar{\redden{missing xref ``canonical ACM''}}
...
\marginpar{$\Sym(\sE) \to \Sym \sE$ (matching some earlier usage)}%
...
\marginpar{removed unmatched right parentheses}
\cite[Chapter V]{Hartshorne1977}, but restrict
to the 2-dimensional rational case.
\marginpar{\vskip10pt moved citation to head of theorem and dropped ``The following\dots''}
...
\redden{$\phi(C_2)$}
\marginpar{changed second $C_1$ to $C_2$; please check}
...
if and only~if
\marginpar{dropped redundant $0<$ condition, in part for layout.}
$e = a_1$ or $e\geq a_2$.
\unif
\end{proposition}
...
\marginpar{dropped extra right parens}
\end{proof}
...
\marginpar{I couldn't tell what the remark ``In the case of a trigonal curve the difference $|a_2-a_1|$ is called the \emph{Maroni invariant}'' was a propos of, in this place.  I moved it to the related exercise at the end of the chapter.}
\end{corollary}
...
It follows 
\marginpar{dropped ``in particular''}
...
\marginpar{\redden{missing xref ``minimal degree''}. Do you mean Theorem~\ref{where are the curves?}, the only result that seems to use Bertini's theorem?}
...
In
\redden{our study of canonical curves of genus 5 (second case,
...
\redden{and}
 \marginpar{then $\to$ and (is this what you mean? You had If A then B then C}
