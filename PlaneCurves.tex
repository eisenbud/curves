...
We focus for now on one singular point $q \in C_0$, with points $r_1,\dots,r_k \in C$ lying over $q$, and we wish to describe the conditions on a rational differential $e\omega_0$ on $C$, where
$$
...
a polynomial in $x,y$ of order $\leq d-3$. \fix{ add justifciation?} In addition, $\omega_0$ will have poles at the points points $r_i$,
...
\begin{fact}\label{adjoint equals conductor}
