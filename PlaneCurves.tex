...
We start toward our first goal by writing down a single rational 1-form on $C$: just take a regular 1-form on $\AA^2$, such as $dx$, and restrict/pull back to $C$. 
...
\fix{$E$ is a bad choice, since we're going to have an exceptional divisor $E$}
...
Next, we find polynomials $H$ of the same degree $m$, vanishing at $A+D_\infty$ and at the points $q_i$ \fix{the vanishing at the $q_i$ is not intuitive. How to make it so?} but not on all of $C_0$. Let $D'$ be the divisor cut on $C$ by $H$ residual to $D_0 + \Delta + A$; that is, we write
$$
(\nu^*H) = D_0 + \Delta + A + D'.
$$
...
If $C_0 \subset \PP^2$ is a nodal plane curve and $\nu : C \to C_0$ its normalization $C$ by plane curves of degree $m$ passing through the nodes is complete.
\end{proposition}
...
LWe will use two instances of the Leray spectral sequence 
...
$$
H^0(\cI_{\{q_1,\dots,q_\delta\}/\PP^2}(m)) \cong H^0(\cO_S(-E)).
$$
...
\fix{I moved the arithmetic genus discussion to Ch 1, where it was already done a little.}


\subsection{Linear series on the normalization of an arbitrary plane curve} \label{arbitrary plane curves}

Suppose again that $C_0 \subset \PP^2$ is reduced and irreducible, but with arbitrary singularities, and again let $\nu : C \to C_0$ be its normalization. Again we want to find the regular differentials on $C$ and the effective divisors on $C$ linearly equivalent to a given $D \in \Div(C)$. Again we take $\widetilde C_0$ to be the interection of $C_0$ with the open set $z \neq 0$, that is, with $\AA^2\subset \PP^2$, and assume that
...
separable extension of $\CC(x)$. By Theorem~\cite[****]{Eisenbud1995} that the module of differentials $\Omega_{\kappa(C)/\CC}$ is generated over $\kappa(\widetilde C_0)$ by $dx$; that is, every differential can be expressed as a rational function
times $dx$.

We focus for now on one singular point $q \in C_0$, with points $r_1,\dots,r_k \in C$ lying over $q$, and we wish to describe the conditions on a rational function $e\omega_0$ on $C$, where
$$
...
form $\omega_0$ has a zero of order $d-3$ at each of the points of $C$ lying over the intersection of $C_0$ with the line  $L = \PP^2\setminus \AA^2$
...
a polynomial in $x,y$ of order $\leq d-3$. \fix{maybe add justifciation?} In addition, $\omega_0$ will have poles at the points points $r_i$,
so $e$ must satisfy some conditions of adjunction, to be determined, at each singular point.

Let $m_i$ be the order of the pole of $\omega_0$ at $r_i$. We can define the \emph{adjoint ideal} of $C_0$ at $q$ to be the ideal
$$
A_q = \left\{ e \in \cO_{\PP^2, q} \mid \ord_{r_i}(\nu^*e) \geq m_i \; \forall i \right\}
$$

In other words, $A$ is the ideal of functions $e$ such that $\nu^* \frac{gdx}{\partial f/\partial y}$ is regular at all the points $r_i$. We accordingly define the \emph{adjoint ideal $\cI_A$ of $C_0$} to be the intersection of $A_q$ over all singular points  $q \in C_0$; the \emph{adjoint series of degree $m$} is then the linear series $H^0(\cI_A(m))$. 

We now see  that every global regular 1-form on the curve $C$ is of the form 
$$
e(x,y) \omega_0,
$$
with $e$ a polynomial of degree $\leq d-3$ in the adjoint ideal $A$.
...
At this point it may seem that without an explicit description of the adjoint ideal we have merely labeled our ignorance. But in simple cases, the adjoint ideal is not difficult to compute. 
...
We have already seen that in case $q$ is a node of $C_0$, there are two points of $C$ lying over it, and the multiplicities of $\omega_0$ at these two points are $m_1=m_2=1$; the adjoint ideal is thus just the maximal ideal $\cI_q$ at $q$. In the case of a cusp, for example the zero locus of $y^2-x^3$, there is only one point $r=r_1$ of $C$ lying over $q$, and the differential $\omega_0$ vanishes to order $m_1=2$; since the pullback to $C$ of any polynomial $g$ vanishing at $q$ will vanish to order at least two at $r$, the adjoint ideal is again the maximal ideal at $q$.
\end{example}
...
In the case of an ordinary triple point---three smooth branches pairwiset ransverse to one another---there are three points of $C$ lying over $q$, and we have $m_1=m_2=m_3= 2$; the adjoint ideal is correspondingly just the square of the maximal ideal at $q$
\end{exercise}
...
In general, one can compute the adjoint ideal for plane curves as follows. 
\begin{theorem}\label{adjoint equals conductor}
If $C_0$ is a reduced plane curve, or more generally a Gorenstein local ring, then the adjoint ideal of $C_0$ is equal to the \emph{conductor ideal}, defined as the annihilator in $\sO_{C_0}$ of the sheaf $\nu_*(\sO_C)/\sO_{C_0}$. 
\end{theorem}
