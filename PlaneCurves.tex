...
In this chapter we'll focus  on one basic problem:
...
Along the way we will see how to test when two given divisors $D$ and $E$ on $C$ arelinearly equivalent; and whether a divisor $D$ linearly equivalent to an effective divisor.

We will pursue these goals in three stages of increasing generality. We'll first achieve these goals in case $C = C_0$ is a smooth plane curve. This is itself of limited interest because most curves cannot be realized as smooth plane curves. In Section~\ref{nodal plane curves} we'll do it for (the normalization of) a plane curve with nodes.  Theorem~\ref{****} says that every smooth projective curve is the normalization of a nodal plane curve, so in principle this covers all smooth curves.

Finally, in Section~\ref{arbitrary plane curves} we'll describe how to achieve our goals for the normalization of an arbitrary plane curve.
...
By choosing coordinates appropriately, we may assume:
\begin{enumerate}
...
Since an automorphism of $\PP^2$ can carry any line in the plane to the line at infinity, and any point on that line to the point $(0,1,0)$, these conditions are not restrictive. Nor are they necessary: in Exercise~\ref{****} we'll see how to do everything in the absence of these assumptions, albeit with slightly more complicated notation.
...
$$
 shows that t will have double poles at the points $p_1,\dots,p_d$ of the divisor $D := C \cap L$.
...
$\AA^2$ has a pole of order $d$ along the line $L$ at infinity. Thus if $h$ has degree at least 2 then $dx/h$ will have no poles at infinity. However, $h(x,y)$ will vanish at points of $C \cap U$, and this may create new poles of $dx/h$. Of course if $h$ vanishes only at  points of $C \cap U$ where $dx$ already has a zero, the zeroes of $h$ may cancel the zeroes of $dx$ rather than creating new poles.
 ...
 $$
 To see that this works, note that on $\tilde C$,
 $$
 df = \frac{\partial f}{\partial x}dx + \frac{\partial f}{\partial y}dy \equiv 0.
 $$
Becaused $C$ is assumed smooth, the functions $\frac{\partial f}{\partial x}$ and $\frac{\partial f}{\partial y}$ have no common zeroes on $\tilde C$. Moreover, the differentials $dx$ and $dy$ have no common zeros on $\tilde C$. It follows that at for all $p \in \tilde C$ we  have
$$
\ord_p(dx) = \ord_p(\frac{\partial f}{\partial y}), and
$$ 
thus the quotient 
$$
...
Since the differential $dx$ has poles of order $2$ at the points $p_i$. The polynomial $\partial f/\partial y$, having degree $d-1$, has poles of order $d-1$. Thus $\omega_0$ has zeroes of order $d-3$ at the points $p_i$; in other words, as divisors,
$$
(\omega_0) = (d-3)D.
$$
...
To find all effective divisors linearly equivalent to a given divisor $D$, we express $D$ as the difference 
$$
D = E - F
$$
of two effective divisors on $C$. Choose any homogeneous form $G(X,Y,Z)$  vanishing on the divisor $E$, but not vanishing identically on $C$, and let $A$ be the residual divisor of $G$ on $C$: that is
$$
A := (G)- E 
$$
as divisors on $C$. Now, let $H$ be another polynomial of the same degree $m$ as $G$, vanishing on $A + F$ but again not vanishing identically on $C$. (Since $m$ is already chosen, there may not exist any such polynomial, which is as it should be: the original divisor $D$ need not be linearly equivalent to any effective divisor.) Let $D'$ be the divisor cut on $C$ by $H$ residual to $A+F$; that is,
$$
D' := (H) -(A + F).
$$
Now, the divisor of anyt rational function such as $H/G$ is linearly equivalent to 0 by definition, so
$$
0 \sim (H) - (G) = D' + F - E.
$$
Thus \emph{$D'$ is an effective divisor linearly equivalent to $D$.}
...
We claim that we find in this way \emph{all} effective divisors $D' \sim D$. To see this, suppose $D'$ is any effective divisor with $D' \sim D$. Carrying out the first step of the process as before, we arrive at a divisor $A$ with 
$$
...
As before, we'll choose homogeneous coordinates  $[X,Y,Z]$ on $\PP^2$ so that the curve $C_0$ intersects the line $L = V(Z)$ at infinity transversely at points $p_1,\dots,p_d$ other than $[0,1,0]$. In particular,  all the nodes of $C_0$ will lie in the affine plane $U = \PP^2 \setminus L$).
...
\fix{revised to here by de, May 7}
...
\begin{lemma}
Let $p_1, \dots, p_g, q_1, \dots, q_g \in \PP^1$ be $2g$ general points, and let $C := \PP^1/p_i \sim q_i$ be the nodal curve of arithmetic genus $g$ obtained by identifying $p_i$ with $q_i$ for $i = 1,\dots,g$. There exists a family $\cC \to  \Delta $ of curves, parametrized by the disc $\Delta$, with $C_t$ a smooth projective curve of genus $g$ for all $t \neq 0$ and $C_0 = C$ as above.
\end{lemma}
