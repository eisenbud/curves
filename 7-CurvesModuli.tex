...
Thus, $M_1$ is not a fine moduli space for smooth curves of genus 1, but it is the next best thing: we don't get a bijection between families of curves of genus 1 over a base $B$ and maps $j : B \to M_1$; but we do get a map with ``finite kernel and cokernel"---what we might call a \emph{Deligne-Mumford moduli space}.


\section{Higher genus}

This situation described in the preceding section for curves of genus 1 persists in all higher genera as well: there does not exist a fine moduli space for smooth projective curves of genus $g$ for any $g$;\footnote{It should be noted that the existence of nontrivial $\PP^1$-bundles implies that there is no fine moduli space $M_0$ either.} but there does exist a Deligne-Mumford moduli space, and in this section we'll describe one way of constructing such a space. 

%One way of dealing with this is to enlarge the category of schemes to the category of \emph{stacks}, where a fine moduli space does exist, but this is technically demanding \fix{boo hoo}; our approach here will be to work with, and describe, the coarse moduli space $M_g$ 



The idea used above for genus 1 curves can be applied more generally: to construct a moduli space, first parametrize curves with a choice of some additional structure, such as a map to projective space, and then mod out by the choices made. For any smooth projective curve $C$ of genus $g\geq 2$, the tricanonical linear series $|3K_C|$ is very ample; it embeds $C$ as a curve of degree $6g-6$ in $\PP^{5g-6}$. Thus we have a way of realizing a given abstract curve $C$ as a curve in projective space, unique up to automorphisms of $\PP^{5g-6}$.
