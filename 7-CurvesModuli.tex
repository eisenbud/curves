...
$$
as $t \to 0$ is reducible, with two components meeting in two points \fix{huh?}. A process called semistable reduction, briefly described in a more general context below, shows that after a base change and a birational
...
\pict{a stable curve}
%(The word ``stable,"  is one of the most overworked terms in algebraic geometry; when we want to distinguish the usage above, we sometimes say, ``Deligne-Mumford stable," or simply ``DM stable." \fix{but we don't use either
%of these, in fact.})
