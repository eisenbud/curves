...
\fix{I find the presentation of the difficulties of these examples muddy. The first is saved by a reduction to a finite group problem; but the original description has both the infinite group AND the problem of orbit closures, and these are somehow swept under the rug. The second example explores the orbit closures, but doesn't consider whether one might take just curves
through a certain set of points, or something else that would kill the group; and in the previous version didn't even mention
the non-finiteness as a problem. We don't need the hyperelliptic example at all -- it's atypical. I think that a more careful treatment of cubics in the plane would be better than this sequence of
examples.}
...
points of $M_g$ correspond one-to-one to the orbits, that is, to the isomorphism classes of smooth curves. \fix{The construction was already done at the end of an earlier section as an "example". Now its back as the real thing.}
...
Let $C$ be any smooth curve of genus $g$ with an involution $\tau : C \to C$,
\fix{why an involution instead of any automorphism?} and consider the family
$$
...
\pict{two copies of $\B\times C \to B$ (surface to curve) with arrows representing $\sigma, \tau$.
