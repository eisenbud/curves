...
\marginpar{\redden{this paragraph may be misplaced here}}
...
\marginpar{dropped ``it were the case that''}
all the
...
\redden{We can now} state our first result. Recall that if
...
\marginpar{dropped repeated ``the'' and unused ``$\TT_p(C), \TT_q(C)$''}
...
\redden{Here}
$\overlw20{pq}$ 
\marginpar{dropped comma in notation for secant, ok?}
...
\blue{semicontinuity of the degree}
...
Similarly, since
$
\{((p,q), r) \in (C_2\setminus \Delta)\times \PP^r \mid r \in T_p(C)\cup
T_q(C) \}
$
...
\Bigr\}.
$$
The tangent lines $T_r(C)$ and $T_{r'}(C)$ both project from $p$ to the
tangent line $T_{\pi_p(r)}(\pi_C)$,
...
to $\PP^r$ that are 
\redden{2-to-1}
\marginpar{hyphens for consistency}
onto a rational curve, we might hope that we
would have a 
\marginpar{dropped ``different\emdash and''}
...
\blue{independent linear conditions}
...
\redden{over $m$, we obtain the lower bound}
$$
...
\blue{linearly general position!}
\index{linearly general position}%
...
\marginparhere{changed $*$ to cdot (or $\times$)}
...
\redden{of}
\marginpar{\redden{should ``of'' be ``on''?}}
...
$M$
\redden{as in \ref{defofM}}
% $= M(d,r) = \bigl\lfloor{\frac{d-1}{r-1}}\bigr\rfloor$.
We have
\begin{align*}
...
\marginpar{moved ``from\dots proof''}
that is,
$$
...
\marginpar{punctuated as full sentences}
\item $d=2r$ 
\redden{and}
...
\marginpar{This is $L$ not $\mathcal L$, right?}
...
\marginpar{changed parens to braces after min to agree with your earlier usage}
$\Gamma$
lies on a conic, and
...
\marginpar{$\leftarrow$ for uniformity with next i.e.; but I'm not sure why these two restated equations are needed}
\redden{(i.e., $k \leq n-4$)}
then
...
\marginpar{\redden{earlier you had $T$ instead of $\mathbb T$ for the tangent}}
...
case 
\marginpar{dropped ``the curve'' for better layout}
...
%Theorem~\ref{basic linear independence}  allows us to analyze 
\redden{We can now revisit}
the case
of equality in 
...
\redden{the geometric Riemann--Roch theorem}
...
\blue{strong secant plane lemma}
\index{strong secant plane lemma}%
\redden{(Lemma~\ref{Strong secant plane lemma})}
 to the 
\blue{canonical curve.}
\index{canonical curve}%
...
\redden{is}
a divisor on $C$ 
\redden{whose degree satisfies $\deg D = 2r(D)\le 2g-2$,}
\marginpar{streamlined; is this what you mean?}
...
\redden{Figure~\ref{Fig9.4} illustrates the two possibilities.}
...
If $|D|$ 
\redden{has} 
\marginpar{to make the tenses coherent}
...
\redden{contradicting generality.}

\redden{Next we show that}
\marginpar{moved the punchline of this part here}
there are only finitely many pairs $p,
q \in C$ such that $\phi_D(p) = \phi_D(q)$; that is, $\phi_D$
is birational onto its image. 
\redden{Indeed, $\phi_D(p) = \phi_D(q)$} 
...
\redden{We turn to the types of singularities. First we eliminate tacnodes.} 
...
\index{Riemann--Roch theorem!geometric}%
\marginpar{added commas around formula, ok?}
...
\marginpar{\hsize=1.2\hsize streamlined and reworked to avoid repetition; e.g., dropped ``and $D$ is a general divisor of degree $g+2$ on $C$.''}
To prove 
\redden{claim (1) of the theorem,}
we
have to show  
\marginpar{\hskip-2.3in dropped ``three things'' referring to cusps and triple points (only?)\\[2pt]dropped ``the fact''}
that the image $C_0 = \phi_D(C)$ 
has only nodes as singularities,
\redden{rather than cusps or triple points}.
...
\leq g-4$; 
\redden{but that's exactly what Marten's theorem says,} 
...
for some 
\redden{$g$ points $p_i$, which must be distinct since $D$ was assumed general.}
...
multiplicity $>g$, and is thus an ordinary $g$-fold point. Since the
$\delta$-invariant of such a point is $\tbinom{g}{2}$ by
\redden{the discussion following Theorem~\ref{divisor classes on blowup}}
(see also
Proposition~\ref{effect of blowup on genus}), there can be no other
singularities.
\end{proof}
...
\marginpar{\vskip-20pt \hskip-200pt added ``of arbitrary characteristic'' for clarity; and ``alg.closed'' because you say so in Ex 10.3, despite its being a standing assumption.\\[2pt]``We can'' because there \llap{was }no~conclusion to the ``If''}
\redden{over an algebraically closed field of arbitrary characteristic}. 
\redden{We can}
re-embed $C$ by a
Veronese map,
\redden{that is, consider $\widetilde C = \nu_m(C)$ where}
$\nu_m :
...
\redden{If $m$ is high enough,} 
\marginpar{is this what you mean?}
...
\marginpar{moved ``By\dots 1''}
By the adjunction formula, $C$ has genus~1.
\redden{For an arbitrary point}
$o \in C$ not
