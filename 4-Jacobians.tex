...
%\fix{the following could be an exercise, best stated after the $g+3$ theorem.}
\begin{theorem}[$g+1$ theorem]\label{g+1 theorem} 
...
%\fix{we should speculate on the ``$g+n$" theorem. ramification will be proven, I guess, in that section. general position
% of all subschemes of length $n-1$ ? some $k$-normality?}

If we consider only general divisors on \emph{general} curves, we can do still better: Theorem~\ref{grd omnibus}) shows that``most" curves of genus $g$ can in fact be embedded in $\PP^{3}$ as curves of degree $d = \lceil 3g/4 \rceil + 3$.
...
Thus $W^r_d(C)$ is closed in $Jac(C)$, and is thus an algebraic variety, so we can talk about its dimension, irreducibility, smoothness or singularity and so on.
...
\item Show that $h^2$ lies in the subring generated by $f_1,\dots, f_4$, and thus the symmetric square $(\AA^2)^{(2)}$ is a hypersurface, singular
...
%\fix{it sees that the argument neededshows more: as long as $D$ is reduced, the image of
