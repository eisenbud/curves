...
\chapter{Jacobians}\label{Jacobians chapter}\label{new Jacobians chapter}
...
From the universal property it is clear that a fine moduli space for divisors of degree $d$ on $C$ is unique if it exists. Indeed, it does exist, and we'll sketch the construction, using symmetric products. This construction relies on the existence of quotients of schemes by finite groups, and pause to discuss such quotients:
...
We define the functor
 ...
 There exists a fine moduli space $\Pic_d(C)$ for invertible sheaves of degree $d$ on $C$; that is, a scheme $\Pic_d(C)$ and a family of invertible sheaves $\cP$ of degree $d$ on $\Pic_d(C)$, such that for any scheme $B$ there is a natural bijection between families of invertible sheaves of degree $d$ over $B$, modulo invertible sheaves on $B$, and morphisms $B \to \Pic_d(C)$, defined by pulling back $\cP$. The tensor product of invertible sheaves makes $\Pic_0(C)$ an algebraic group, which acts on each $\Pic_d(C)$. The sheaf $\cP$ is called a \emph{Poincar\'e sheaf} for $C$.
 \end{fact}
...
$f(p)dz \in \CC = T^*_{C,p}$. 

It follows that
the kernel of $T^*(\mu_1)O(p)$ is thus the set of global differential forms that vanish at $p$,
and the differential $T(\mu_1)(p)$ carries the tangent space to $C$ at $p$ to $H^0(K_C(-q)) \subset H^0(K_C)$. Since $|\omega_C|$ is  base-point free, there is a global differential $\omega$ 
...
Even with the description of the Picard varieties $\Pic_d(C) \cong J(C)$ in the last section, the Picard varieties are mysterious objects.  But even the facts that there exists a  moduli space for invertible sheaves of degree $d$ on $C$, and that this space is irreducible of dimension $g$ suffice to prove a non-trivial theorem: 
...
In Chapter~\ref{uniformpositionchapter} we will prove similar maps for degree $g+1$ and $g+2$: 
A general effective divisor of degree $g+1$ corresponds to a map $C\to \PP^1$ with only
simple ramification; and a general divisor of degree $g+2$ correspods to a birational  map to $\PP^2$, onto a curve that has $\binom{g}{2}$ ordinary nodes and no other singularities unless $C$
is hyperelliptic, in which case the image has an ordinary $g$-fold point and no other singularities.

We proved in Theorem~\ref{very ample} that every divisor of degree $\geq 2g+1$ is very ample, and this is sharp: the canonical divisor plus 2 points is never very ample. In Theorem~\ref{g+3 theorem}
we get a lower degree embedding by taking a general divisor. This result is also sharp: a hyperelliptic curve of genus $g$ cannot be embedded in any projective space as a curve of degree $<g+3$, as we'll see in Chapter~\ref{ScrollsChapter}. %\fix{I don't think that's there yet}.

If we consider only general divisors on \emph{general} curves, we can do still better: Theorem~\ref{BN omnibus}) shows that``most" curves of genus $g$ can in fact be embedded in $\PP^{3}$ as curves of degree $d = \lceil 3g/4 \rceil + 3$.
...
The preimage of $W^r_d(C)$ is
$$
W^r_d(C) = \left\{ \cL \in \Pic_d(C) \mid \dim(\mu^{-1}(\cL)) \geq r \right\}.
$$
Which is a Zariski-closed subset by the upper-semicontinuity of fiber dimension.
Thus $W^r_d(C)$ is closed in $Jac(C)$, and is thus an algebraic variety, so       s we can talk about its dimension, irreducibility, smoothness or singularity and so on.
...
What is the dimension of ?  Clifford's Theorem says that if $d\leq 2g-2$ and $r \geq d/2$ then the set
 $W^r_d(C)$ is empty. Since $W^r_d(C)$ is a variety, we can strengthen this by bounding its dimension.
 
\begin{theorem}[Martens' theorem]\cite{Martens}
If $C$ is a smooth projective curve of genus $g$ and $d\leq 2g-2$  then
$$
\dim(W^r_d(C)) \leq d-2r;
$$
moreover, equality holds for some $0\leq r \leq d/2$ if and only if $d=2g-2, r= g-1$ or
$d=r=0$ or $C$ is hyperelliptic; and in this last case, equality holds for all $r\leq d/2$.
\end{theorem}

We will prove the inequality here, postponing the rest to Theorem~\ref{full Martens}, which we prove as a corollary of the uniform position theorem. Here we need only a weaker general position result. Given 
...
\begin{proof}
%Let $C^d$ be the Cartesian product and $\alpha : C^d \to C_d$ the quotient map; let $\Phi^r_d = \alpha^{-1}(\Sigma^r_d)$. Since the fibers of $\alpha$ are finite, the lemma is equivalent to the assertion that $\dim \Phi^r_d \leq d-r$; that is,  $\Phi^r_d$ has codimension $\geq r$ in $C^d$.
We do induction on $r$, the case $r= 0$ expressing the fact that since $C$ is nondegenerate
and $d\leq n$, a general set of $d$ points on $C$ is linearly independent.

Consider the incidence correspondence 
$$
X := \{(D,\Lambda)\subset C_d\times Gr(d-r-1, \PP^n) \mid D\subset \Lambda\}
$$
where $Gr(d-r-1, \PP^n)$ is the set of $d-r-1$-planes in $\PP^n$.
For each $s\geq 0$ the fiber of $X$ over a point where $\overline D$ has dimension exactly
$d-r-1-s$ is $s$-dimensional. 
\fix{I presume this is supposed to be the fiber of $X$ over a point $D\in C_d$
where $\overline D$ has dimension exactly
$d-r-1-s$. But isn't this is isomorphic to the Grassmannian of $s-1$ dimensional spaces in $\PP^{n-(d-r-1-s-1}$,
which is a lot larger than $s$-dimensional. What am I missing??}

By induction this set has dimension $\leq d-r-s$,
so $X$ has dimension at most $d-r$. Since the projection $X\to \Sigma^r_d$ is surjective,
$\dim \Sigma^r_d\leq d-r$ as well.
%
%Let $\pi : C^d \to C^{d-r}$ on the first $d-r$ factors.
%\fix{I don't understand the following}
%We will proceed by induction on $d$. A general $(d-r)$-tuple of points on the curve are linearly independent, so the general fiber of the restriction $\pi_\Phi : \Phi \to C^{d-r}$ is finite \fix{ I think it's empty!}. By  induction, we may assume that the locus of $(d-r)$-tuples of points on $C$ that span only a $(d-r-1-s)$-plane has codimension at least $s$ in $C^{d-r}$. Since the fibers over this locus have dimension $s$, we conclude that $\dim \Phi \leq d-r$.
\end{proof}
...
$W^r_d$ have dimension at least $r$, Marten's inequality will follow from the corresponding inequality
$$
\dim C^r_d \; \leq \; d-r.
$$
By the Geometric Riemann-Roch theorem~\ref{geometric RR}, $D\in C^r_d$ if and only if
$\dim \overline{\phi_K(D)}\leq d-r-1$, so by  Lemma~\ref{elementary secant plane lemma} the set of such $D$ has dimension $\leq d-r$, as required.
\end{proof}
