...
Clifford's Theorem says that if $d\leq 2g-2$ and $2r>d$ then the set
 ...
 Martens' Theorem bounds the dimension of $W^r_d(C)$ in the remaining cases, $d/2 \geq r > d-g$:
...
The listed cases of equality are easy to verify: for example the condition $r>d-g$ implies that $W^r_d$ consists of special line bundles, and
on a hyperelliptic curve Corollary~\ref{special on hyperelliptic} shows that such sheaves with $r(\sL) = s$ can be written $\sL_0^{\otimes s}(p_1+\cdots + p_{d-2s})$, where
$\sL_0$ is the unique $\g^1_2$ on $C$ and the $p_i$ are base points---a family of dimension $d-2s$.

We will prove the inequality here, postponing the proof that only hyperelliptic curves achieve equality nontriviallyl to Theorem~\ref{full Martens}, which we prove as a corollary of the strong general position result Theorem~\ref{uniform position}. For the inequality we need only a weaker general result:
...
obtained by subtracting one point that is in the support of $D$.
...
\{q+D' \mid q\notin \Lambda \cap C.
$$
From this we see that $\Sigma^r_d$ is in the closure of  $\Sigma^{r-1}_d$. It follows that the sequence
$$
\dim \Sigma^0_d, \dim \Sigma^1_d,\dots, \dim \Sigma^r_d
$$ 
is strictly decreasing, and thus $\dim \Sigma^r_d \leq d-r$ as claimed.
\end{proof}


\begin{proof}[Proof of Martens' inequality] Since we have assumed $r>d-g$, the set $W^r_d$ contains only  sheaves.
