...
As an application of the mere existence of the spaces $C_d$ and $\Pic_d$, we show in Section~\ref{g+3 section} that a general divisor of degree $g+3$ on any curve of genus $g$ gives rise to an embedding in $\PP^3$ as a curve of degree $g+3$, and we can prove related results for general divisors of degree $g+2$ and $g+1$. 

%Similar techniques, in Section~\ref{g+2 section} show that a general divisor of degree $g+2$ on any curve gives a birational morphism to a plane curve of degree $g+2$. Moreover, if the curve is not hyperelliptic then its image in $\PP^2$ has  only nodes
...
By Hurwitz' theorem the total ramification of such a map is $2g+2$. To show that there are $2g+2$ distinct 
ramification point, it suffices to show that $D-2p -2q$ has no sections when $p\neq q$. In the contrary case
$D ~ E+2p+2q$ for some effective divisor $E$ of degree $g-3$, and the set of such divisor classes
is the image of $C_{g-3} \times C \times C$, a subvariety of dimension only $g-1$. Thus a general divisor
of degree $g+1$ is not equivalent to a divisor of this form.
\end{proof}

It would be natural to hope that a general divisor of degree $g+2$ defines a map to $\PP^2$ whose
image has only ordinary nodes as singularities, but this is not quite true. Here is the correct result,
...
On the other hand, the corresponding result does show that a general divisor of degree $g+3$ is very ample on any curve:
...
we get a lower degree embedding by taking a general divisor. 
