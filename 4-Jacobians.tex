...
Our first goal is to find a space parametrizing effective divisors on $C$. This is readily available: since an effective divisor of degree $d$ on $C$ is an unordered $d$-tuple of points on $C$, with repetitions allowed, it corresponds to a point in the \emph{$d$th symmetric power} $C^{(d)}$ of $C$, defined to be the quotient of the ordinary product $C^d$ by the action of the symmetric group $\Sigma_d$ on $d$ letters. 

We should say a few words here about the construction of the symmetric product in general. This is completely elementary and general: given any variety $X$, we can form the ordinary Cartesian product $X^d$, and take the quotient of $X^d$ by the action of the symmetric group permuting the factors of $X^d$. The resulting variety $X^d/S_d$ is called the $d$th \emph{symmetric power}, or \emph{symmetric product}, of $X$, and denoted $X_d$ or $X^{(d)}$. (For some reason, it seems to be $X_d$ for $X$ a curve, and $X^{(d)}$ for $X$ any higher-dimensional variety.)

The symmetric power $X^{(d)}$ of an $r$-dimensional variety $X$ is a variety of dimension $rd$; it's affine if $X$ is affine and projective if $X$ is projective. (The quotient of an affine (resp., projective) variety by a finite group is again affine (resp., projective.) But there is one aspect of the symmetric powers that is special to the case of curves: for $X$ any variety of dimension $\geq 2$, the symmetric powers $X^{(d)}$ are singular for all $d \geq 2$, while if $X$ is a smooth curve then the symmetric powers $X^{(d)}$ are all smooth as well. This can be notationally messy to prove (see for example~\cite{***} (ACGH)). If we're willing to invoke the classical, or analytic, topology, however, it's not hard to see: in the case of $X = \PP^1$, the fundamental theorem of algebra says that the symmetric power $(\PP^1)^{(d)}$ is simply the projective space $\PP^d$ associated to the vector space of homogeneous polynomials of degree $d$ on $\PP^1$; and in the analytic topology every curve looks locally like $\PP^1$.
...
$$
is a closed subvariety of the product $C^{(d)} \times C$, whose fiber over any point $D \in C^{(d)}$ is simply the divisor $D \subset C$
\end{exercise}

This is called the universal divisor on $C$ by virtue of the fact that for any family of divisors of degree $d$ on $C$---that is, a scheme $B$ and a subscheme $\cE \subset B \times C$ flat of degree $d$ over $B$, there is a unique map $\phi : B \to C^{(d)}$ such that $\cE$ is the pullback via $\phi$ of $\cD\subset C^{(d)} \times C$. Indeed, this amounts to saying that $C^{(d)}$ is the \emph{Hilbert scheme} parametrizing subschemes of $C$ of degree $d$.

\begin{exercise}
Show by contrast that for $X$ a variety of dimension $r \geq 2$, the symmetric power $X^{(d)}$ is \emph{not} the Hilbert scheme of subschemes of dimension 0 and degree $d$ on $X$.
\end{exercise}
...
$$
and this corresponds to a regular map $\alpha : \PP^1 \to C^{(d)}$. 
...
In the other construction, we can actually parametrize the set of linear series $g^r_d$ on $C$: that is, there is a scheme \fix{probably this should be Roman} $\cG^r_d(C)$ parametrizing pairs $(L, V)$ with $L \in \Pic^d(C)$ and $V \subset H^0(L)$ a subspace of dimension $r+1$. Again, the scheme structure may be characterized by saying that $\cG^r_d(C)$ represents the functor of families of linear series on $C$. Note that the natural map $\cG^r_d \to W^r_d$ is an isomorphism over the dense open subset $W^r_d \setminus W^{r+1}_d$, and more generally its fiber over a point of $W^s_d \setminus W^{s+1}_d$ is a copy of  the Grassmannian $\GG(r, s)$. 
