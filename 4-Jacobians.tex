...
If $G$ is a finite group acting by automorphisms on an affine scheme $X:=\Spec A$ then $X/G$ is by definition $\Spec(A^G)$, the spectrum of the ring $A^G$ of invariant elements of $A$. It is a basic theorem of commutative algebra that the fibers of the map $X\to X/G$ are actually the orbits of $G$ (see \cite[Theorem ***]{E}.) Since the map $X\to X/G$ is finite, $\dim X/G = \dim X$. The construction localizes, and thus passes to more general schemes as well. (The situation is more complicated for infinite groups, where geometric invariant theory comes into play.) 

In particular, for any variety $X$, we can form the ordinary Cartesian product $X^d$, and take the quotient of $X^d$ by the action of the symmetric group permuting the factors of $X^d$. The resulting variety $X^d/S_d$ is called the $d$th \emph{symmetric power}, or \emph{symmetric product}, of $X$, which we denote by  $X^{(d)}$. 
...
There is one aspect of the symmetric powers that is special to the case of curves:  if $X$ is a smooth curve then the symmetric powers $X^{(d)}$ are all smooth as well. (By contrast, if $\dim X \geq 2$ then the symmetric powers $X^{(d)}$ are singular for all $d \geq 2$.) For example, identifying a divisor of degree $d$ on $\PP^1$   with the degree $d$ form vanishing there, we see that $\PP^1)^{(d)}$ may be identified with the projective space of forms
$\PP^d$. The general case follows because locally analytically the action of the symmetric group on $C^d$ is the same as in the case $C=\PP^1$.
...
\fix{We haven't spoken of Hilbert schemes yet; and I'm not sure how the innocent reader is supposed to prove the following ``fact'' or the next exercise.}
...
\fix{DE revised to here 10/24/21}
To express the resulting construction in relatively modern terms, let $C$ be a smooth projective curve of genus $g$ over $\CC$, and consider the space $V = H^0(K_C)^*$ of linear functions on the space of holomorphic differentials $H^0(K_C)$. Integration over a closed loop in $C$ defines a linear function on 1-forms, so that we have a map
$$
