...
To define the universal family, consider the morphism $\alpha: C^d \to \coprod^d C$ the disjoint union of $d$ copies of
$C$, where the map from the product to the $i$-th copy is the $i$-th projection, and the map
$\beta:\coprod^d C \to C$ which is the identity on each copy of $C$. Let $\Gamma \subset C^{d+1} = C^d \times C$
be the closure of the graph of $\alpha\pi$
\end{fact}
