...
The fact that line bundles of a given degree of a curve $C$ are parametrized by the points of a variety $\pic^d(C)$ has huge consequences. Just applying dimension theory to $\pic^d(C)$, for example, gives us a proof of the fact that every curve can be embedded in projective space as a curve of degree $g+3$, as we'll see in Theorem~\ref{g+3 theorem} below.
