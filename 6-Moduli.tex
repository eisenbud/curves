...
of projective schemes over $E := \Spec \CC[\eps/(\eps^2)$:
...
A $U$-module $M$ is flat if and only if $Tor_1^U(\CC,M) = 0$. 
...
defines a scheme over $E$ which restricts to $I$ modulo $(\eps)$. 
...
Finally, we must construct a homomorphism from a flat family over $\CC[\eps]$ in a way inverse to the construction above: Given the ideal $\tilde I subset S[\eps]/(\eps)^2 = S\oplus \eps S\eps$,
let $J$ be the image of $\tilde I$ in $S\oplus S\eps/I\eps$. We claim that $J$ is the graph of a homomorphism $\phi: I \to  S\eps/I\eps \cong S/I$; that is, $J$ projects isomorphically onto $I$ in the first factor. The fact that $\tilde I$ restricts to
$I$ modulo $\eps$ show that at least the image of $J$ in the first factor is $I$. The kernel of the projection is
the intersection of $J$ with the second factor $(\eps S)/(\eps I)$. However, an element of this intersection
would represent an element of $S[\eps]/((\eps^2)+\tilde I)$ that is killed by $\eps$ but is not in $\eps S/\tilde I = (\eps S)/(\eps I)$
\fix{complete this}
