...
\item \emph{Moduli of smooth curves of genus $g$}. The objects are isomorphism classes of smooth, projective curves of genus $g$. A \emph{family of curves of genus $g$} over $B$ is an equivalence class of smooth, projective morphism $f : \cC \to B$ whose fibers are curves of genus $g$, wheres two such families $f, f'$
...
We have already encountered examples 1 and 2 in Chapter~\ref{}. Severi varieties will be discussed in Chapter~\ref{}; we will discuss Hurwitz spaces at the end of this chapter, in connection with basic facts about the moduli space $M_g$. Finally, Examples 5 and 6---Hilbert schemes, and the moduli space $M_g$ of abstract curves---are the most fundamental, and will be discussed in the remaining sections of this Chapter.
...
In the remainder of this chapter, we will discuss the moduli space most central to the theory of algebraic curves: the moduli space of curves of genus $g$ up to isomorphism.
...
There are two immediate issues to deal with. The first, that we are excluding hyperelliptic curves in this construction, has a relatively straightforward solution: instead of looking at the canonical map, we look at the bicanonical map $\phi_{2K} : C \to \PP^{3g-4}$ (or, if we want to include the case of genus 2, the tricanonical map  $\phi_{3K} : C \to \PP^{5g-6}$). This means we have to replace the relatively simple ``space of smooth plane quartic curves" (an open subset of $\PP^{14}$) with the more daunting ``locally closed subset of the Hilbert scheme of curves of genus $g$ and degree $6g-6$ in $\PP^{5g-6}$," but since we've introduced and constructed the Hilbert scheme in Chapter~\ref{**} this is more a matter of comfort than rigor.

The second issue is more serious: whereas in the case of genus 2 curves in the preceding section we were taking the quotient of a variety by a finite group---which we know always exists---in the proposed construction here we want to take the quotient of a subset of the Hilbert scheme by the positive-dimensional group $PGL_{5g-5}$, and the existence of this quotient is more problematic.
...
Geometric invariant theory, or GIT, deals with exactly this issue in general. The situation to which we apply GIT is this: we have a quasiprojective variety $X \subset \PP^N$ and a reductive subgroup $G \subset PGL_{N+1}$ that carries $X$ into itself; we want to know if we can form a quotient $X/G$, and what its properties are.

When we discussed the quotient of an affine variety $X$ by a finite group $G$, it was clear how we had to proceed: we took the coordinate ring $R$ of $X$, on which $G$ acted; and the quotient $X/G$ was just the spectrum $\Spec(R^G)$ of the ring of invariant functions on $X$. We can try to do a similar thing here, by letting $A$ be the homogeneous coordinate ring of $X \subset \PP^N$, and taking the quotient $X/G$ to be $\Proj(A^G)$. But there's one problem we have to deal with first: since the elements of the homogeneous coordinate ring are not functions on $X$, the group $G$ may not act on $A$. Thus, we need to start with an action of $G$ on $A$ compatible with the action of $G$ on $X$. This is called a \emph{linearization} of the action, and always exists after a Veronese re-embedding of $X$ (that is, replacing $A$ by the direct sum of the graded pieces of $A$ of degree divisible by a suitable $m$).

So, suppose we are given a linearization of the action of $G$ on $X$. By Hilbert's theorem, the subring $A^G \subset A$ of invariant elements is finitely generated over our ground field, and if we want to take the quotient of $X$ by $G$ our first guess would be to take $X//G := \Proj(A^G)$. The question then is, in what sense is this a quotient, and in particular
what is the relationship between the points of $X//G$ and the orbits of $G$ acting on $X$? 
...
\item Lastly, there are the \emph{unstable} orbits. These are the ones that cannot be included in any quotient without resulting in a non-separated space, and thus do not correspond to points of the quotient.
...
$$
there exists a surface $S$ birational to $\tilde \cC$ such that the induced map $S \to \Delta$ is regular and has stable fibers.s
...
A similar approach works through genus 10, but then this approach fails as well: if we represent a general curve of genus 11 as a plane curve with nodes, the nodes are no longer general points of $\PP^2$, and the same argument doesn't work. Ad hoc (and much more difficult) arguments were given in genera 11, 12 13 and 14, but so far no-one can go further in producing general curves. 
...
\subsection{Coarse vs. Fine moduli spaces}
...
\section{Hurwitz spaces and the dimension of $M_g$}
