...
\item \emph{Severi varieties: curves with a birational embedding in $\PP^2$}. The objects here are plane curves
...
%; for example, if the map is a birational embedding, we can specify the curve just by specifying a set of polynomial equations cutting  out the image. 

For example, we can look at pairs $(C,f)$ where $f : C \to \PP^1$ is a branched cover of degree $d$; this yields various Hurwitz spaces.
...
Thus one must look for good embeddings and linearizations for which the objects one cares about---in our case perhaps the smooth curves---become stable points, and this is actually attained by choosing the multiple $d$ in the sketch above sufficiently large. 
%\fix{to take into account...: look up D\&M}. 
...
An alternative approach to this problem is to enlarge the category of schemes to the category of  \emph{stacks}, where in fact a fine moduli space for curves of genus $g$ does exist. This is the approach taken by Deligne and Mumford in~\cite{DM}. The type of stack they introduce---now called \emph{Deligne-Mumford stacks}---have associated coarse moduli spaces, so their approach actually produces both.
