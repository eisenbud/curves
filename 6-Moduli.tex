...
\subsection{Examples}
\fix{I'd like to reorder this chapter in order of difficulty: eff divisors, line bundles, Hilbert scheme, $M_g$; and postpone the discussion of the coarse moduli space until the $M_g$ section, perhaps adding some more details about GIT.}
...
\fix{The Yoneda's Lemma that I learned says 
$$
Hom_{funct}(Hom_{Schemes}( -, X), F) = F(X)
$$ Thus any map of a representable functor to the functor of families corresponds to a family. 
But the proof of the isomorphism we want is direct: it's built into the definition you give of the
universal family.}
...
smooth curves.  
\fix{ the reader will want to know *why* a fine $M_g$ doesn't exist. This needs some talk about
the tangent space/local problem and the automorphisms of the versal deformation space, I think.}
...
identity map $\CC^1 \to \CC^1$, in which the nonzero point ``degenerates" to the zero point. Thus the moduli "space" should be an object with two points, one in the closure of the other---evidently not a scheme. The relevant ring of invariants, $\CC = \CC[x]^{\CC^*} is no use: it suggests that the quotient should be a single point (and indeed this is the best approximation among schemes).
...
Though Riemann already (correctly) calculated the dimension of the moduli space $M_g$ as a real variety by heuristic arguments (as we did in preceding chapters), carrying out the programme outlined above for smooth curves turned out to be quite difficult. The first rigorous treatment came only in 1969, with the publication of the landmark paper \cite{Deligne-Mumford}. Two results in that paper fundamentally altered the landscape of curve theory:
...
Examples of both of these can be found in \cite[****]{GeomSchemes}.\fix{add precise reference}
...
\fix{citations to add: Constructions: Grothendieck Fonde;ments; \cite[Section IV]{Grothendieck19571962}; Mumford \cite[Chapter 15]{Mumford1966}, curves on an alg surface; Homogeneous Hilbert Schemes
\cite{MR2361849}; Iarrobino~\cite{Iarrobino1985} and/or other work on punctual Hilbert schemes. \cite{BayerThesis}, Ramkumar \cite{fiber-full} 
}
