...
This means that, in  $\eps a= \sum(r_i+\eps s_i)(g_i+\eps h_i) \in T$.
It follows that 
$\sum r_i\g_i = 0$ Since $\phi$ is a homomorphism this implies that $\sum r_i h_i \in I.$ Thus
$\eps a \in  \eps I\subset T$, whence $a\in I$.  Writing $a = p_i g_i$ 
we have $a \equiv \eps p_i h_i \hbox{ mod } I'$, as required.

Finally, we must construct a homomorphism from a flat family over $U$ in a way that is inverse to the construction above: Given an ideal $\tilde I \subset T$ such that $T/\tilde I$ is flat over $U$
let $J$ be the image of $\tilde I$ in $S':= S\oplus S\eps/I\eps$. We claim that $J$ is the graph of a homomorphism $\phi: I \to  S/\eps I$. Equivalently, we claim that the subspace $J$ projects isomorphically onto $I$ in the first factor. 

By hypothesis, $\tilde I + \eps S = I +\eps S$, so the projection  to the first  summand of $S'$ maps $J$ onto $I$. The kernel of this projection is
the intersection of $J$ with the second factor $(S )/(\eps I )$. To show that this is 0 we must show that
if $r\in S$ and $\eps r \in \tilde I$ then $\eps r \in \eps I$. Since $\eps r \in \tilde I$, the condition of flatness implies
...
$J$ defines a homomorphism $\phi$ as claimed, which means that
 $\tilde I$ is generated by $\{g+\phi g\mid g\in I\}$.
\end{proof}
