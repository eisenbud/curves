...
Third, to the extent that we can describe the intersection theory of a moduli space, it opens up the possibility of doing enumerative geometry on it. As a very elementary example, knowing that the parameter space for lines in $\PP^3$ is the Grassmannian $\GG(1,3)$, and knowing something about the geometry of Grassmannians allows us to conclude the basic existence theorem: \emph{given any four curves in $\PP^3$, there is a line meeting all four}. That's a very elementary statement, but the same idea was used to prove the existence of linear series on curves (the ``existence" half of the Brill-Noether theorem, discussed in Chapter~\ref{Brill-Noether}). 
...
morphism $B\to M$ be unique. Such a space $M$ with its universal family $\phi$, if it exists, is called a \emph{fine moduli space}. This can be expressed more abstractly but more succinctly by saying that there is an isomorphism of functors:
$$
...
$p(m)$. The space $Hilb_{p}(\PP^2)$ is thus the projective space $\PP^{{d+2\choose 2}-1}$ of forms of degree $d$,
...
\begin{definition}\fix{change this to a property of $M_g$ and Hurwitz; don't make a def. refer to somewhere where DM stacks are treated}
...
Thus, in plain language, the theorem that $M_g$ is a coarse moduli space \fix{this is a property of Mg, not "course"} for smooth curves of genus $g$ means: 
\begin{enumerate}
