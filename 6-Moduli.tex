...
%\fix{need a reference for Matsusaka}
...
y^2 - x^3 - t^2ax - t^3b
$$
has the cuspidal curve $y^2-x^3$ as limit. \fix{what's the simplest way to see this?}
$$y^2 = x(x-1)(x-\lambda)$$
$x -> x-\lambda/3$.
...
%\fix{ Uniqueness?? else we'd have maps M\to M' \to M, compositions identity on closed points.; so what??}
...
Let $C$ be any smooth curve of genus $g$ with an involution $\sigma : C \to C$, and let 
$$
...
%\end{enumerate}
% \end{exercise}
%\fix{Check ACGH to see whether we can put in something local-analytic}
