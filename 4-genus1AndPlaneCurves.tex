...
\marginpar{replaced ``the historical appendix'' by chapter number, in part for ease of linking; dropped ``more''} 
\redden{(See Chapter \ref{Appendix-History}}
for details.)
\looseness=-1
...
\redden{what we know as the Riemann sphere, or $\PP^1_\CC$.} 
...
\marginpar{removed quotes, placed thm number in parentheses}
...
\marginpar{dropped ``and'', changed semicolon to period in (1), and normalized punctuation also in the subitems}
...
\marginpar{or else ``we also have $f_{x}(p)\ne0$''}
\redden{is nonzero,} 
...
genus of $C$, then 
\marginpar{dropped ``we must have''} 
$2g-2 = d(d@{-}@3)$, whence 
\vspace*{-2pt}
$$
...
\redden{and on}
\marginpar{\hskip-60pt $\leftarrow$ the ``plus'' threw me off for some reason. Is this what's meant?}
...
\marginpar{removed unmatched left quotes before ``failure''}
$E'$ to impose 
...
\marginpar{dropped ``hand'' twice, ok?}
of the equality is 
$$
...
\marginpar{dropped ``any'' to allow line break} 
\redden{sections,}
...
\marginpar{requested term ``parametrized'' indexed as ``parametrization of the space of curves'', ok?}
the points of a 
...
\marginpar{indexed ``double cover'' under cover!2-sheeted to catch all in the same place; added ``see'' entry under ``double cover''}
...
\marginpar{\vskip-30pt promoted this subsection and the next to section}%
...
\marginpar{dropped ``spanned by $o,q$'' (notation is standard and has been defined before)}  
...
There is 
\marginpar{dropped ``also'' to allow line break} 
...
\redden{$\phi_D(D)$}
\marginpar{\redden{should this be $\phi_D(E)$?}}
...
\marginpar{dropped repeated ``map''}
$$
...
\redden{of genus 1 and degree 3}
\marginpar{to allow line break} 
