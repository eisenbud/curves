...
%
%If $n=3$ consider the projection $\Gamma = \{ ((p,q), H) \in C_2 \times {\PP^3}^* \mid p,q \in H\} \to C_2$.
...
% 
% Note that this
%argument also applies to the hyperelliptic case.
...
$binom{g+1}{2} -g = \binom{g}{2}$, so the singularity cannot have multiplicity $>g$, and is an ordinary $g$-fold point. Since the $\delta$-invariant of such a point is $\binom{g}{2}$ \fix{give a ref to the plane curve chapter} there can be no other singularities of $C_0$.
\end{proof}
...
& = M\bigl( M(r-1) + 1 + \epsilon \bigr) - \bigl(  \frac{M(M+1)}{2}(r-1) + M  \bigr) \\%there was an extra+ 1
...
We will prove below that this monodromy group is actually the full symmetric group. But first we introduce the general setting.
