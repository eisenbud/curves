...
\begin{proposition}\label{mult and stat}
 If $C\subset \PP^n$ is a reduced, irreducible, curve that is neither planar nor strange, the $mult(C)\subset C_2$
 and $stat(C)\subset C_2\setminus \Delta$ are closed subsets of dimension $\leq 1$.
\end{proposition}

\begin{proof}
To show that $mult(C)$ is closed, consider the finite morphism 
$$
\{((p,q), r) \in C_2\times \PP^n \mid r \in \overline{p,q}\} \to C : ((p,q),r)\mapsto r.
$$
By the semicontinuity of the degree of fiber dimension, the set of points lying in fibers
of length $\geq 3$ is closed, and $mult(C)$ is the image of this set. Because the map is finite, this shows that
...
$$
is closed in $C_2\setminus Delta\times \PP^n$, its image under the first projection,
...
follows that $stat(C) = C_2\setminus \Delta$. Now any three
lines that meet pairwise must be either coplanar or meet in a point. Thus if $C$ is not strange,
the all all the tangents to $C$ lie in a common plane, and $C$ planar, contradicting our hypothesis
...
To this end, consider the incidence correspondence
$$
\begin{diagram}
I := \{(p,r,r')\in C^3 \mid p,r,r' \hbox{ are distinct smooth points of $C$} \\
\hbox{such that $\pi_p(r) = \pi_p(r')$ is a smooth point of $\pi_p(C)$.}\},
\end{diagram}
$$
...
and thus they line in the 2-plane spanned by $p$ and this line; thus $(r,r') \in stat(C)$.
The projection $(p,r,r') \mapsto (r,r')$ is finite since the secant line $\overline{r,r'}$ meets $C$ in only finitely many points, 
so $\dim I \leq \dim stat(C)$. If the projection from some point $p$ were everywhere ramified,
...
the fiber $\pi_p^{-1}(q)$ contains two distinct points of $C$. Thus the projection of $I$ to the first factor $p$ maps
$I$ onto $C$, with 1-dimensional fibers, so $I$ and thus also $stat(C)$ has dimension 2. \end{proof}


\begin{proof}[Proof of Theorem~\ref{linear general position}]
First suppose that $n=3$, and consider the incidence correspondence
$$
I := \{ ((p,q), H) \in mult(C) \times \PP^{3\vee} \mid \overline{p,q}\in H\}.
$$
Since $\dim \overline{p,q} = 1$, the fibers of the projection of $I$ to $mult(C)$ have dimension 1,
so $\dim I = 1+\dim mult(C)  \leq 2$. Consequently the image of  the projection of $I$ to $\PP^{3\vee}$ has
...
$C$ under the projection $\pi_p: C\to C'$ from a general point of $C$ is not strange.
By Proposition~\ref{mult and stat} we may choose two smooth points
$r,r'\in C\subset \PP^n$ such that $T_r(C)\cap T_{r'}(C) = \emptyset$, so 
...
$$
\begin{aligned}
I_1 &= \{(p,H) \in C \times \PP^{n*} \mid p\in H \hbox{ and some subset of $n$ points of $H\cap C$
that contains $p$ is dependent}\}\\
\subset 
I &= \{(p,H) \in C \times \PP^{n*} \mid p\in H \hbox{ and some subset of $n$ points of $H\cap C$ is dependent}\}.
\end{aligned}
$$
Suppose that the general hyperplane in $\PP^n$---and therefore every hyperplane in $\PP^n$---contains 
a dependent set of $n$ points. It follows that the first projection of $I$ is surjective, with all fibers
isomorphic to $\PP^{n-1}$, so $I$ is irreducible of dimension $n$. But the projection of $I_1$ to $\PP^{n*}$ is also
surjective, so $\dim I_1 = \dim I$, and $I_1$ is closed in $I$, 
%\fix{say why}
so they are equal, and $p$ is part of a dependent set
$\{p, p_2,\dots, p_n\}$ in every hyperplane containing $p$. 
%\fix{this seems to be a weak form of the uniform position lemma -- without char 0}
...
are dependent points of $H'\cap C'$, contradicting the induction hypothesis, and proving that
...
%\end{proof}
%
%\begin{proof}[Proof of Theorem~\ref{basic linear independence}]
...
%
%If $n=3$ consider the projection $\Gamma = \{ ((p,q), H) \in C_2 \times {\PP^3}^\vee \mid p,q \in H\} \to C_2$.
%Since the set of $(p,q)$ definining a multisecant has dimension $\leq 2$, not every plane in $\PP^3$
%contains a multi-secant, as required.
%
%Thus we may suppose $n\geq 4$, and do induction on $n$.
%
