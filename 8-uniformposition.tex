...
One way to study a curve in $\PP^n$ is to use properties of its hyperplane sections. For a start, we show that any $n$ points of a general hyperplane section are linearly independent except in a very special case. Then we will prove a much stronger statement in characteristic 0,
the \emph{uniform position lemma}.
...
all the tangent lines to $C$ meet in a point $p\in \PP^n$. Note that the projection of $C$ from $p$ is inseparable, and thus there are no strange curves in characteristic 0. Pierre Samuel showed that the only smooth curve that is strange (in any characteristic) is the conic in characteristic 2; a proof is given in \cite[Theorem IV.3.9]{Hartshorne1977} (but see
Exercise~\ref{strange curves} for singular examples in finite characteristic.) 

For non strange curves, general hyperplane sections consist of points in linearly general position in the following sense:
...
and $H$ is a general hyperplane, then every subset of $n$ points of
$H\cap C$ is linearly independent.
\end{proposition}
...
We do induction on $n$ beginning with $n=3$. Since $C$ is not strange, Exercise~\ref{multisecants to strange} shows that 
 %\cite[Proposition IV.3.8]{Hartshorne1977} shows that 
 not every secant of $C$ is a multisecant, and this takes care of the case, $n=3$.  It also shows that there is a smooth point  $p\in C$ such that the projection $\pi_p$ from $p$ is birational. Let $d= \deg C$.

We claim that $C'$ is not strange. By By Exercise~\ref{multisecants to strange}  \cite[Proposition IV.3.8]{Hartshorne1977} it suffices to show that the  tangent lines 
to $C'$  at general points do not meet. Choose general points $q,r\in C$, and let $V\subset \PP^n$ be the projective space spanned by the tangent lines at $q,r$, which has dimension 3 since $C$ is not strange. Since $C$ is nondegenerate and $n>3$, we may assume that the projection center
$p$ is not in $V$. Thus projection from $p$ is an isomorphism when restricted to $V$, so the tangent lines at $\pi_p(q), \pi_p(r)$
which are the images of the tangent lines at $q,r$, do not meet, showing that $C'$ is not strange.

By induction there is a hyperplane $H'\subset \PP^{n-1}$ whose
