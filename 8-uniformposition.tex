...
One way to study a curve in $\PP^n$ is to use properties of its hyperplane sections. For a start, we show that any $n$ points of a general hyperplane section of a reduced irreducible curve in characteristic 0 or a nonsingular curve in any characteristic are linearly independent except in a very special case (but see Exercise~\ref{strange curves} for
interesting singular examples in positive characteristic). 

In the next section we return to the characteristic 0 case to prove the much stronger \emph{uniform position lemma}.
...
all the tangent lines to $C$ meet in a point $p\in \PP^n$. In this case the projection of $C$ from $p$ is inseparable, and thus there are no strange curves in characteristic 0. Pierre Samuel showed that the only smooth curve that is strange (in any characteristic) is the conic in characteristic 2; a proof is given in \cite[Theorem IV.3.9]{Hartshorne1977} 

If $C\subset \PP^n$ is a reduced curve then, by Bertini's theorem, a general hyperplane
section of $C$ consists of smooth points. For a non-strange irreducible curve $C$, a general hyperplane section $H\cap C$ consists of points in linearly general position.
...
A \emph{multisecant} line to a curve $C\subset \PP^n$ is a line whose
scheme-theoretic intersection with $C$ has length $\geq 3$.

\begin{lemma}\label{incident tangents}
If $C\subset \PP^n$ is a non-strange, non-planar reduced irreducible curve, then not every
secant line to $C$ is a multisecant and for an open set of pairs of points
 $(p,q)\in C\times, C$, the tangent lines to $p,q$ do not meet.
meet.
\end{lemma}
...
$(r,s)\in C\times C$, the tangent lines at $r,s$ intersect. 

Since each pair of tangent lines at general points meet, they must all pass through
a single point or lie in a single plane. In the first case the curve would be strange,
...
The set of hyperplanes whose intersection with $C$ is in linearly general position is open in $\PP^{n*}$ so it suffices to show that it is nonempty.

 Since not every secant is a multisecant,
projection $\pi_p$ from a general point  $p\in C$ ] is birational onto its
 image $C'\subset \PP^{n-1}$. 
We claim that $C'$ is not strange. By Lemma~\ref{incident tangents} it suffices to show that the  tangent lines 
to $C'$  at general points do not meet. Choose general points $q,r\in C$, and let $V\subset \PP^n$ be the projective space spanned by the tangent lines at $q,r$, which has dimension 3 by the argument above. Since $C$ is nondegenerate and $n>3$, we may assume that the projection center
$p$ is not in $V$. Thus projection from $p$ is an isomorphism when restricted to $V$, so the tangent lines at $\pi_p(q), \pi_p(r)$
which are the images of the tangent lines at $q,r$, 
...
$\PP^n$ whose points project to the points of $H'\cap C'$, together with $p$, and this is a set in linearly general position.
\end{proof}
...
$\pi_\Lambda$ is an isomorphism, while if $n\geq 3$ then $\pi_\Lambda$ is a birational map whose image is a curve with only ordinary nodes.
\end{proposition}
...
$$
I:=\{(\lambda, p)\mid \lambda\in C^{2)} \hbox{ is a length 2 subscheme of }C,\ p\in \overline \lambda\subset\PP^n\}
$$
...
we need to show that $\Lambda$ does not lie on any trisecant line. 

By Proposition~\ref{basic linear independence} the family of trisecant lines to $C$ is a proper subscheme of the irreducible two-dimensional family of secant lines, so the union of the trisecant lines is 2 dimensional, and we see that the fibers of $\pi_\Lambda$ all have dimension $\leq 2$. Furthermore, the general fiber of the projection
...
We start by giving a precise definition of the mondromy group in a more general setting, and prove that points of a general hyperplane section of an irreducible curve
...
$$
The image $M$ of this map is called the \emph{monodromy group} of the map $f$. It depends on the labeling of the points of $\Gamma$, but is well-defined  up to conjugation. In our setting $\pi_1(U,p_0)$, and thus the monodromy group, is independent of the choice of open set $U$: if $U' \subset U$ is a Zariski open subset, the complement of $U\setminus U'$ has
...
Theorem~\ref{uniform position lemma} fails over fields of finite characteristic; see Exercise~\ref{strange curves} for examples, and \cite{Rathmann} and \cite{Kadets} for what is known. 
...
To
 show that  $M$ is the full symmetric group, it suffices to show that $M$ is twice transitive; and that $M$ contains a transposition. It follows that $M$ contains all transpositions, and hence that $M = S_d$.
...
\fix{Joe was going to put in a $g+1$ theorem}
...
In characteristic $p > 0$ it we do not know whether a general projection of a smooth curve to $\PP^2$ is nodal \fix{what's wrong with the proof of Prop~\ref{nodal projection}??} ---but it's still true that every curve can be birationally mapped onto a nodal curve in $\PP^2$
