...
In the following sections of this chapter we give a series of applications of this result, and in Sections~\ref{high degree ACM} and
\ref{canonical ACM} we further apply the result to show that canonical curves and curves of high degree are
arithmetically Cohen-Macaulay. In Section~\ref{uniform position section} we return to the characteristic 0 case to prove the much stronger \emph{uniform position lemma}. Finally we give some applications
...
arithmetic genus using Theorem~\ref{RR for integral curves}, the Riemann-Roch theorem for integral curves
...
\item $\deg (\sL \otimes \sF) = \deg (\sF) +\rank(\sF)\deg (\sL).$
...
copies skyscraper sheaves with 1-dimensional fibers at various closed points $p_i\in C$ we see that $\chi(\sG) = \dim_\CC(H^0(\sG))$. Applying additivity again, we get item 1.

For item 2, note first that we can write $\sL$ as $\sL_1\otimes_C(\sL_2)^{-1}$ for very ample invertible sheaves
$\sL_i$ such that $\sL_i\otimes_C\sF$ is generated by global sections. If $\sL$ is very ample and $\sL\otimes \sF$ is generated by global sections,
...
it has the form $\sO_C(D)$ for some effective divisor $D$ supported on the smooth locus of $

\end{proof}
For example suppose that $\sF$ is a coherent sheaf on an integral projective curve $C_0$ that is generated by its global sections. Choosing a maximal number $r$ of generically independent such sections, we get a short exact sequence
 $$
 0\to \sO_{C_0}^r \to \sF \to \sG \to 0
 $$
 where $\sG$ is  a coherent sheaf with finite support. Given such a sequence we define the rank of $\sF$ to be $r$
 and define the degree $\deg \sF$ of $\sF$ to be the length of the $\sO_{C_0}$-module $\sG$---these are well-defined integers.
 In this case
 the Riemann-Roch theorem (without duality) becomes
 $ \chi(\sF) = \deg \sF + r \chi(\sO_{C_0}),$
which follows at once from the additivity of $\chi$ on exact sequences. This formula may be taken as an alternate definition
of $\deg \sF$, or a proof that this number, defined as a length, is independent of the map $\sO_C^r\to \sF$.
Now suppose that $C$ is embedded in projective space by a very ample invertible sheaf $\sO_C(1)$. if $\sF$ is
any coherent sheaf then $\sF(n) = \sF\otimes_C\sO_C(n) $ is generated by global sections for large $n$,
so there is an exact sequence 
$$
 0\to \sO_{C_0}(-n)^r \to \sF \to \sG\otimes \sO_C(-n) \to 0,
 $$
and since $\sG$ has finite support, $\sG\otimes \sO_C(-n) \cong \sG$, so $\chi(\sF) = \deg \sF(n) + r \chi(\sO_{C_0}(-n))$.
Since $\sO(-n)$ can be represented as the ideal sheaf of a divisor $D$ supported on the nonsingular locus of $C$,
we can compute $\sO(-n)$ as in the smooth case, and 

Less trivially, we will explain in Chapter~\ref{linkageChapter} that there is is a sheaf $\omega_{C_0}$ called the \emph{canonical} or \emph{dualizing} sheaf of ${C_0}$ that is the analogue of the sheaf of differentials on a smooth curve, and in these terms we can rewrite
the formula above,  with duality,  as:
\begin{theorem}\label{RR for integral curves} If ${C_0}$ is a projective curve and $\sF$ is a coherent sheaf of rank $r$ and  degree $d$ then
$$ 
h^0(\sF) -h^0(\Hom_{C_0}(\sF, \omega_{C_0})) = \deg \sF + \rank(\sF)(1-p_a({C_0})).
$$
\end{theorem}
