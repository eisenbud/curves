...
If $C\subset \PP^r$ is a nondegenerate, reduced, irreducible curve, and $h$ is a linear form defining
the hyperplane $H$, then the natural
sequence
$$
0\to \sI_C \rTo^h \sI_C(1)\to \sI_{H\cap C}(1)\to 0.
$$
is exact, and the subscheme
$H\cap C$ spans $H$; that is, $H^0(\sI_{(H\cap C)/H}(1)) = 0$.
\end{proposition}
...
Geometrically: since $C$ is nondegenerate, reduced, and irreducible, every hyperplane meets $C$ in a finite subscheme $\Gamma := {H\cap C}$.
...
$h$ is a nonzerodivisor modulo $\sI_C$, and thus, setting $\Gamma = {H\cap C}$ we have
we have 
$$
\begin{aligned}
...
from which it follows that $H^1(\sI_{C/\PP^n}) = 0.$ From the long exact sequence in cohomology it follows that
 the restriction map $H^0(\sI_C(1))\to H^0(\sI_{\Gamma/H}(1))$ is surjective. Since
