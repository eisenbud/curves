...
In this Chapter we focus on the linear systems that exist on curves of genus 4,5, and 6, and what this says about maps to $\PP^r$. 

The Brill-Noether theorem of the preceding chapter answers many of our naive questions, but we won't invoke it in genus 4 or 5; instead, we'll be analyzing the linear systems on an ad-hoc basis, observing after the fact that our conclusions agree with the general Brill-Noether theorem. In genus 6, by contrast, we'll see for the first time a question we can't answer naively; in that analysis, we will have to invoke Brill-Noether for one key fact (though there are relatively ad-hoc arguments for this as well). \fix{get rid of the last phrase??}

Indeed, this reflects the historical origins of the subject. The Brill-Noether theorem was not originally conceived as a result of a chain of abstract logic; rather, it was simply an extrapolation of what had been established on an ad-hoc basis in genera $g \leq 6$. \fix{this needs justification, a reference}
...
\subsection{Trigonal curves of any genus} 
\fix{nearly all of this subsection is currently done in the scrolls chapter; maybe the only missing statement there is that the scroll must be smooth except in low genus. I think we are already referring to that chapter for the hyperelliptic case.}

\fix{ in the hyperelliptic case a good analysis of ALL special linear series is possible -- but maybe not yet in the book, perhaps it should be an exercise? and I thought that there was something similar for trigonal curves, though I have never seen it.}

The picture of trigonal curves in genus 6 is very much like that of trigonal curves of genus 5, and for that matter trigonal curves of any higher genus; rather than focus here on genus 6, we'll consider a trigonal curve of any genus $g \geq 4$. 

Suppose that $C \subset \PP^{g-1}$ is a trigonal curve of genus $g$, and let $\cD = \{D_\lambda\}_{\lambda \in \PP^1}$ be the $g^1_3$. By the geometric Riemann-Roch Theorem, the divisors $D_\lambda$ each span a line $L_\lambda$, and these lines sweep out a surface $S = \cup L_\lambda$ containing the canonical curve. By Corollary~\ref{hyperelliptic and trigonal}, this surface is a rational normal scroll whose homogeneous ideal is minimally
...
Every non-trigonal canonically embedded curve of genus 6 in $\PP^5$ is the complete intersection of a del Pezzo surface of degree 5 with a quadric.
\end{theorem}
