...
A special case of the Serre duality theorem on $\PP^{r}$ asserts that  there is an isomorphism $\eta: H^{r}(\omega_{\PP^{r}}) \cong k$, where $\omega_{\PP^{r}} = \cO_{\PP^{r}}(-r-1)$, the top exterior power of the cotangent bundle. Further, for any such isomorphism and any coherent sheaf $\cF$ on $\PP^{r}$ the natural map
$$
\cHom(\cF, \omega_{\PP^{r}}) \to \Hom(H^{r}(\cF), H^{r}(\omega_{\PP^{r}})) 
...
A scheme is said to be Gorenstein if it has a dualizing sheaf that is locally free---the canonical bundle. A more general form of Theorem~\ref{canonical is ext} says that if $\pi: X \to Y$ is a finite morphism of pure dimensional Gorenstein schemes, and $Y$ has a canonical bundle $\omega_{Y}$, then the canonical bundle of $X$ may be
represented as 
$$
...
One way in which syzygies are useful is in understanding \emph{linkage}. We are in the case of curves in $\PP^{3}$, so we give the definition in that case. See~\cite{PeskineSzpiro} for the general definitions and theorems.
...
connected by an even number of direct linkages, and similarly for \emph{odd linage}d
\end{definition}
