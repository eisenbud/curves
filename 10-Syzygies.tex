...
$$
is a free resolution. We have $\sI/\sI^{2} = \oplus_{i}\sO_{X}(-d_{i})$. Also, the last map of the Koszul
complex is the dual of the first one, twisted by $\sO_{Y}(\sum_{i}d_{i})$.
Thus 
 $\coker \phi_{c}^{\vee}$ may be canonically identified with $\wedge^{c}(\cI/\cI^{2})$.
and 
$$
\sExt^{c}_{\cO_{Y}}(\cO_{X}, \omega_{Y}) = \sHom(\coker \phi_{c}^{\vee}, \omega_{Y}).
$$
This computation suggests the bold idea that if $Y$ is a scheme with a dualizing module,
then the dualizing module of a closed subscheme
