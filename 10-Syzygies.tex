...
\begin{theorem}
 ...
 $F = S^n(-1), G= S^2$. If $M$ is 1-generic, then $r\geq n$, the ideal
...
$F_1 \subset F_1 \otimes K$. This suggests the (correct) generalization that the Theorem can be generalized
to case where the $F_i$ are not free, but are sufficiently ``like'' free modules.

\begin{fact}
Theorem~\ref{what makes a complex exact} is true in this form over any Cohen-Macaulay ring; for more general
...
then $EN(M)$ is acyclic. In this case the projective dimension
...
Finally, we must show that if  $\codim I_{2}(M) = n-1$ then $I_{2}(M)$ is prime. Since $M$ is 1-generic the entries of of the second row of $M$ are linearly independent, and since the dimension of the span of all the linear forms is at least $n+$, some element in the first row is outside the span of the the elements in the second. After a permutation of columns we may assume that $l_{1,1}, l_{2,1}, l_{2,2},\dots l_{2,n}$ are linearly independent.

 indetransformation we may assume that $l_{1,1}$ is not in any associated prime of $M$. (This is  called ``prime avoidance'' \cite[??]{E}, and is almost obvious in our case since $k$ is infinite and a subspace contained in a finite union of subspaces of a vector space over an infinite field must be contained in one of them.) Thus it suffices to show that $I_{2}(M)$ becomes prime after inverting
\end{proof}
\end{proof}
%
