...
In this section we study the syzygies of curves in projective space, with an emphasis on what they say about varieties (or schemes) containing the curve. For curves in $\PP^{3}$ we ask about the relationship of a curve to the 1-dimensional complete intersections containing it---this is the theory of \emph{linkage} (often called by its French name \emph{liaison}). Questions about abstract curves can be turned into questions about embedded curves via the canonical embedding. We will discuss what is known about the syzygies of canonical curves, and introduce Green's conjecture, which relates the Clifford index of an abstract curve to the syzygies of it canonical embedding. Along the way, we make explicit the syzygies of rational normal scrolls, given by the Eagon-Northcott complex.
...
%
%Once a curve is embedded in projective space, the first algebraic invariant we have looked at is it's Hilbert polynomial; but this simply recovers the genus and the degree. A finer invariant is the Hilbert function of the homogeneous coordinate ring, but for the canonical series this also recovers only the degree and genus. Still finer is the numerical data assocated to the minimal free resolution of the homogeneous coordinate ring of the curve. For example according to the conjecture of Mark Green (now verified in many special cases), this data determines the ``smallest'' base point free linear series on the curve, in a precise sense that we will describe.

\section{Syzygies and free resolutions} 

Let 
$R = R_{0} \oplus R_{1}\oplus\cdots$ be a positively graded ring, with $R_{0}$ a field,
 and let $M$ be a finitely generated graded $S$-module. An  \emph{$R$-free resolution} of $M$ is a sequence of graded free $R$ modules $F_{i}$ and homogeneous degree 0 maps $d_{i}$ of the form
$$
...
\end{tabular}}
\noindent
This means that there are 2 linear relations on the 3 generators of $I$ (they are in fact just the columns of the transpose of the matrix $M$).
...
is finitely generated, and $\depth M$ is the largest integer $\ell$ such that $H^{i}(\cO_{X}(t) = 0$ for all $0<i<ell-1$ and all $t$.
\end{theorem}
...
We first note that direct linkage is a symmetric relation:
\begin{lemma}
 If $G$ is a complete intersection of codimension 2, and I is an ideal of codimension 2 containing $G$, then $G:(G:I)$ is the unmixed part of $I$. In particular, if $I$ is unmixed and $J = G:I$ is directly linked to $I$, then $I = G:J$ is is directly linked to $J$.
\end{lemma}

\begin{proof}
 Since $G$ is a complete intersection, the associated primes of $G$ are all of codimension 2, and it follows that for any ideal $I$
\end{proof}
