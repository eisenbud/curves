...
Of course it is far from obvious that such a dualizing sheaf will exist on an arbitrary pure-dimensional scheme, and in general there is no such sheaf! However,  dualizing sheaves do exist on any scheme that is embeddable in a smooth scheme, and thus, in particular, they exist on any projective scheme.

To extend such computations to the general case of possibly non-reduced schemes, we must abandon the idea that the the canonical sheaf of $X$ must ``come from'' differential forms on $X$. For example, consider the ring $R = k[x,y,z]/x^{2}$ and the scheme $X = \Proj R$, a double line in $\PP^{2}$. Writing $d: \sO_{X}\to \Omega_{\sO_{X}/k}$ for the universal derivation, we have $0 = d(x^{2}) = 2xd(x)$ so (at least in characteristic $\neq 2$), $dx = 0$. Thus 
\def\red{{\rm red}}
$$
...
To understand where a general construction could come from, consider first the situation where $\iota: X\subset Y$ is a closed immersion of smooth varieties, and $X$ has dimension $d$ and codimension $c$ in $Y$.
In this case the conormal bundle of $X$ in $Y$ is by definition the sheaf
$\cI/\cI^{2}$, where $\cI$ is the ideal sheaf of $X$ in $Y$. At any point $p$ of $X$, the kernel of the map of local rings $\cO_{Y,p} \rTo^{\iota^{*}} \cO_{X,p}$ must be generated by a subset of a set of minimal generators of the maximal ideal $\gm_{Y,p}\subset \cO_{Y,p}$, and is thus a complete intersection. It follows that the right exact sequence
$$
\cI/cI^{2} \to \iota^{*}(\Omega_{X}) \to\Omega_{X} \to 0
$$
is exact on the left as well. All the terms are vector bundles on $X$, and thus the sequence
is locally split. It follows that 
$$
\wedge^{d}\Omega_{X} = \wedge^{c+d} \iota^{*}(\Omega_{Y})\otimes \wedge^{c}(\cI/\cI^{2})^{\vee} = \sHom(\wedge^{c}(\cI/\cI^{2}), \omega_{Y}).
$$ 

Grothendieck's idea for a general construction was to recognize that the expression
$$
\omega_{X} = \sHom(\wedge^{c}(\cI/\cI^{2}), \omega_{Y}),
$$
valid for a smooth variety $X\subset Y$ of codimension $c$, can be interpreted in that case as
$$
\omega_{X} = \sExt^{c}_{\cO_{Y}}(\cO_{X}, \omega_{Y}).
$$
To see this, recall that $X$ must be locally a complete intersection in $Y$. It is enough to check the equality locally, and the Ext in the formula is computed locally from the Koszul complex on a minimal generating set for the ideal $I$ of $X$ in $Y$. At a point $p\in X$ this complex has the form
$$
0\to \wedge^{c} \cO_{Y,{p}}^{c} \rTo^{\phi_{c}} \cdots \to \cO_{Y,p}^{c} \rTo^{\phi_{1}}\cO_{Y,p} \to\cO_{X,p} \to 0.
$$
...
This computation suggests the bold idea of that the dualizing module of any closed subscheme
$X\subset Y$ of pure codimension $c$ can be computed in terms of the canonical module of $Y$ by the formula
$$
...
This makes sense only given the web of theorems necessary to prove that the sheaf 
$\sExt_{Y}^{c}(\cO_{X}, \pi^{*}\omega_{Y})$ is independent of $\pi$; and given that it makes sense, we would like to know that such sheaves satisfy some form of Serre duality. For all this, see the book
\ref{Altman-Kleiman}.
...
Note that one possible choice of $\pi: X\to Y$ in the theorem would be a Noether normalization, that is, a finite map to $X \to \PP^{1}$. Another is the inclusion of $X$ into a complete intersection curve.
...
One way in which syzygies are useful is in understanding \emph{linkage}. We have already used the relation of \emph{direct linkage} in Chapter 8 \fix{make ref refs}. There, and in the classical literature, we used the notion in the case where the union of two (reduced) curves in $\PP^{3}$ without common components is a complete intersection, such as in the case of the union of a twisted cubic and one of its secant line, which together are the complete intersection of two quadrics. In this setting it is obvious that the relation is symmetric, and that the degrees of the two curves add up to the degree of the complete intersection, and we showed that the (arithmetic) genera of the two curves is related by the formula~\ref{***}. 

It is possible to extend the definition to a more general case, where the two curves may not be reduced or irreducible, and may have common components. 

We  give the definition in the case of curves in $\PP^{3}$, where the idea is most useful. See~\cite{PeskineSzpiro} for the general situation.
...
$$
p_{a}(X) -p_{a}(Y) =\frac{(d_{1}+d_{2})}{2} (\deg X -\deg Y)
$$
...
the associated primes of $J$ are precisely the associated primes of $I$ whose primary components do not contain $G$.
...
\item Under these hypotheses, the sum of the mulitiplicities of $R/I$ and $R/(G:I)$ is the multiplicity of $R/G$.
...
\begin{proof}[Proof of Lemma~\ref{double colon}]
 1) Suppose that $G = \bigcap Q_{i}$ is the primary decomposition of $G$. We have
$I:J = \bigcap (Q_{i}:J)$. If $P_{i}$ is the associated prime of $I$ that is the radical of $Q_{i}$, then it is easy to see from the definition that $(Q_{i}:J)$ is  the unit ideal if and only if $J\subset Q_{i}$ and is otherwise $P_{i}$-primary.
...
It is obvious that $I\subset G:(G:I)$, so it suffices to show that equality holds after localizing at an associated prime $P$ of $I$. Thus we may harmlessly assume that $R$ is local to begin with, and both $G$ and $I$ are primary to the maximal ideal $P$ of $R$. 
