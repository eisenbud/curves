...
\begin{proof} We first show that $r\geq n$. As noted above, to say that the $2\times n$ matrix of linear forms $M$ is 1-generic means that
the kernel of the corresponding map $ \phi: k^2\otimes k^n \to S_1$ contains no pure tensors. In the projective
space $\P^{2n-1} = \P(k^2\otimes k^n)$ the pure tensors form a variety isomorphic to $\P^1\times \P^{n-1}$, and thus of dimension $n$. Consequently the kernel of $\phi$ can have dimension at most $n-1$, whence the image of $\phi$ 
...
\begin{theorem}\label{what makes a complex exact}
...
For example, if $n=1$, the theorem simply says that a map $F_1\to F_0$ is a monomorphism iff it becomes a monomorphism after tensoring with the field of rational functions $K$, which follows from the flatness of
...
If $R$ is a regular local ring, and 
\end{theorem}
