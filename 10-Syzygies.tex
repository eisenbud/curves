...
$C\subset D$ is unmixed of dim 1. Define $C'$ by $\cI_{C'} = (\cI_D:\cI_C)$. 
By Lemma~\ref{double colon},
\begin{align*}
...
$$
 ...
 $$
 Equating the constant terms in the expression above
 gives
 $$
(*)\qquad 1 - \bigl(\frac{d_1d_2(d_1+d_2-4)}{2} +1\bigr) = 
1-p_a(C) +
\chi(\omega_{C'}(-d_1-d_2+4)).
$$
Now 
\begin{align*}
...
\end{align*}
Substituting this into (*) and using the fact that $\deg C + \deg C' = \deg D = d_{1}d_{2}$
we get 
$$
(\deg C' - \frac{\deg C+\deg C'}{2})(d_{1}+d_{2}-4)= p_{a}(C')-p_{a}(C);
$$
that is, 
$$
(\deg C' - \deg C)\frac{d_{1}+d_{2}-4}{2} = p_{a}(C')-p_{a}(C)
$$
as required.
...
TODO:
\begin{itemize}
 \item prove: linked curves have dual Rao modules; state converse.
\item Construction of curves with given Rao module: Bourbaki's theorem.
\item state minimal linkage, all curves derived.
\item state general curves of high genus are minimal
\end{itemize}


\begin{exercise} (Schwartau, Thesis) Let $C,C'$ be unmixed 1-dimensional subschemes of 
$\PP^{3}$. Choose hypersurfaces $F = 0$ containing $C$ and $F'=0$ containing $C'$
