...
\begin{figure}
\begin{center}
\centerline {\includegraphics[height=2in]{"Fig16.1.pdf"}}
\caption{The smallest scrolls: from the left, $S(0,1) \cong \PP^{2}$ \fix{mislabeled in the drawing}
...
If $a=r$ then $V(I)$ is a smooth, nondegenerate curve. In Chapter~\ref{SyzygiesChapter} we will construct the Eagon-Northcott complex $EN(M)$. By Theorem~\ref{EN}, the complex $EN(M)$ is a free resolution,
...
\begin{figure}
\begin{center}
\centerline {\includegraphics[height=2in]{"Fig16.2.pdf"}}
...
The term 
$H^{0}(\sO_{\PP^{1}}(q + (p-i)a_{1}+i a_{2}))$ is nonzero if and only if 
