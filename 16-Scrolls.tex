...
\item If  $C$ is a smooth curve of genus $g\geq 2$ lying on a singular scroll $S(0, d)\subset \PP^{d+1}$, then
$C$ is hyperelliptic and the scroll is the union of the secant lines to $C$ corresponding to the
hyperelliptic involution if and only if $C\sim 2H$ or $2H+F$ on $\widetilde S(0,d) = S(1,d+1)$ in which case
the degree of $C$ is $2d$ or $2d+1$ and the genus of $C$ is $d-1$ or $d$ respectively. 
\end{enumerate}
\end{corollary}
