...
\item If $a=n$ then $V(I)$ is a rational normal curve, and more generally the variety $V = V(I) \subset \PP^n$ has degree $a$ and codimension $a-1$.
\end{enumerate}
\end{theorem}
...
a proper subspace of $ \PP^{a_1+a_2+1}$, is $a_2$. Equivalently, if the scroll is defined from a multiplication
map $H^0(\sL_1)\otimes H^0(\sL_2) \to H^0(\sO_X(1))$ with invertible sheaves $\sL_i$ on a variety $X$, with 
$h^0(\sL_1) =2$, then $a_2$ is the maximal integer such that $\sL_1^{-a_2}\\sO_X(1)$ is effective.
\end{proposition}
...
The second characterization follows because the rulings of the scroll are the divisors of section of $\sL_1$.
\end{proof}
\fix{What if some of the linear series $V_1 = \langle 1,\sigma\rangle,V_2, V_3$ where the scroll comes
from $V_1\otimes V_2 \to V_3$ and $X\subset \PP(V_3)$ are not complete,
 What if if the scroll is higher-dimensional?
 I think this corresponds to
a ``maximal'' decomposition of the embedding linear series $V$ as a direct sum of sections of $V_2$ times
powers of $V_1$.  Make an exercise?}
...
\fix{Add Castelnuovo curves modulo the 2n+3 lemma}
