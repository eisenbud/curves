...
to the rational case. As a warm-up we examine two familiar examples: \fix{Joe, I tried to follow your mo, starting with the next two examples.
...
and let $\pi: X \to \PP^1$ be the natural projection. Set $\sL =   \sO_{\PP(\sE)}(1)$, the tautological 1-quoteint of $\pi^*(\sE)$.
...
More explicitly:
\begin{enumerate}
 \item If $C_1, C_2\subset X$ are the curves defined by the projections $\sE \to \sO_{\PP^1}(a_i)$, then $\phi(C_i)$
 is defined by the vanishing
...
In the proof that Castelnuovo curves lie on surfaces of minimal degree we invoked a result of Castelnuovo:

We showed in **** that $\geq 2n+1$ points in linearly general position impose $\geq 2n+1$ conditions on quadrics. 
\begin{enumerate}
 \item Show that
any number of points lying on a rational normal curve in $\PP^n$ impose only $2n+1$ conditions on quadrics. 
\item As a partial converse, prove the following theorem of Castelnuovo:
\begin{theorem}
  any set of $\geq 2n+3$ points that imposes only $2n+1$ conditions on quadrics
lies on a rational normal curve. 
\end{theorem}
A very classical proof is given in~\cite[***]{Griffiths-Harris} and a more modern on in~\cite[***]{Montreal}. Here is a hint toward
a third proof, using Theorem~\ref{}:
We may take the points to have coordinates given by the rows of the following matrix:
\begin{pmatrix}
 
\end{pmatrix}
Show that the $2\times 2$ minors of the matrix
\begin{pmatrix}
 
\end{pmatrix}
vanish on these points and that the matrix is 1-generic. 

\item 
\end{enumerate}

Show that $d\geq 2n+3$ points.
...
\fix{add an exercise as in DE's copy of "Principles" and perhaps a version of the proof based on Montreal}
