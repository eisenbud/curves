...
The following result was proven by Macaulay \cite[p.~424]{Macaulay1900}, who (correctly) considered it to be a version of the Riemann-Roch theorem. It is now usually referred to as the Cayley-Bacharach Theorem, after an
...
\cite{MR1376653} (where the result is incorrectly attributed to Bacharach) for generalizations and related conjectures. Here we will state and prove the version for a smooth curve.

\begin{theorem}[Cayley-Bacharach]\label{CB} Let $C$ be a smooth plane curve of degree $d$, suppose that
$D', D''$ are effective divisors on $C$ such that $E:=D+D' = C\cap C'$, the complete intersection of $C$
with a curve $C'$ of degree $d'$. The For any integer $0\leq k \leq d+d'-3-k$, the difference in the number of conditions imposed 
on forms of degree $k$ by $D'$ and $E$ is equal to the degree of $D''$ minus the
number of conditions it imposes on forms of degree $d+d'-3 -k$.
\end{theorem}
%\begin{proof}
% The number of conditions imposed by $E$ minus the number imposed by $D'$ on forms of degree $k$ is 
% $$
% N := = L(kH-E') - L((k-d)H)= L(kH-D') - L(kH-E) 
% $$
% 
% The failure of $D''$ to impose independent conditions on forms of degree
% $d+e-3-k$ is 
%\begin{align*}
% M:=  &\gamma'' - (L((n+d-3-k)H) -L((n+d-3-k)H-D''))\\
% =  &\gamma'' - L(K+(d-k)H) + L(K+(d-k)H - D'')).
%\end{align*}
% 
%Using the Riemann--Roch theorem and the fact that the degree of the divisor $kH$ is $kd$ we see that 
%\begin{align*}
% N &= kd-\gamma' - g +1 +L(K-kH+D') - \bigl(kd-\gamma - g +1 +L(K-kH+E)\bigr)\\
% &= \gamma''+L(K-kH+D') - L(K-kH+E)\\
% &=\gamma'' + L(K +(d-k)H - D'') - L(K-(d-k)H)\\
% &= M
%\end{align*}
%as required.
%\end{proof}
