...
Show that the vector space of sextic curves in $\PP^2$ vanishing doubly at 4 points \fix{lin gen pos??} has dimension
exactly 16.
\end{exercise}

\begin{exercise}(Tangential degeneracy~\cite{kaji-tangentialDegeneracy}) Show that the linear series in in Exercise~\ref{1,d-1 on quadric}
...
In any finite set o $\Gamma \subset \PP^2_\RR$ there are 3 non-colinear points unless $\Gamma$ is contained in a line.
\end{theorem}
