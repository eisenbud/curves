...
\begin{corollary}\label{CBM cor 2}
 Suppose that $C\subset \PP^2$ is a smooth plane curve of degree $d\geq 4$.
 
\begin{enumerate}
 \item If $\sV$ is a $g^1_e$ on $C$ with $e\leq d-1$ then $e = d-1$ and $\sV$
 corresponds to projection from a point of $C$.
 \item If $\sV$ is a $g^2_e$ on $C$ with $e\leq d$ then $e = d$. Furthermore,  the
...
$L$ meets $C$ in an additional point $s$. The line through $s$ and $o$ meets $C$ additionally in a point $r$,
and by Theorem~\ref{equiv on smooth plane curve} we have $r\sim p+q-o$, as in Figure~\ref{group law on cubic}.
Moreover the map $(p,q) \to r$ involves only solving linear equations, and is thus given by rational functions
in the coordinates of $p,q$.
...
\end{figure}
\fix{Silivo: let's show that $o$ is a flex by adding its flex tangent; and let's make the picture a little less symmetric:
the point $o$ is special for the embedding, but the point $p$ is not!}
