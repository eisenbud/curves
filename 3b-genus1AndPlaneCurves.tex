...
If $C$ is a reduced plane curve of degree $d$, then projection from
a point of $C$ defines a map of degree $e\leq d-1$ from $C$ to $\PP^{1}$.
For smooth plane curves the converse is also true:

\fix{ Exercise~\ref{gonality of smooth plane curve} in Ch 2 shows that there is no g^{1}_{e}$ of degree $e\leq \leq d-2$; uses any d-2 points impose
indep conditions on forms of degree $d-3$, the canonical class.}

\begin{proposition}
 If $C\subset \PP^{2}$ is a smooth plane curve of degree $d$, 
 and $\sV := (\sL, V)$ is a $g^{1}_{e}$ on $C$ with $e\leq d-1$,
 then $e=d-1$ and $\sV$ is the complete linear series $\sV -p$ for some $p\in C$.
\end{proposition}

\begin{proof}
Suppose that the divisor $D\subset C$ is the fiber of a map to $\PP^{1}$,
with $\deg D = e\leq d-1$.
\end{proof}
