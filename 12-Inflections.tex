...
\centerline{ \includegraphics[scale=.5]{"main/Fig12-1-threeCusps"}}
 \caption{A quartic plane curve with 3 cusps. \fix{Silvio: doesn't need to be blue.
...
\fix{Silvio: let's use color here}
\begin{figure}
\inprogress
\centerline {\includegraphics[height=2.8in]{"main/Fig12-2"}}
\caption{
$\Sigma_{1}(L) =$ Lines meeting $L$.
$\Sigma_{2}(p)$: Lines passing through $p$.
$\Sigma_{1,1}(\Lambda)$: Lines in the plane $\Lambda$.
As $L_{1}$ moves to meet $L_{2}$ at $p$ and therefore lie in a plane
$\Lambda$, the intersection
$\Sigma_{1}(L_{1})\cap \Sigma_{1}(L_{2})$ moves to
$\Sigma_{2}(p) \cup \Sigma_{\Lambda}$.
\fix{Silvio: I realize that this is in process, but here are some notes:
...
\end{fact}
Figure~\ref{Schubert cycles and intersection product in G(2,4)}\fix{missing figure?} illustrates the degeneration of $\Sigma_1\cdot \Sigma_1$ to $\Sigma_2 \cup \Sigma_{1,1}$.
