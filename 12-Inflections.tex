...
If $\cD$ is very ample, so that it may be viewed at the linear series cut on $C$ by hyperplanes for some embedding $C \subset \PP^r$, then $p$ is an inflection point if $a_r > r$; that is, if there is a hyperplane $H \subset \PP^r$ having contact of order $r+1$ or more with $C$ at $p$. For example, if $C \subset \PP^2$ is a smooth plane curve, this coincides with the classical notion of a flex point: $p \in C$ is inflectionary if the tangent line to $C$ at $p$ has contact of order 3 or more with $C$ at $p$.
...
A linear series on a smooth projective curve can have only finitely many inflection points; in fact the sum of the weights of all the inflection points depends only on the genus of the curve and the degree and dimension of the the linear series. The result is called the Pl\"ucker formula:
...
\end{theorem}
  ...
  The idea behind the argument is simple: we will show that the subgroup of $Aut(C)$ of automorphisms that fix each  of the finitely many Weierstrass points is either
...
Note, finally, that we have equality in case $h=0$ and $p=2$---that is, the hyperelliptic locus---and only in that case; all other components of the locus of curves with automorphisms have codimension strictly greater than $g-2$.
...
in the Chow ring $A^*(G)$ of the Grassmanian $G = G(\ell, e)$ (or the cohomology ring, which is the same as the Chow ring for Grassmannians) is independent
