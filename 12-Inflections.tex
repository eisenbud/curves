...
To simplify the notation, we will write $\ell := r+1, e:= d+1$. 

\goodbreak

\begin{theorem}\label{transversality of ramification}
The set of $\grd$s on $\PP^{1}$  with ramification sequence $\balpha = (\alpha_{0}, \dots, \alpha_{r})$ 
at a
...
A subspace $W$ of dimension $\ell = r+1$ has ramification sequence $\geq \alpha$ at $p$ if, 
$\dim W\cap V_{e-a_{\ell-i}}(p)\geq  i$  for  $i = 0,\dots, \ell$.
Such a condition is called a Schubert condition
on $W$, and we pause to describe the \emph{Schubert varieties} in $G(r+1,d+1) = \GG(r,d)$ that consist
...
Given a flag in $V$ and any  $\ell$-dimensional subspace $\Lambda \subset V$, we can derive the nested sequence of $e$ subspaces of $\Lambda$:
$$
...
$$
\Sigma_{\bbeta}({\cal V}) \; := \; \left\{ \Lambda \in G \mid \dim(\Lambda \cap V_{e-\ell+i-\beta_i}) \geq i \; \forall i \right\}
$$
...
%Thus the variety  $\Sigma_{\bbeta}({\cal V})$ is the set of $\ell $-planes $\Lambda$ for which the $i$-th jump in the sequence of intersections $\Lambda \cap V_{e-\ell} = 0$ occurs
...
Since this condition really involves only the single space $W = V_{e-\ell-\gamma+1}$, we sometimes 
 write it
as $\Sigma_\gamma(W)$. 
...
Using these ideas we  can rephrase Theorem~\ref{transversality of ramification} in terms of Schubert cycles,
...
If on the contrary 
$$
\Lambda \; \in \; \bigcap_{i=1}^\delta \Sigma_{{\bbeta}^i}({\cal V}^i),
$$
