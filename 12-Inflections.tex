...
If $\cD$ is very ample, so that it may be viewed at the linear series cut on $C$ by hyperplanes for some embedding $C \subset \PP^r$, then $p$ is an inflection point if $a_r > r$; that is, if there is a hyperplane $H \subset \PP^r$ having contact of order $r+1$ or more with $C$ at $p$. Thus, for example, if $C \subset \PP^2$ is a smooth plane curve, this coincides with the classical notion of a flex point: $p \in C$ is inflectionary if the tangent line to $C$ at $p$ has contact of order 3 or more with $C$ at $p$.
...
In Proposition~\ref{independence on rnc} we showed that if $C \subset \PP^r$ is a rational normal curve, and $\Gamma \subset C$ any proper subscheme of $C$ of degree $r+1$, then $\Gamma$ spans $\PP^r$.  Corollary~\ref{uninflected curves} implies that the
...
\input{footer}\begin{exercise}(Buchweitz)
