...
The projective space $\PP^{n}$ may be defined as $\Proj \Sym(\CC^{n+1})$, and comes equipped with a tautological line bundle $\sO_{\PP^{n}}(1)$, which is associated to the graded module $(\Sym \CC^{n+1})(1)$. In an exactly parallel way, we may make a projective space bundle $\PP(\sE)$ over a variety $B$ from a vector bundle $\sE$ on  $B$ 
by taking $\PP(\sE) = \Proj \Sym (\sE)$, and it too has a tautological line bundle associated to the graded sheaf
$(\Sym(\sE))(1)$. 

The map
$\sO_{B}  = (\Sym(\sE))_{0} \hookrightarrow \Sym(\sE)$ corresponds to the \emph{structure map}
$\pi: \PP(\sE) \to B$. If $\sE$ has rank $n+1$, then over any closed point $b\in B$ we have
$\sE_{b} \cong \CC^{n+1}$, and so the fiber $\pi^{-1}(b)$ is $\PP^{n}$. The restriction of 
$\sO_{\PP(\sE)}(1)$ to $\pi^{-1}(b)$ is $\PP^{n}$ is $\sO_{\PP^{n}}(1)$.

Conversely, if $\pi: X\to B$ is a map whose fibers are isomorphic to $\PP^{n}$, and if $X$ carries a line bundle $\sL$ whose restriction to each fiber of $\pi$ is $\sO_{\PP^{n}}(1)$, then $X\cong \PP(\sE)$,
where $\sE = \pi_{*}(\sL)$, and $\sL \cong \sO_{\PP(\sE)}(1)$.

Finally, the divisor class group of line bundles on $X$ is
$\Pic X \cong \Pic B \oplus \ZZ h$, where $h$ is the class of the tautological bundle, and
the map $\Pic B\to \Pic X$ is pull-back by $pi$.

It is clear from this description that the pair $(\PP(\sE), \sO_{\PP(\sE)}(1))$ determines $\sE$; but 
$\PP(\sE)$ alone determines $\sE$ only up to twisting with a line bundle on $B$. For example, 
if $\sE$ is itself a line bundle on $B$, then $\PP(\sE) \cong  B$, but $\sO_{\PP(\sE)}(1)) \cong \sE$.


 
A nonsingular rational normal scroll $X := S(a_{1}, \dots, a_{r})\in \PP^{N}$ is fibered over
$\PP^{1}$ by linear subspaces $L_{\lambda} \cong \PP^{r-1}$, where $\lambda\in \PP^{1}$. We may describe this fibration in the notation of Section~\ref{}, by saying that the 
fiber $L_{\lambda}$ is the span of set of points $\phi_{i}(\lambda)\in C_{a_{i}}$. More precisely, if $M$ is
...
projective spaces of dimension $r-1$.

Because the fibers of $\pi$ are embedded as linear spaces, the line bundle $\sO_{\PP^{N}}(1)$
restricts to a line bundle on each fiber $\PP^{r-1}$ of $\pi$ that is the  equal to $\sO_{\PP^{r-1}}(1)$.

Putting this together we have outlined the proof of the first statement of the following:

\begin{fact}
 If $X := S(a_{1},\dots a_{r})\subset \PP^{N}$ is a nonsingular rational normal scroll, then
 $X$ is isomorphic to a projective space bundle $\PP(\sE)$, and the restriction of $\sO_{\PP^{N}}(1)$ to $X$
 is $\sO_{\PP(\sE)}(1)$. Further, writing $\pi: X\to \PP^{1}$ for the structure map, we have
$$
\sE = \pi_{*}\sO_{X}(1) = \sO_{\PP^{1}}(a_{1}) \oplus \cdots \oplus \sO_{\PP^{1}}(a_{r}).
$$
\end{fact}

To check the last statement we reverse the process: Let 
$\sE =  \sO_{\PP^{1}}(a_{1}) \oplus \cdots \oplus \sO_{\PP^{1}}(a_{r})$
and consider the map  
$$
\phi:  \PP(\sE) \to \PP(H^{0}(\sE)) = \PP^{r-1+\sum a_{i}}.
$$
defined by the complete linear series associated to the line bundle $\sO_{\PP(\sE)}(1)$.
The variety $\PP(\sE)$ contains subvarieties corresponding to the rank 1 quotients 
$\sO_{\PP^{1}}(a_{i})$ of $\sE$, and the bundle $\sO_{\PP(\sE)}(1)$ restricts to
the bundle $\sO_{\PP(\sO_{\PP^{1}}(a_{i}))}(1)$. The sections of $\sO_{\PP^{1}}(a_{i})$ restrict
as well, and we see that the image of $\phi$ contains the rational normal curves
$C_{a_{i}}\subset \PP^{a_{i}}$, embedded in linearly disjoint spaces. Furthermore, each 
 fibers of $\PP(\sE)$ meets each $C_{a_{i}}$ in a single point. Thus the embedded
 variety $X\subset \PP^{N}$ is a scroll, as claimed.
 
 Finally, the Divisor class group of a scroll $X$ is given as follows:
 $$
 \Pic X = \ZZ H\oplus \ZZ F
 $$
where $F$ is a fiber of the structure map.
