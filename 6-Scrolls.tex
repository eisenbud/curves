...
\begin{theorem}Let $M$ be any 1-generic $2\times a$ matrix of linear forms on $\PP^n$, and
...
\fix{the following should be a ``cheerful fact'' --- do we have an environment?}

\begin{theorem}\label{Macaulay's Theorem} Every minimal prime over the ideal $I$ of $p\times p$ minors of a homogeneous $p\times q$ matrix forms has codimension $\leq q-p+1$. If $I$ has codimension $q-p+1$, then it is unmixed---that is, there are no embedded primes---and the $p\times p$ minors are linearly independent over the ground field.
\end{theorem}


\section{Scrolls as Projective Bundles (the deep and inscrutable name)}
