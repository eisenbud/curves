...
\begin{lemma} \label{size of 1-generic} If $k$ is an algebraically closed field, then there exist 1-generic $p\times q$ matrices of linear forms in $n+1$ variables over $k$ if and only if $n\geq p+q$;
...
\begin{proof}
If we think of a polynomial ring $k[z_0,\dots,z_n]$ as the symmetric algebra
of a vector space $V$ of rank $n+1$, then we may regard a $p\times q$ matrix of
linear forms $M$ as coming from a map $m: k^{p}\otimes k^{q}\to V$. The matrix is 1-generic
