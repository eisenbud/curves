...
The projective space $\PP_{\CC}^{n}$ may be defined as $\Proj \Sym_{\CC}(\CC^{n+1})$. The inclusion
of rings $\CC = \Sym_{\CC}(\CC^{n+1})_{0}\subset \Sym_{\CC}(\CC^{n+1})$ induces a structure map
$\pi: \PP_{\CC}^{n}\to \Spec \CC$. 

The variety $\PP_{\CC}^{n}$ comes equipped with a tautological line bundle $\sO_{\PP^{n}}(1)$, which is associated to the graded module $(\Sym_{\CC} \CC^{n+1})(1)$, and a tautological map 
$$
\CC^{n+1}\otimes \sO_{\PP^{n}} =\pi^{*}(\CC^{n+1}) \to \sO_{\PP^{n}}(1)
$$
that induces an isomorphism on global sections.

In an exactly parallel way, we may make a projective space bundle $\PP(\sE)$ over a variety $B$ from a vector bundle $\sE$ on  $B$ 
by taking $\PP(\sE) = \Proj \Sym_{\sO_{B}} (\sE)$.
...
In relating this to scrolls, we will, for simplicity of notation and concreteness, again concentrate on the case of 2-dimensional scrolls. The case of $r$-dimensional scrolls is exactly parallel, and we give some references. 
 
Let $X := S(a_{1}, a_{2})\subset \PP_{\CC}^{N}$ be a nonsingular scroll of degree $a=a_{1}+a_{2}$ and $N = a_{1}+a_{2}+1$. The variety $X$ is fibered over
$B:=\PP_{C}^{1}$ with fibers being the lines joining the corresponding points of the directrices $C_{a_{1}}$ and 
$C_{a_{2}}$. More precisely, if $M$ is a
...
then the map 
$$
\sO_{\PP_{\CC}^{N}}^{a}(-1)\to \sO_{\PP_{\CC}^{N}}^{2}
$$
...
$$
\biggl(\sO_{\PP(\sE)}(1), H^{0}(\sO_{\PP(\sE)}(1) = H^{0}(\sO_{\PP^{1}}(a_{1}) \oplus \sO_{\PP^{1}}(a_{2})\biggr).
$$
Because $\sE$ is generated by global sections, this linear series is base point free and thus defines
a morphism
$$
\phi:  \PP(\sE) \to \PP(H^{0}(\sE)) = \PP^{N}.
$$
