...
\begin{theorem}\label{where are the curves?}
 Let $X = S(a_{1}, a_{2})$ be a scroll of dimension 2 with $a_{1}\leq a_{2}$, and let $F,H$ denote the class of the ruling and the hyperplane section, respectively. There are smooth curves in the class $D = pH+qF$ if and only if one of the following holds:
\begin{enumerate}
\item $D~F$; that is, $p=0, q=1$; or
 \item $D~C_{a_{1}}$; that is, $p=1, q=-a_{2}$; or
 \item $p\geq 1$ and $D\cdot C_{a_{1}}\geq 0$ and $D\cdot C_{a_{2}}>0$; that is, $q\geq 1-pa_{2}.$
\end{enumerate}
\end{theorem}

This result is based on the following computation of the global sections of line bundles:

\begin{theorem}\label{global sections}
Suppose that $D$ is a divisor on the scroll $X = S(a_{1}, a_{2})$ with $a_{1}\leq a_{2}$. If $D \sim pH+qF$, then 
$$
H^{0}(\sO_{X}(D)) = 
\sum_{0\leq i\leq p}H^{0}\bigl(\sO_{\PP^{1}}(q + (p-i)a_{1}+i a_{2})\bigr).
$$
Thus 
$$
h^{0}(\sO_{X}(D)) = 
...
$$
Moreover,
$|D|$ is base point free iff $p\geq 0$ and $q\geq -pa_{2}$.
\end{theorem}
