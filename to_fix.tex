...
\item{1-LinearSeries}
\fix{make sure we don't assume that $C$ is a smooth curve without saying so!}
A famous result asserted by Franchetta and proved in \cite{****} is that the canonical sheaf (and its powers) are the \emph{only} sheaves that can be chosen uniformly among all, or even almost all, smooth curves. For a more precise statement, see \cite{****}.
\fix{unresolved citations}


\item{2-CurvesOfGenus0And1} 
 \fix{DE wrote to Iarrobino 8/13 to ask whether he knows about the ideal of a general rational curve.}
Needs a chapter reference to Fulton-Harris

\item{3-Jacobians}
Clean

\item{4-Curvesofgenus2and3}
one fix: number of bitangents to a curve of degree d

\item{Moduli}
consider moving early -- after genus 23
 \fix{add references)} for $M_g $general type
 
 one can show that the family
 covers a surface birational to a product of $C$ and some other curve 
 \fix{put in an argument,
...
\fix{Severi section: this material is partly repeated in the plane curves chapter. Especially if this chapter comes early, we could leave it here, and 
prune the plane curve treatment -- probably the right choice.}

dimension of the Severi variety: \fix{is it so easy to show that the nodes impose independent conditions? I don't see how a "local" computation
can do this.}


\item{5-BrillNoether}
\fix{Add chart from 3264 comparing the Clifford, Castelnuovo and Brill-Noether bounds}
\fix{reconcile nodal models with the exercise currently right after $g+3$ in the Jacobians chapter}
\fix{add references, attributions, etc.} to the omnibus B-N theorem
\fix{let's say more about the history here, or at the end of this section} history of BN
\fix{give good reference for "arithmetically Cohen-Macaulay"}
\fix{reference for Castelnuovo's $2n+3$ -- or just prove it!}
several missing Theorem references

\item{6-Curvesofgenus45}
 \fix{this needs justification, a reference} justification for an historical "fact"
 Since genus 45 now comes AFTER B-N its odd not to use BN --- or at least draw the comparison as we go. This needs to be
 rewrittent if the order of chapters stays. Similarly,
 \fix{we already said in Ch 1 that the canonical model is always ACM. Let's not be coy...}
\fix{add pictures illustrating some of the possibilities above.}--possibilities for the plane models. Joe draw pictures?
\fix{ Exercise~\ref{many quadrics} in Chapter~\ref{ScrollsChapter} shows that a surface on 3 quadrics in $\PP^4$ has minimal degree. Can we show it's a scroll without invoking the classification?}

\fix{precise reference. fairly exhaustive??} One can go further and ask about the geometry of the plane curve $B$ and how it relates to the geometry of $C$, and a fairly exhaustive list of possibilities is given in \cite{ACGH}. 

\fix{why is it clear that there are no others?}--about components of $W^1_4$ in genus 5 -- near the end of the Ch.
...
Do something about the automorphism section.  possibly \fix{add the Prop that a general curve has no automorphisms, by dimension count}
