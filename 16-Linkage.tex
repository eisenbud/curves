...
\blue{Max Noether and Georges-Henri Halphen}
\marginpar{dropped ``in'' after ``received'', ok?}
\index{Noether, Max}%
...
work of
\blue{Clebsch, Brill, Noether and Macaulay}
toward
...
and
\redden{a version of}
\marginpar{original had ``to'' in place of the red expression, but that didn't fit the sentence. Is this what you mean?}
...
\blue{complete intersection}
\index{complete intersection}%
of surfaces, then the degrees and genera
...
\marginpar{\vskip-20pt \hskip-10pt $C\to C_1$; dropped unused $c,d$}
whose union is the complete
\redden{intersection}
\marginpar{singular, OK?}
of two surfaces $S,T$
\redden{of degrees $s,t$,}
with $S$ smooth.
Then $ \deg C_1+\deg C_2 = st$ and
\marginpar{rearranged for better layout}
$$
...
\redden{is zero,}
\marginpar{or else keep the = and drop ``the multiplier''}
and indeed the line
...
\begin{proof}
\redden{The sum of degrees of $C_1$ and $C_2$ is the degree of $ C_1\cup C_2=S\cap T $, or $st$ by B\'ezout's theorem.}

By the
\blue{adjunction formula}
\index{adjunction formula}%
in $\PP^3$ the
\blue{canonical divisor}
of $S$ has
...
$$
Subtracting,
\marginpar{dropped repeated ``we get''}
$$
...
.
$$
\redden{Combining the last two displays yields}
the second formula
of the theorem.
\end{proof}
...
To say that the union of distinct reduced irreducible curves $C\cup C'$
is a complete intersection $X$ means that
the ideal $I_X $
\redden{equals}
$I_C\cap I_{C'}$. Since
\redden{the latter}
...
$(I_X:I_C) = I_{C'}$.
\marginpar{dropped ``It turns out that''}
This
relationship
\redden{underlies}
...
\marginpar{changed quotes to italics}
\index{rope}%
\textit{rope}\emdash that is, the subscheme $C \subset
\PP^3$ defined by the square of the ideal $\cI_{L/\PP^3}$ of a line $L
\subset \PP^3$\emdash is linked to the reduced line $L$ in the complete
intersection of two quadrics, as
\redden{discussed}
in Exercise~\ref{line and rope}. (This makes sense, since a rope is a
...
that $I_2 = (I_{X}:I_1)$,
\redden{too,}
is unmixed of codimension 2,
\redden{and therefore}
saturated.
...
\blue{reflexive.}
Since
\marginpar{undisplayed}
$I_{C_2}R= \ann_{R}(I_{C_1}R) \cong \Hom_R(R/I_{C_1}R, R)$,
the proposition follows.
\marginpar{\vskip 3pt \leftskip-50pt \hskip-250pt removed what appeared to be a restatement of Def 16.3 -- sorry if I missed some subtlety.  The original is commented out, not deleted}
\end{proof}

% We define the equivalence relation of \emph{linkage} among purely
% 1-dimensional subschemes of $\PP^{3}$ as the
% closure of the relation of direct linkage, and we say that two such
% subschemes are \emph{evenly linked}
...
The degrees and
\blue{arithmetic genera}
\marginpar{dropped parentheses}
\index{arithmetic genus}%
of directly linked schemes are related \null exactly as in the
\redden{pilot case of Section~\ref{SLinkage}.}
...
In Chapter~\ref{RiemannRochChapter} we claimed that the
\marginpar{dropped ``Recall that'' and quotation marks}
...
\redden{Let $C$ be}
a purely \1-dimensional projective scheme over $\CC$.
\begin{enumerate}

\item $\sHom_C(\omega_C, \omega_C) = \sO_C$; thus if $C$ is integral
\marginpar{replaced ``and'' by semicolon to avoid running to a new line}
...
$$
In particular,
\marginpar{undisplayed for better layout}
...
\marginpar{it's $\to$ its\\[2pt]placed last sentence in parentheses}
its double dual as a $B$-module;
\redden{this completes the argument}.
...
Dualizing sheaves do exist on any projective Cohen--Macaulay scheme. We
have already seen constructions
in three
\marginpar{dropped ``different''}
cases:
...
\marginparhere{\redden{changed comma to period (or did stuff drop out?)}}
$$
...
coherent sheaves!
\marginpar{dropped ``However,'' (``does'' does the job)}
An adjoint functor
...
\blue{locally a complete intersection.}
...
\marginpar{combined ``See\dots'' with paragraph}
...
\marginpar{\vskip-40pt suppressed subsection header redundant with proof header}
...
\redden{we have}
\marginpar{to avoid number and formula abutting}
$\chi(\omega_C(m)) = -\chi(\sO_C(-m))$. It follows that the leading
coefficient of the Hilbert polynomial of $\omega_C$
\marginpar{\redden{removed comma below ``is'' (or did stuff drop out?)}}
is
...
$$
whence
\marginpar{\vskip10pt suppressed parentheses in numerator}
$$
...
\marginpar{dropped ``The following result\dots Ferrand'' (repeated below)}
...
\redden{A more general form of this result appears as Proposition 2.5 in}
\cite{MR364271},
\redden{with an attribution to}
\blue{Daniel Ferrand.}
...
Let $X$ be the
\blue{complete intersection of two hypersurfaces,} 
of degrees
...
true: the
\marginpar{\redden{Has H-R module been defined?}}
\blue{Hartshorne--Rao modules,}
...
\marginpar{moved this sentence into CF to avoid awkward double numbering}
hypothesis is largely unnecessary); here is a 
\redden{sampling:}
\begin{theorem}
Let $S = \CC[x_0, \dots, x_3]$ be the homogeneous coordinate ring of
\marginpar{removed theorem environment to avoid double numbering}
...
\label{LR property}
Let $M = D(C_0)$
\redden{the Hartshorne--Rao}
\marginpar{dropped repeated ``the'', hyphenated for consistency}
invariant of a purely
\1-dimensional subscheme of $\PP^3` `$, and suppose that
\marginpar{\redden{did you mean to index ``minimal'' by itself or ``minimal H-R invariant?}}
...
that of $C_0$; that is, we need a formula for the adjoint ideal of any
curve singularity.
...
\marginpar{reworked for better flow. Dropped (for plane curves) in connection with Gorenstein's statement, since the statement in the theorem is for plain curves}
\redden{we can think of}
$\delta(C_0)$ as the number of nodes equivalent to the
singularities of $C_0$. 	
\redden{The formula for $\delta(C_0)$ in} 
...
% 
\blue{Hyman Bass}
\marginpar{reworded; ``see [Bass...] at the end'' was odd\\[2pt]dropped ``the reason''}
\index{Bass, Hyman}%
\citeyear{Bass}
\redden{explains that this is why}
\index{Grothendieck, Alexander}%
\blue{Grothendieck}
named Cohen--Macaulay rings that have
\blue{cyclic canonical modules}
\index{canonical modules!cyclic}%
\index{cyclic canonical module}%
\redden{after Gorenstein.}

This condition is always satisfied for singularities of curves in the
\marginpar{\redden{opened new paragraph at ``This condition'', but it's not clear to me what condition is meant, nor could I find the intended passage in Chapter 15.  Maybe the whole sentence ``The formula\dots after Gorenstein'' should be moved to the footnote, with the current footnote in parentheses.}}
plane, 
\redden{a fact we}
exploited in Chapter~\ref{PlaneCurvesChapter}.
See Example~\ref{nongorenstein} for a singularity that is not locally
planar, and behaves differently.
...
\index{conductor}%
$\mathfrak f_{C/C_0}$
\marginpar{dropped ``of $C/C_0$, which is defined to be'' and parenthesized}
...
resolution of $\sO_C$ of the form
$$
0\to \sO_{\PP^2}(-d) \ruto{\ F} \sO_{\PP^2} \to \sO_C \to 0
$$
and 
\redden{so}
\marginpar{to allow line break}
$\omega_C \cong \sExt^1_{\PP^2}(\sO_{C_0}, \omega_{\PP^2})$
is the cokernel of the map $\sO_{\PP^2}(-3) \to \sO_{\PP^2}(d-\nobreak3)$ 
\redden{of}
\marginpar{``of'' avoids ``by'' echo}
multiplication by $F$. Thus
$\omega_{C_0}$ is locally cyclic, and since 
$\omega_{C_0}/\omega_C$ 
\redden{has finite support,} 
this module is
globally cyclic, so
...
\marginpar{\redden{took $gTr$ to be $g$ times trace, right?}}
...
We showed earlier that the adjoint ideal of a planar triple point is
the square of the ideal of the point. Consider now
the germ of a spatial triple point singularity $R_0 \colonequals
...
The ring $R$ is also finite over $S \colonequals k\[x,y,z\]$, and is
...
Using 
\marginpar{\vskip-20pt\hskip-20pt dropped ``for singular curves''\\[5pt] For a blockbuster result this discussion was a bit disorganized. I hope I got it right. The reference to the first Chern class didn't seem to be part of the logic but it fit nicely with the idea that one can come at the degree from different directions, so I merged it with the longer CF to make the section less choppy.}
\blue{dualizing sheaves} 
\index{dualizing sheaf!for singular curves}%
we can state a 
\redden{more}
\blue{general version of the Riemann--Roch theorem},
\redden{applicable to any reduced, irreducible projective curve and any coherent sheaf thereon}.
...
We first need
\redden{to extend the notion of}
degree of a sheaf:

\begin{definition}
If $\sF$ is a sheaf of generic rank $r$ on a projective reduced and
irreducible curve $C$ over $\CC$ we define the 
\index{degree!of a sheaf|defi}%
\emph{degree}
of $\sF$ 
\redden{as $\deg \sF$}
\unskip ${}\colonequals\chi(\sF) -r\chi(\sO_C)$.
\redden{Keeping in mind the definition of the arithmetic genus, we can write}
\begin{equation}
...
This 
\redden{formula}
\marginpar{I'm guessing what you meant by ``tautology''; it didn't seem to be one in the strict sense}
would be
\redden{pointless}
if there were no other way to compute $\deg \sF$, but
\redden{that is not the case:}

\begin{fact} 
The degree of $\sF$ 
\redden{coincides with that of a certain}
divisor class, 
the 
...
Using 
\redden{these facts and}
the dualizing property of $\omega_C$ we can 
\redden{reexpress the equality}
\eqref{general RR without duality} 
\redden{in the definition of $\deg\sF$ as follows:}
...
(and necessarily 
\marginparhere{moved the parenthetic phrase}
...
\begin{exercise}[\redden{linkage} addition \cite{Schwartau}]
...
\blue{saturated ideal of codimension 2}
...
\redden{Proposition~\ref{similarities}(1).} 
...
to a line by the 
\blue{complete intersection of two quadrics.}
\index{complete intersection!of two quadrics}%
