...
\redden{(a more complete exposition can be found in \cite[Part III]{Eisenbud1995}).}
We
...
\redden{This is followed in Section \ref{Greenconj} by an introductory account of} 
Green's conjecture.
...
\marginpar{dropped years in Fitting index entry (not given for others)}
\index{ideal!Fitting}%
\index{Fitting ideal}%
\index{Fitting, Hans}%
...
\redden{Two special cases of $EN(\phi)$ are of interest in their own right: when}
$\rank G = 1$ and when
...
\redden{the start} 
\marginpar{save a line to fit a table on a subsequent page and so avoid the underfull pages you mentioned}
of a long series of
...
\marginpar{dropped ``various'' to allow line break} 
