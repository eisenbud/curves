...
\marginpar{dropped ``Remark'' heading (in part to avoid breaking Nakayama's name}
...
\index{Cayley, David}%
\index{Hilbert, David}%
\marginpar{indexed Cayley and Hilbert}
as examples,
...
and
\marginpar{split up ``theorems'' (but let ``Hilbert's'' be shared)}
syzygy theorem
...
\marginpar{removed Gantmacher citation, which is given later for the
same result}
...
\marginpar{\vskip-50pt added $\phi_a=$ for later reference; also made entries 2 and 3 on last two rows 0 instead of dots, right?}
of $M$,
and thus after a change of bases and variables the matrix $N$ is
transformed into the direct sum of these matrices.
Translating this
\marginpar{dropped ``form'' (to allow line break and avoid echo)}
...
Suppose that
$F$ is a minimal free resolution of a module $M$ as illustrated in the
...
is convenient to tabulate the numbers so that $\beta_{i,j}$ is in the
$i$-th column and $(j-i)$-th row:
\marginpar{\vskip10pt replaced $j\backslash i$ idiom on header row, ok?}
$$
\small
...
where 
\redden{a dash}
\marginpar{I move this from a later Betti table to where the dash is first used.  Also I made the dashes shorter than a minus sign}
represents 0.  
\end{example}
...
Recall that the grade of an ideal is the length of a maximal regular
...
for all $i$,
\marginpar{moved ``for all $i$''\\[2pt]reformatted for better layout}
$$
...
free modules
\redden{$\phi:F\to G$}
\marginpar{or does it have be written with a left arrow?}
is injective
...
The length of
\marginpar{dropped ``any'' and
\llap{replaced sentence }fragment by colon}
...
\redden{over $S \colonequals  \CC[x_0, \dots x_r]$,}
...
\index{Ext@$Ext_S^m$}%
\marginpar{added square as you do elsewhere}
\qed
\end{theorem}
...
To go further, we can 
\redden{use}
\marginpar{to allow line break}
the invertible sheaves $\sL$ on $C$
with $h^0(\sL)=2$ and $h^1(\sL)\geq 2$.
The existence of such a series guarantees that the ideal of $C$ contains
the ideal of $2\times 2$ minors of the $2\times h^1(\sL)$
matrix
...
\marginpar{used word ``Case'' and underline instead of level-4 header}
\smallbreak

In this subsection we let $\phi:F = R^{f}\to R$ be a homomorphism from
a free module to a ring $R$, and we
\marginpar{dropped ``we will'' (``we'' was repeated)}
...
\marginpar{changed quotes to italics, ok?}
...
\marginpar{image $\to$ im (your usage in chapters 6 and 17)}
...
\redden{is given by the downward arrows in the diagram}
\marginpar{transposed, because it took me a while to understand the original.  Hope I got it right.}
$$
\xymatrix@C=1pt{
...
\redden{$\phi_{a}$ given on page~\pageref{phia}}
\marginpar{and removed repeated matrix}
%$$
...
\marginpar{used def environment}
$f\colonequals \rank F\geq  g\colonequals  \rank G$, then the
\redden{Eagon--Northcott complex of $\phi$}
\index{Eagon--Northcott complex}%
is the complex of free $S$-modules
\marginpar{changed $:=$ to colon+space as in previous occurrences.\\[3pt]\redden{should $EN(M)$ be $EN(\phi)$?}}
\begin{multline*}
EN(M): \quad
...
\leftarrow 0,
\end{multline*}
\redden{which has the following properties:}
\begin{enumerate}
...
\marginpar{\vskip10pt removed ``3'' from header row}
$$
\small
\begin{tabular}{r|@{\hskip20pt}ccccc}
\downstrut
...
\marginpar{moved to allow line break}
\redden{Obviously}
the
...
\marginpar{\vskip8pt dropped the subscript $j$ in the eqno since the parametrization is not used}
\begin{equation}
...
\redden{As before a dash indicates a place that is}
definitely 0; 
\redden{asterisk indicate some that are}
definitely
nonzero. The entries of the rows marked 0 and 1 are 
\redden{greater than or equal to}
the
...
\marginpar{reworded to avoid repeating of names in cites and quirkiness}
thesis 
\redden{(reworked as \cite{Schreyer-canonical}),} 
...
genus 9 by 
\redden{Michael Sagraloff} 
in his Saarbr\"ucken thesis \citeyear{Sagraloff}  
\redden{under Schreyer's supervision}.
...
\blue{acyclic}
\marginpar{dropped ``the complex'' and redefinition of acyclic}
%(that is, $H_i(\FF) = 0$ for all $i>0$) 
\index{acyclic}%
if and only~if
$
\codim X_i \geq i
...
\begin{exercise}
\redden{Let $q:F\to G$, with $F = S^2(-1)$ and $G = S^2$,}
\marginpar{reworded for better layout}
\redden{be described by the matrix $\bigl(` `\begin{smallmatrix} x_0&x_1\\ x_2&x_3 \end{smallmatrix}` `\bigr)@$, and let $Q\subset \PP^3$ be the quadric defined by the determinant of the this matrix.}
\begin{enumerate}
...
$I_{\rank G}(\phi)$.
\item 
\marginpar{dropped unused notation $r$ and once-used $I(\phi)$}
