...
we have seen that for every  curve $C$ of genus $g\geq 3$ that is not hyperelliptic,
$
...
$\beta_{1,2} =  \tbinom{g-2}{2}$ for every canonical curve.

However, $\beta_{2,2}$, the number of cubic generators required, may vary, and reflects the geometry of the curve.  We have seen in Chapter ***
...
Betti table of the homogeneous coordinate ring $S_{C}$ is
$$
\small
\begin{tabular}{r|@{\hskip20pt}ccccc}
\downstrut
...
\end{tabular}
$$
whereas if $C$ is trigonal then 
 the ideal of $C$ requires two cubic generators in addition to three quadrics. 
 
Since the Hilbert functions are the same in the two cases, the number of 
linear relations on the quadrics, must increase from 0 to 2. Since the homogeneous
...
the Veronese surface in $\PP^{5}$. By Theorem *** the Veronese surface is contained
...
then the resolution of $S_{C}$ has Betti table
$$
\small
\begin{tabular}{r|@{\hskip20pt}ccccc}
\downstrut
...
\end{tabular}
$$
whereas if $C$ is either trigonal or isomorphic to a plane quintic, the Betti table is
$$
\small
\begin{tabular}{r|@{\hskip20pt}ccccc}
\downstrut
