...
introduction to Green's conjecture.
...
Any Koszul complexes $K$ is \emph{symmetric} in the sense that 
...
he corresponding family $N$ of $b\times c$ scalar matrices over $\PP^{a-1}$
...
\item The Koszul complex that resolves the homogeneous coordinate ring $S/(Q,
\index{Koszul complex}%
...
The conclusion thatl the grade of $I(\phi_{r+2})$ is at least $r+2$ can
...
\index{Green!Mark}%
 we begin at the beginning of the resolution, with 
...
If $C$ is a smooth canonical curve of genus $g$ over a field of characteristic 0,
 and $R_{C}$ is the
\index{Green's conjecture}%
...
Perhaps the most definitive of the various partial results related to Green's conjecture
...
Let $g = 2c+1$ be an odd integer. The curves of Clifford index $<c$ form an effective
...
Hint: Use the Riemann-Roch theorem.
\end{exercise}
...
maps, and 
Imitate the proof of Theorem~\ref{ENgeneral} to prove
...
Hint: Use the formula for the Hilbert function of the canonical curve.
\end{exercise}
...
Hint: To show that the rank of the syzygy matrix is $n-1$, tensor with
