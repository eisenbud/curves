...
%Canonical map to $\PP^1$. Embedding in $\PP^3$ as $(2,3)$ on a quadric, via any degree 5 invertible sheaf. Ideal is 1 quadric, 2 cubics.
...
%
%As with curves of genus 1, there are no nontrivial linear system of degree 0 or 1 on a curve of genus 2; the first positive-dimensional linear system occurs in degree 2. Unlike the case of genus 1, however, this system is unique: by Riemann-Roch, if $D$ is any divisor of degree 2 on a curve $C$ of genus 2, we have
%$$
...
%
%The canonical system gives a map $\phi_K : C \to \PP^1$ expressing $C$ as a double cover of $\PP^1$; as in the case of genus 1, this means we can realize $C$ as the smooth projective compactification of the affine curve given by
%$$
...
%
%\begin{theorem}
% Let $C$ be a smooth (is reduced, irreducible enough?) curve of arithmetic genus $g$, and let $\cL$ be an invertible sheaf on $C$ of degree $\geq 2g+1$. The image of 
% $C$ under the complete linear system $|\cL|$ is projectively normal ( when $C$ is singular, aritmetically Cohen-Macaulay).
%\end{theorem}
%
%\begin{proof}
% The invertible sheaf $\cL$ is very ample by \ref{?}. Thus it suffices We must show that the multiplication map $H^0(\sL)\otimes H^0(\cL^{m}) \to H^0(\cL^{m+1})$ is surjective for all $m\geq 1$.
