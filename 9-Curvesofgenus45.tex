...
In this chapter we
\redden{study}
\marginpar{avoid repetition in same sentence}
...
\marginpar{dropped ``we have'' (avoid repetition)}%
...
\index{P^3@$\PP^3$}
\blue{complete intersection of a quadric $Q = V(F)$ and a cubic surface}
...
\redden{each case.}
\marginpar{combined related figures to get them closer to where mentioned}
...
\redden{Putting together}
\marginpar{\leftskip-1.25 in dropped repeated definition of trigonal. But\redden{don't you need to account for $g=0$ as well?}}
...
\marginpar{dropped comma after ``then''}
...
\marginpar{dropped ``will'' to allow line break}
...
\marginpar{\vskip -140pt \leftskip-100pt See if you like this proposed new figure. Is the caption too verbose?}
...
\blue{3-sheeted cover}
\index{3-sheeted cover!of $\PP\sp 1$}
of $\PP^1$?
\redden{Equivalently,}
\marginpar{line break}
does $C$ have a
\blue{$g^1_3$?}
\index{g@$g^1_3$}
\item Is $C$ expressible as a
\blue{4-sheeted cover!of $\PP\sp 1$}
of $\PP^1$?
...
\subsection*{First case: the intersection of the quadrics is a curve}
%\label{nontrigonal genus 5}

\redden{We first assume that}
...
points
\marginparhere{\vskip -100pt David: the points on the middle line are red because my notes from the meeting with you and Joe said you'd asked me to distinguish one line. But I may have misunderstood.  Will the figure be ok if I just change the red dots into black? (The surface can now be imagined to be twisted, but I guess it could still be seen as flat.  Not sure what to do about this.)}
...
$\Gamma$ imposes
\blue{independent conditions on quadrics,}
...
%\index{independent conditions!on degree $d$ forms, by up to $2d+1$ points in plane}
\marginpar{\redden{the string from $2d+1$ through $d$ was highlighted --- I need help choosing one or more meaningful index entries}}
...
\blue{hypersurface of degree 5}
\index{quintic!hypersurface}%
in $\PP^{14}$, called the
\blue{\emph{discriminant}}
\index{discriminant}%
\index{Bertini's theorem}%
hypersurface. By
\blue{Bertini's theorem,}
...
\redden{one or two}
\marginpar{\hskip-1in spelled out numbers since next to a formula}
...
\blue{complete intersection of three quadrics}
\index{complete intersection!of three quadrics}%
...
\blue{Fulton's}
\blue{``elementary B\'ezout theorem'':}
...
\marginpar{added quotes (or italics instead?)}%
...
\blue{trigonal canonical curve;}
\index{trigonal!canonical curve}%
\marginpar{replaced parens by semicolon}
see \cite{MR453723}.
\index{genus 5!canonical curves|)}
\end{fact}
...
\blue{curve of genus 4,}
\index{genus 4!curve}%
and $|D|$ a $g^1_3$ on $C$. Show that the following are equivalent:
\begin{enumerate}
\item $D \sim K-D$.
\marginpar{repunctuated as full sentences}
...
nondegenerate
\blue{cubic surface $S \subset \PP^4` `$.}
