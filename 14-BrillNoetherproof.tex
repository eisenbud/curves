...
\marginpar{dropped ``in'' and repeated name}
...
\marginpar{$G\to\GG$ (here and below)}
...
\redden{discrete valuation ring.}
...
\marginpar{is r.P.v. being defined here?}
...
$\cP$ 
\marginpar{unindented to allow line break}
...
\redden{by Kleiman and Laksov and Kempf}
...
\marginpar{moved cite to first line of lemma}

\begin{npt}
\begin{lemma}[{{\cite[Proposition 6.5.2]{MR2223408}}}]
\label{specialization to cuspidal curve}
Let 
\marginpar{moved ``in $\PP^1$'' to avoid gratuitous break at $\in$}
$p_1,\dots,p_g$ be distinct points 
\redden{in $\PP^1$, and let $C_0$ be}
the curve
obtained by
...
\redden{Thus,}
\marginpar{started new sentence for readability}
...
\redden{0-dimensional}
\marginpar{changed to 0 for better line break}
scheme $2q$
...
\redden{combine}
\marginpar{to allow line break later in paragraph}
...
Lemma~\ref{specialization to cuspidal curve}.
As in the argument
\redden{at the start of Section~\ref{nonexistence},}
possibly
...
\redden{the Brill--Noether theorem}
raises
...
\marginpar{dropped ``then''; if we can't have a dispensation here, then perhaps display $G_d^r(...)?$}
either
...
statement made
\redden{after Corollary~\ref{Weierstrass points}:}
that a general
