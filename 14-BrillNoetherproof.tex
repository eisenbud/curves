...
Interestingly, Castelnuovo's goal was not to prove the Brill-Noether theorem, which was considered established at the time (or at least not in need of further demonstration). Given that
...
if $\cL'$ is an invertible sheaf on $C$ and we choose a trivialization $\phi: \cL'|_{U}\cong \sO_{U}$ of $\cL'$ on a neighborhood $ U\subset C$
...
this would prove Theorem~\ref{BN-basic}; but it is not proper, and thus it is a priori possible that $W^{r}_{d}(C_{0})=\emptyset$ but $W^{r}_{d}(C_{t})\neq \emptyset$ for $t\neq 0$. In other words, a family of invertible sheaves $\sL_{t}$ on $C_{t}$ for $t\neq 0$ may not have a limit
in $\pic_{d}(C_{0})$. 
%Indeed, the limit of a family of invertible sheaves need not be an invertible
%sheaf. 
Nevertheless, such families do have limits, and we can describe these limits quite precisely.
...
%We can extend $\cL^\circ$  to a rank 1 torsion-free sheaf on all of $\cC$, and we regard
...
\begin{exercise}
  ...
  Hint: Use the canonical embedding to realize $C$ as a plane quartic and project from the point of intersection of the tangent lines at $p$ and $q$.
\end{exercise}
