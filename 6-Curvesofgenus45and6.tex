...
In genus 4 we have a question that the elementary theory based on the Riemann-Roch formula cannot answer: are nonhyperelliptic curves of genus 4 expressible as three-sheeted covers of $\PP^1$? The answer will emerge from our analysis in Proposition~\ref{genus 4 trigonal} below. \fix{is the reader as interested as the writer in precisely what methods are needed?}
...
So: suppose now that $C \subset \PP^{g-1}$ is a trigonal curve of genus $g$, and let $\cD = \{D_\lambda\}_{\lambda \in \PP^1}$ be the $g^1_3$. By the geometric Riemann-Roch, the divisors $D_\lambda$ each span a line $L_\lambda$, and these lines sweep out a surface $S = \cup L_\lambda$ containing the canonical curve. By Theorem ****, this surface is a rational normal scroll whose homogeneous ideal is minimally
generated by  ${g-2\choose 2}$ quadrics. In genus 6 the argument above shows that the curve lies 
on at least 6 independent quadrics, and by Theorem **** \fix{canonical curves are ACM -- to be written}  these generate all the quadrics in the homogeneous ideal of the curve. The general case is similar:
\begin{proposition}
The space of quadrics in the ideal of any canonical curve has dimension ${g-2\choose 2}$. Thus the quadrics in the ideal of a trigonal canonical curve generate the ideal of a rational normal scroll.
\end{proposition}

 lies on exactly 6 quadrics, which therefore coincide with the generators
of the By Corollary~\ref{hyperelliptic and trigonal} the 6 quadrics containing a trigonal canonical  curve of genus 6 generate the ideal of this surface, which is a rational normal scroll. 


The first thing to observe is the
\begin{proposition}
