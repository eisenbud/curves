...
the left is
\redden{a generic point of} the ``principal'' component: a
\marginparhere{edited\\[10pt] I'd lost sight of the Silvio note here; I'll have a new version later today}
smooth
...
\redden{principal}
component
\marginpar{extended overline (assuming this is the same object as on line 2 of page 338)}
...
\redden{Later (page~\pageref{rigid?})}
...
\redden{with}
\marginpar{dropped ``the cases'', ``these''}
$g =
...
As we've seen in
\redden{Theorem~\ref{canonical genus 4},}
...
\marginpar{moved comma relative to ``in''}
...
\marginpar{I took missing tag `balanced CI' to be identical to `balanced
CI in higher codim'}
...
\marginpar{dropped ``us'' to allow line break} 
...
\marginpar{removed quotes around ``expected'' (since it's been defined)}
\end{proposition}
...
\redden{these}
possibilities:
\marginpar{\vskip10pt removed quotes around types}
$$
h^0(\cO_C(2)) =
\begin{tcases}
...
$$
By the
\marginpar{dropped ``and note that''; both changes to allow line break}
...
\marginpar{dropped ``and'' between cites}
\index{Mumford, David}%
\index{Nasu, Hirokazu}%
...
\marginpar{\vskip-30pt added comma after matrix, ok?}
\end{exercise}
...
\marginpar{Should this be ``linear series''?}
...
\redden{Section~\ref{degree 4 genus 0} (genus 0)}
...
\marginpar{punctuated as full sentences and removed ``and''}
