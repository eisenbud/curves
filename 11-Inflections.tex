...
Note: it's much less clear whether it can be the case that a general tangent line to $C$ can intersect $C$ in a point other than $p$, except of course in case $r=2$. See~\cite{} for results on this question.\fix{Joe will write to Noam to ask for a reference.}
...
$N(S)$ is finitely generated for any surface---in characteristic 0 it is the quotient of the second integral homology group by the torsion elements. Its rank is called the Picard number of the surface. The result about $N(S)$ that we need is:

\begin{theorem}[Hodge Index theorem]\label{hodge index}
If $H\subset S$ is an ample divisor on a smooth projective surface, and $D$ is a divisor with $D\cdot H = 0$, then 
...
%$$
%L(f) := \sum_{i=0}^n (-1)^i{\rm Trace}\left(f^* : H^i(X,\CC) \to H^i(X,\CC)\right).
%$$
%The Lefschetz fixed point formula  says that if $f$ has isolated fixed points, the number of those points, properly counted, is equal to $L(f)$.
%
