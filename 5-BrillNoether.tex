...
%
But maybe that wasn't the question we meant to ask! After all, we're interested in describing curves in projective space as images of abstract curves $C$ under maps given by linear systems on $C$. Observing that the linear series that achieve equality in Clifford's theorem give maps to $\PP^r$ that are 2 to 1 onto a rational curve, we might hope that we would have a different---and more meaningful---answer if we  restrict our attention to linear series $\cD = (\cL,V)$ for which the associated map $\phi_\cD$ is at least a birational embedding; that is, we would like to know bounds for the genus and degree of a reduced,
irreducible curve in $\PP^r$. 

%We will show that the maximal dimension of a birationally very ample linear series of degree $d$ on a curve of genus $g$ is a decreasing function of $g$. 
%With this understanding, we need only ask \fix{I don't understand the "we need only": For plane curves they only exist at the bound. Is there a band
...
A classical result of Castelnuovo gives such a bound, which is in some sense explained by the classification of the curves that achieve it. For positive integers $d$ and $r$, we write
$$
...
\begin{example}For a more typical situation, consider the case $r=3$
Then we have $d= 2M+1+\epsilon$ with $\epsilon = 0$ if $d$ is odd or 1 if $d$ is even, so 
$$
p_a \leq \pi(d,3) = 
\begin{cases}
d^2/4-d+1 & \text{ if } 2|d\\
d^2/4 -d+3/4& \text{ if } 2 \not | d
\end{cases}
$$
 If $d$ is even and $C$ is a curve of class
$(d/2, d/2)$ on a smooth quadric or $d$ is odd and $C$ is a curve of class $((d-1)/2, (d+1)/2)$
on a smooth quadric then $g = \pi(d,3)$  so these two classes of curves on a smooth quadric
achieve the bound, and a variant of this works for singular quadrics as well. The classification  explained in Chapter~\ref{ScrollsChapter}
shows that these are the only Castelnuovo curves in $\PP^3$.
\end{example}

\begin{fact}
A famous theorem of Gruson and Peskine~\ref{Gruson-Peskine} completes the picture of the possibilities for the degree $d$ and  genus $g$ \fix{Check P-G; I think these can all be smooth, but I'm not sure}
 of a curve in $\PP^3$: Such curves always exist if
$$
g\leq \pi_1(d,r) := 
\begin{cases}
 d^2/6-d/2+1&\text{ if } 3|d\\
 d^2/6-d/2+1/3 & \text{ if } 3\not | d
\end{cases}
$$
but for $\pi_1(d,r) < g\leq \pi(d,r)$,
such a curve must lie on a quadric, and thus such curves exist if and only if the equations $d=a+b\ g=(a-1)(b-1)$ can be solved for $a,b$. A first example
of non-existence is
\end{fact}
To prove the result, we will give lower bounds for the dimensions of the linear series  cut on a curve $C$ by hypersurfaces of degree $m$. For large values of $m$ the line bundle $\cO_C(m)$ is non-special and $H^1(\sI_{C/\PP^r}(m) = 0$, so a lower bound on the dimension of its space of sections translates, via the Riemann-Roch theorem, into an upper bound on the genus $g$ and this is the Castelnuovo bound. To execute this strategy, we need to know more about the set of points that can be the general hyperplane section of a curve. 
It turns out that what we need follows from a linear independence result:
...
We will prove this as a consequence of the far stronger Uniform Position Theorem, in the next section. From Lemma~\ref{general position lemma} we
can derive a seemingly crude bound on the number of conditions imposed by hyperplane sections:


To describe the consequence for the dimensions of linear series, the following definition will be convenient:
