...
Suppose that $C$ is a smooth projective curve of genus $g\geq 2$. If $D$ is a divisor on $C$ with $\deg D \leq 2g-2$, then $r(D) := h^0(D) - 1 \leq d/2$. Moreover, if equality holds then either $D\sim 0$ or $D\sim K$ or
 $C$ is hyperelliptic, and $D$ is a multiple of the unique divisor of degree 2 that moves in a pencil.
\end{theorem}

\begin{proof} We prove the inequality, and  postpone the case of equality to give a slick characteristic 0 proof using  uniform position (Corollary****). Note that the result is true in any characteristic; see for example \cite[Theorem ****]{Hartshorne}.

When $r(K-D)\leq 1$, the inequality in Clifford's Theorem is immediate from the Riemann-Roch Theorem
$r(D)-r(K-D) = \deg D - g +1$. On the other hand, when $r(K-D) \geq 1$ we note that
$$
 r(D)+r(K-D) \leq r(K) = g-1.
$$
Adding these formulas gives $2r(D)\leq d$ as required.
...
The monodromy group of the general hyperplane section of an irreducible curve $C$
in $\PP^r_\CC$ is the full symmetric group; in particular the varieties $Y^k$ are irreducible.
\end{theorem}
...
Given this, let $p \in C$ be a general point, and let $H \subset \PP^r$ be a general hyperplane containing the tangent line $T_pC$. The fiber of $\Phi$ over the point $H \in {\PP^r}^*$ consists of the point $p$ with multiplicity 2, and $d-2$ reduced points; applying Lemma~\ref{transposition lemma}, we deduce that $M$ contains a transposition.
\end{proof}
...
\begin{proof}
 ...
 a family of dimension $r(D)+r(E)$.
 
In the case where $D+E$ is birationally very ample then, restricting to an open set,
we may identify $C$ with its under the complete linear series $|D+E|$. and we see that every (general) hyperplane section $H\cap C$ contains a divisor equivalent to $D$. Since the monodromy of the general hyperplane section
is the symmetric group, this implies that every subset of $\deg D$ elements in $H\cap C$ is
