...
For $2\leq r<n$
 let $V_r$ be the set of $r$-tuples of points of $C$ that are either linearly dependent or whose span contains another point of $c$.
We do induction on $n$ beginning with $n=3$. Since $C$ is not strange, \cite[Proposition IV.3.8]{Hartshorne1977} shows that not every secant of $C$ is a multisecant, and this takes care of the case, $n=3$. It also shows that there is a smooth point  $p\in C$ such that the projection $\pi_p$ from $p$ is birational. Let $d= \deg C$.

We claim that $C'$ is not strange. By \cite[Proposition IV.3.8]{Hartshorne1977} it suffices to show that some pair of tangent lines
to $C'$  do not meet. Choose general points $q,r\in C$, and let $V\subset \PP^n$ be the projective space spanned by the tangent lines at $q,r$, which has dimension 3 since $C$ is not strange. Since $C$ is nondegenerate and $n>3$, we may assume that the projection center
...
\fix{Ex \ref{linear bound is sharp} and the castelnuovo 2n+3 fact following were duplicated in
the Brill-Noether chapter; I deleted that one. Would it be better there?}
