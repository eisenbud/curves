...
One way to study a curve in $\PP^n$ is to use properties of its hyperplane sections. For a start, we show that any $n$ points of a general hyperplane section are linearly independent except in a very special case. Then we will prove a much stronger general position statement in characteristic 0.
...
all the tangent lines to $C$ meet in a point $p\in \PP^n$. Note that the projection of $C$ from $p$ is inseparable, and thus there are no strange curves in characteristic 0. The only smooth curve that is strange is the conic in characteristic 2 (but see
...
Show that $C$ is a complete intersection, defined by the equations
$$
x_0^{q-1}x_2 - x_1^q, x_0^{q-1}x_3 - x_2^q,\dots, 
x_0^{q-1}x_n - x_{n-1}^q.
$$
Show that every secant line to $C_0$ meets $C_0$ in  if $r<n$ and $a_1, \dots, a_r$ are linearly independent points on $C_0$ then
the points in the linear span $
\end{exercise}
...
\fix{put in Rathmann's argument, referring as he does to H for the strange case; and also the example 1.2 in his paper to show that smooth is necessary.} 
