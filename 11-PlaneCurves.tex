...
sheaf is an invertible sheaf, just as in the smooth case. 
...
that of $C_0$. But the sheaf of differentials on $C_0$ is generally not well-behaved, not even locally free, and it does not
have the properties that make the canonical sheaf useful. The canonical
sheaf on $C_0$ is defined differently.
...
$$
\end{fact}
For a proof in the case of the inclusion of a projective variety see \cite[Section III.7]{Hartshorne1977}, while for the more general assertion above the
...
$\sExt^1_{\PP^1}(\coker \alpha, \sO_{\PP^1})$ is has the same length and the same annihilator
...
$\omega_{C_0}$ is locally cyclic, and  since its support is finite, it is also globally cyclic, that is,
...
Despite its antiquity, many questions about the family of plane curves, such as which ones degenerated into which others, and in what way, remain open. All plane curves of degree $d$ have the same Hilbert function, and thus the same arithmetic genus
