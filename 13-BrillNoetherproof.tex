...
We will also adopt this general approach, and we will prove the existence and non-existence parts together. However, the strength of Theorem~\ref{osculating intersection} compared to Exercise~\ref{secant general position} suggests specializing to a $g$-cuspidal curve $C_0$ rather than a $g$-nodal one, and we will use
this refinement. 
...
codimension of an ideal of minors of a certain matrix of functions.

\begin{theorem}\label{local existence}
Let $\sC/B$ be a family of reduced irreducible projective curves of arithmetic genus $g$. If
one fiber  $C_0$ of the family has $\dim W^r_d(C_0) = \rho(p,r,d) \geq 0$
locally at a particular invertible sheaf $\sL_{0}$,  then $\dim W^r_d(C_b) = \rho$ locally for all $b$ in a neighborhood of $0 \in B$ and invertible sheaves in a neighborhood of $\sL_{0}$
in the relative Picard variety $\pic_{d}(\sC/B)$. In particular, $W^r_d(C_b)$ is nonempty for all $b$ in a neighborhood of $0 \in B$.
\end{theorem}

The idea of the following proof is a modern form of the discussion in the original paper of Brill and Noether. The reason for including singular curves in the family is that for large $g$ 
it is hard to construct a smooth curve and sheaf satisfying the hypothesis, and we will do this with
singular curves, below.

\begin{proof}  Over a sufficiently small neighborhood $U$ of $0\in B$ we may choose $m$ sections
...
By the semicontinuity of fiber dimension, we may choose $m$ so large that $h^{1}(\sO_{C}(D)) = 0$
and that for every invertible sheaf $\sL_{b}$ of degree $d$ on $C_{b}$, with $b\in U$ we have
$h^{1}(\sL_{b}(D_{b}) = 0$. b
By Theorem~\ref{easy RR} we have
$$
 \hbox{ and } h^0(\cL_{b}(D_{b})) =  m - g + 1
$$

The pushforward of the Poincar\'e bundle $\cP$ on $\pic_{d+m}(\sC/B) \times \sC$ to $\pic_{d+m}(\sC/B)$ is thus a vector
bundle $\sE$ of rank $d + m - g + 1$ on $B$ whose fiber over a point representing $\sL_{b}$ is the vector space $H^0(\cL(D_{b})$. Similarly,
the pushforward 
${\pi_1}_*(\cP)|_{D_{b}}$
...
\cP  \rTo \cP_{D}
$$
pushes forward to give a map of vector bundles $\phi : \cE \to \cF$ on $\sC$ which, on a fiber over $b\in U$, is the evaluation of sections $\sigma \in H^0(\cL_{b}(D_{b}))$ at the points $p_i$.

If $\cL_{b}$ is an invertible sheaf of degree $d$ on $C_{b}$ then the space $H^0(\cL_{b})$ is the kernel of the map $\phi$ at the point corresponding to $\cL_{b}(D_{b}) \in \pic_{d+m}(\sC/B)$. Locally on $\sC$ the map $\phi$ can be defined by a $(d+m) \times (d+m-g+1)$ matrix of regular functions. The locus $W^r_d(C_{b})$ is the translate
by $\otimes \sO_{\sC}(D)$ of the locus where $\phi$ has rank $d+m-g-r$ or less, and by \cite[Exercise 10.9]{Eisenbud1995} this locus is either empty or its components have codimension $\leq (r+1)(g-d+r)$ in $\pic_{d+m}(C)$, which has dimension $g$. Thus every component of $W^r_d(C)$ has dimension at least $\rho$. 
\end{proof}
...
We will say that we \emph{crimp} $C$ at $p$ to obtain $C_{0}$.
...
that is, \emph{we can smooth $C_0$}. 
