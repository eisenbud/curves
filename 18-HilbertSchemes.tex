...
In earlier Chapters, we described some  degree $d$ embeddings curves of  genus $g$ in projective spaces for small $d,g$. In this chapter, we will try to describe the family of all such curves in $\PP^r$---that is, the Hilbert schemes $\cH  := Hilb_{dm-g+1}(\PP^r)$---focussing mainly on the case $r=3$ of space curves
...
The first case to consider is that of the Hilbert scheme  $\cH = Hilb_{3m+1}(\PP^3)$. The corresponding restricted Hilbert scheme $\cH^\circ$, parameterizing twisted cubics, is one we have encountered and described multiple times already: in Proposition~\ref{hilb of twisted cubics} we showed that $\cH^\circ$ is irreducible of dimension 12,
...
$H^0(\sN_{C/\PP^3}) = \CC^{12}$, so $\cH^\circ$ is smooth at this point.

We will not repeat those arguments here. Instead, we want to use the example of $Hilb_{3m+1}(\PP^3)$ to describe a phenomenon fairly common in Hilbert schemes: the presence of \emph{extraneous components}.

\subsection{The other component of $\cH = Hilb_{3m+1}(\PP^3)$}

Again, let $\cH = Hilb_{3m+1}(\PP^3)$ be the Hilbert scheme and $\cH^\circ \subset \cH$ the open subset of smooth, irreducible, nondegenerate curves of degree 3 and genus 0; that is, twisted cubics. We might naively expect that the closure $\overline{\cH^\circ}$ would be all of $\cH$, but this is in fact not the case: in fact, $\cH$ has two irreducible components, one the closure of $\cH^0$ and the other (the ``extraneous component") described below.

To describe the extraneous component, suppose we start with a plane cubic curve $C_0 \subset \PP^2 \subset \PP^3$. This is of course a curve of degree 3, but its Hilbert polynomial is $p(m) = 3m$ rather than $3m+1$, reflecting the fact that (as we have verified many times) the genus of $C_0$ is 1, not 0.

But that can be very simply fixed: if $p \in \PP^3 \setminus C_0$ is any point not lying on $C_0$  we take $C := C_0 \cup \{p\}$. This has the effect of increasing the Hilbert polynomial by 1, so that the Hilbert polynomial of $C$ is $3m+1$; and so $C$ does correspond to a point of $\cH$. In fact, the locus in $\cH$ of curves $C$ of this form is open, and its closure is a second irreducible component of $\cH$. 
...
Before we go on with our exploration of Hilbert schemes of curves, we should introduce a small simplification of the notation: for the Hilbert scheme $\cH_{dm-g+1}(\PP^r)$ of curves of degree $d$ and genus $g$ in $\PP^r$ we will write simply $\cH_{d,r,g}$; and similarly we'll write $\cH_{d,r,g}^\circ$ for the open subset of smooth, irreducible and nondegenerate curves.
...
The open set $\cH^\circ \subset \cH_{0,r,d}$ parametrizing smooth, irreducible nondegenerate rational curves $C \subset \PP^3$ is irreducible of dimension $(r+1)(d+1)-4$; in case $r=3$ in particular it has dimension $4d$.
\end{proposition}
