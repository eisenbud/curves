...
\marginpar{dropped ``the exposition''} 
\cite{Kleiman-PicardScheme},
which has extensive references to the original literature. 
\marginpar{removed \string\fix; the reference to a Mazur quote ``above'' suggests it was outdated}

A consequence of the modern theory is that we can extend the
\marginpar{indexing Picard scheme/variety in one entry, OK?}
\index{Picard variety/scheme}%
...
\marginpar{removed extra right parens}
...
\marginpar{closed up ``quasi projective''}
\index{quasiprojective scheme}%
\blue{quasiprojective schemes}
as well
...
of 
\marginpar{dropped ``all'' (to allow line break)} 
...
x_{d}]$. The ring of 
\blue{invariants of the symmetric group}
...
\redden{(See \cite[Exercises 1.6, 13.2, 13.3, 13.4]{Eisenbud1995}.)}
...
\redden{formal neighborhood}
\marginpar{will ``formal neighborhood'' be understood?}
...
\marginpar{moved this sentence into the fact and removed unused theorem number since it doesn't interact well with the Fact; but kept the italics.}
...
\redden{have}
\marginpar{assuming the subject is ``tensor product and inverse''}
degree~0, 
...
\blue{projective algebraic group}
...
\index{calculus}%
\index{historical context}%
\marginpar{created an index entry ``historical context'' for such very interesting accounts; will backindex later}%
problem. A goal of the nineteeth century
mathematicians was  to make sense of 
\blue{integrals of algebraic functions.}
\index{integration}%
...
\index{path integral}%
of 
\blue{meromorphic differentials}
...
\marginpar{indexed ``elliptic integral''}%
...
\marginpar{I swapped the calls to this figure and the next (and their placement), because this figure doesn't show a higher-dimensional lattice, and the other figure doesn't show an elliptic curve.}
...
\index{path integral}%
of 
\blue{holomorphic differentials}
...
equal, $\mu(D) = \mu(D')$; 
\redden{equivalently, if and only if}
...
\redden{the part proved by Abel; the converse, which is substantially more subtle, was proved by Clebsch}.
\marginpar{moved the red sentence from within the proof}
...
$$
and by the 
\blue{universal property}
\index{universal property}%
of the 
\blue{symmetric product,}
...
\redden{give an algebraic description of}
the structure of the
\index{Weil, Andr\'e}%
Jacobian, using the following fact: 
\marginpar{moved name to head of result}
...
\marginpar{will ``generically injective'' be clear?}
...
\marginpar{or just ``one-to-one''; or do we really need ``set-theoretically''?}
\redden{bijective}. To prove that it is an embedding, we must show
...
\blue{fundamental theorem of calculus}
...
codifferential 
\marginpar{dropped comma before ``is'', ok?}
...
If 
\marginpar{added ``of Theorem...'' because of intervening text}
...
\marginpar{\hskip-80pt $\leftarrow$ undisplayed for better layout; I think that's harmless since the notation is local to this proof}
...
\marginpar{Im $\to$ im (to agree with other occurrences)}
...
\redden{This shows that}
\marginpar{for variety; there is ``Thus'' on the next line}
...
\redden{(see \null Theorem~\ref{needed for nodes})}
...
\marginpar{replaced ``, and'' by period}
...
\marginpar{dropped quotes around 0} 
...
The set $W^r_d(C)$ can be given the structure of a scheme, with which
it is a 
\blue{fine moduli space}
\index{fine moduli space}%
...
\marginpar{added footnote for hitherto undefined term; took the def from Chap 10}
...
\marginpar{The format here looks a bit strange.  Maybe drop the C.F.header?}
...
\marginpar{dropped parentheses and capitalized ``See''}
...
\marginpar{indexed under ``secant plane lemma'', ok?}%
...
\marginpar{replaced $\to$ by $\mapsto$}
...
\index{geometric Riemann--Roch theorem}%
\index{Riemann--Roch theorem!geometric}%
\redden{(Theorem~\ref{geometric RR}),}
...
\redden{The theorem can be extended to show}
...
\begin{exercise}\label{quotient of projective}
 Let $G$ be a 
\blue{finite group acting}
\index{finite group action}%
on a 
\blue{quasiprojective scheme}
\index{quasiprojective scheme}%
$X$. 
...
\marginpar{removed parentheses and started new paragraph for consistency}
\item Conclude that the 
\blue{symmetric square}
...
\blue{\emph{natural transformation}:}
...
\marginpar{does $W^r_d(C)$ have a name under which to index this?}
...
\index{Abel--Jacobi map}%
$\mu_1$. Show that the 
\blue{self-intersection} 
\index{self-intersection}%
of the curve $C$ is 2,
\marginpar{1. are these alternative routes or one route in two steps?\\2. Do we have to say ``self-intersection in $J(C)$'', or qualify $C$ somehow?}
\begin{enumerate}
...
\marginpar{moved up the map name}
\redden{Abel--Jacobi map $\mu_2 : C_2 \to J(C)$.}
\end{enumerate}
...
\index{g@$g+1$ theorem}%
If $D \in C_{g+1}$ is a 
\blue{general effective divisor}
