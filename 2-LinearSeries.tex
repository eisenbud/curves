...
$\sL$ is an invertible sheaf on $X$ and $V = \langle\sigma_{0}, \dots \sigma_{n}\rangle$ is a vector space of global sections, we define the \emph{base locus} of $\sV$ to be the subscheme 
$$
B_\sV := \bigcap_{i= 0}^{n}\{\sigma_{i} = 0\}.
$$
Let $W:=X\setminus B_\sV$, the open subscheme where not all sections $\sigma_{i}$ vanish.

The closed subscheme  For any point $q\in W$ we  may choose an open neighborhood $W'\subset W$ of $q$, and an identification 
$$
...
The most useful linear series are those that define morphisms defined on all of $X$. This happens when
...
functions
$x_{0}/x_{j},\dots, x_{n}/x_{j}$ with $\phi$ we get functions $\sigma_{0},\dots,\sigma_{n}$ on $W_{i,j}$.  The function $\sigma_{j}$, is the pull-back of the constant function 1 on $U_{j}$, so it generates $\sO_{W_{i,j}}$, and we see that the module generated by
the $\sigma_k$ is the free module on 1 generator, and the collection of these modules defines an invertible sheaf, together with
$n+1$ global sections. Note that the subscheme  $\{\sigma_k = 0\} \subset W_{i,j}$  is the scheme-theoretic preimage of the
the hyperplane $\{x_k = 0\}\subset \PP^n$.
