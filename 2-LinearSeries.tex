...
Given $f: C\to \PP^{n}$, the preimages of hyperplanes not containing the image of $C$ will be finite sets of points on $C$, possibly with multiplicities in the case of a hyperplane tangent to the image, or passing through a singular point of the image. Such a set of points with non-negative multiplicities is called an \emph{effective divisor} on $C$. In the case of a map from a singular curve, or a more general scheme, an effective divisor (sometimes called an effective Weil divisor) is simply a purely codimension 1 subscheme; a \emph{divisor} is an formal linear combination of effective divisors, with integer coefficients. The divisors coming as the pullbacks of hyperplanes are special: since a hyperplane is defined by just one equation, which is locally given by the vanishing of a function, the pullback of a hyperplane will be locally defined by the vanishing of a single function as well; that is, it is an  \emph{effective Cartier divisor} (On a smooth curve every effective divisor is locally defined by a single equation, so this is not an issue for us.) A \emph{Cartier divisor} more generally is a formal integral linear combination of effective Cartier divisors.
...
is again projective, and the only projective subvarieties of the affine variety $\CC$ are points. 
%(From the complex analytic point of view this is a consequence of the 
%``maximum modulus principle'': any nonconstant analytic function on a compact variety must take on a maximum value at a finite set of points, but the maximum principle says that when restricted to a disk around any such point, the maximum modulus will occur on the boundary of the disk.)
...
Of course we want to understand morphisms more than set-theoretically, and we want to be able to produce such maps from data on $C$. For this we use the notion of linear system (sometimes called linear series): 
...
A \emph{global section} of $\sF$ is an element $t_{i}\in F_{i}$ such that 
$\sigma_{i,j} t_{i} = t_{j}$. Such a section may be realized as the image of the constant function 1 under
a homomorphism of sheaves $\sO_{X} \to \sF$. By Theorem \cite[Thm ***]{H} the space $H^{0}(\sF)$  of global sections is
...
If $D\subset X$ is an effective Cartier divisor, then we define $\sO_{X}(-D)$ to be the ideal sheaf of $D$. It is an invertible
sheaf because, by definition, $D$ is locally defined by the vanishing of a (locally defined) nonzerodivisor in $\sO_{X}$.
We write $\sO_{X}(D)$ for its inverse.

\begin{example} [Invertible sheaves on $\PP^{n}$] If $H\subset \PP^{n}$ is a hyperplane defined by the vanishing of a linear form $\ell = \ell(x_{0}, \dots x_{n})$ then the ideal sheaf $\sO_{\PP^{n}}(-1) := \sI_{H/\PP^{n}}\subset \sO_{\PP^{n}}$ is generated on the open affine set 
$U_{i}:= \{x_{i}\neq 0\} \cong \AA^{n}$
by $\ell/x_{i}$, and is thus an invertible sheaf. 
Moreover, if $H'$ is the hyperplane defined by another linear form $\ell'$, then 
$$
\frac{\ell'}{\ell}\cdot\sI_{H/\PP^{n}} = \sI_{H'/\PP^{n}} 
$$
so the sheaves $\sI_{H/\PP^{n}}$ and $\sI_{H'/\PP^{n}} $ are isomorphic, justifying the name $\sO_{\PP^{n}}(-1)$.

More generally, the $p$-th tensor power of $\sO_{\PP^{n}}(-1)$ is called $\sO_{\PP^{n}}(-d)$; it is isomorphic to the
ideal sheaf of any hypersurface of degree $d$. Because polynomials satisfy the unique factorization property,
every effective divisor $D\subset \PP^{n}$ is a hypersurface of some degree $d$, so
$\sO_{\PP^{n}}(-D) \cong \sO_{\PP^{n}}(-d)$. Note that if $d>0$ then $H^{0}(\sO_{\PP^{n}}(-D)) = 0$, since it may be realized
as the sheaf of locally defined functions vanishing on $D$, and there are no such
globally defined functions except 0.

We take $\sO_{\PP^{n}}(d)$ to be the inverse of $\sO_{\PP^{n}}(-d)$. If $D$ is the hypersurface defined by 
a form $F$ of degree $d$, then $\sO_{\PP^{n}}(-D)$ is generated on $U_{i}$ by $F/(x_{i}^{d})$, so
$\sO_{\PP^{n}}(D)$ is generated on $U_{i}$ by $x_{i}^{d}/F$
Starting from the inclusion 
$
\sO_{\PP^{n}}(-D) \subset \sO_{\PP^{n}}
$
and taking inverses, we see that 
$
\sO_{\PP^{n}} \subset \sO_{\PP^{n}}(D)
$
and the global section $1\in H^0(\sO_{\PP^{n}})\subset H^0(\sO_{\PP^{n}}(D))$, restricted to
$U_{i}$, is $F/(x_{0}^{d})$ times the local generator of $\sO_{\PP^{n}}(D)$ and thus vanishes on $D$.
 Because every
rational function on $\PP^{n}$ has degree 0, and any two global sections differ by a rational
function, it follows that every global section of $\sO_{\PP^{n}}(d)$ vanishes on a divisor of degree $d$. Thus
we may identify $H^{0}(\sO_{\PP^{n}}(d))$ with the ${n+d\choose n}$-dimensional vector space of forms of degree $d$ on $\PP^{n}$.
\end{example}

\subsection{The map to projective space coming from a linear system} For any $\CC$-vector space $V$ of dimension $n+1$ with basis $x_{0}, \dots, x_{n}$, we write $\Sym(V) \cong \CC[x_{0},\dots, x_{n}]$ for the symmetric algebra on $V$, and
$\PP(V)\cong \PP^{n}_{\CC}$ to be the projective space ${\rm Proj}(Sym_{V})$, which is naturally isomorphic to the
space of lines in $V^{*}$. 
...
Given a linear system $\sV:=(\sL, V)$ on a scheme $X$, where
$\sL$ is an invertible sheaf on $X$ and $V = \langle\sigma_{0}, \dots \sigma_{n}\rangle$ is a vector space of global sections, we write
$W\subset X$ for the open subscheme where not all sections $\sigma_{i}$ vanish; that is, 
$$
W = X\setminus \bigcap_{i= 0}^{n}\{\sigma_{i} = 0\}.
$$
...
$$
W'\ni p \mapsto \bigl(t(\sigma_{0}(p)),\dots, t(\sigma_{n}(p))\bigr) \in \PP(V).
$$
This map is obviously a morphism on $W'$. A change of neighborhoods $W'$ or of identifications $t$ would simply multiply
each value $t(\sigma_{i}(p))$ by a unit, and thus define the same morphism. It follows that the morphisms
defined on different $W'$ agree on overlaps, and thus define a morphism $W' \to \PP(V)$.

\subsection{The linear system coming from a map to projective space}

Conversely, suppose that we are given a morphism $\phi: X\to \PP^{n}$. With notation as in Example~\ref{linear systems on Pn} we may choose an open cover $W_{i,j}$ of $X$ such that $\phi(W_{i,j})\subset U_{j}$. Composing the regular
functions
$x_{0}/x_{j},\dots, x_{n}/x_{j}$ with $\phi$ we get functions $\sigma_{0},\dots,\sigma_{n}$ on $W_{i,j}$, and 
these are all multiples of the regular function $\sigma_{j}$, the pull-back of the constant function 1 on $U_{j}$. Thus
we may regard the 
