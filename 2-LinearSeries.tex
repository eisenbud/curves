...
\end{theorem}
 ...
 \subsection{The equation of a hyperelliptic curve}
  ...
  In fact, we claim that \emph{there is a unique smooth projective curve $C$ expressible as a 2-sheeted cover of $\PP^1$ branched over a given collection of points $p_1,\dots,p_{2g+2} \in \PP^1$}. Given this, it's not hard to write down the equation of this curve: if the coordinate of the point $p_i \in \PP^1$ is $\lambda_i$, it is the smooth projective model of the affine curve 
  $$
...
\subsection{The canonical map of a hyperelliptic curve}
...
Note that as a consequence of this observation, we see that \emph{a hyperelliptic curve $C$ has a \emph{unique} linear series $g^1_2$ of degree 2 and dimension 1}, that is, a unique map of degree 2 to $\PP^1$. Also, since any special linear series is a subseries of the canonical series, it must likewise factor through the degree 2 map $C \to \PP^1$; thus in particular \emph{any very ample linear series on $C$ must be non-special}. Finally, since the linear series $\cO_C(1)$ is special on any plane curve, a smooth plane curve cannot be hyperelliptic; thus the  degree of any projective embedding $C \hookrightarrow \PP^r$ of a hyperelliptic curve $C$ is at least $g+3$. In fact, we will see in Chapter~\ref{5} that any hyperelliptic curve can indeed be embedded as a curve of degree $g+3$ in projective space, and we'll describe the geometry of the image curve in Chapter~\ref{7}. Moreover, we'll also see that this is maximal among all curves; that is, any nonhyperelliptic curve admits an embedding of degree $<g+3$. 

\

\fix{this uses the genus formula for smooth plane curves. Do we assume this, should we give a quick proof, or should we just find another way to see that a smooth plane curve is nonhyperelliptic?}
