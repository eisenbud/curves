...
``maximum modulus principle'' for analytic functions: any function on a compact set must take on a maximum value at some point p, but the maximum principle says that the maximum modulus will occur on the boundary of a disk centered at $p$ on any chart where the function is defined, and this implies that the function is constant \fix{huh?}

If we are given the family of divisors on $C$ that are the preimages of the hyperplane intersections, we can reconstruct the map $f$ set-theoretically: we take a point $p\in C$ to the point of projective space that is the intersection of those
hyperplanes whose preimages contain $p$. To extend this idea to the definition of a map of schemes, we introduce invertible sheaves.
