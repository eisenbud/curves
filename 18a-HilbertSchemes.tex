...
%\fix{Joe called this the Cartesian approach---the first time this phrase was used. If worth putting in then it needs an explanation.}
...
Theorem~\ref{tangent hilb}. \fix{right ref?}
...
\fix{added: best argument?} The irreducibility of the Hilbert scheme in the case $d>2g-2$ also follows from this argument 
...
%\fix{The following needs some background; it really only makes sense in conjunction with the argument
%estimating dim Hilb through Brill-Noether and the remark (was it made in the BN chapter?? that if $\rho\geq -2$
%then the locus has expected dimenison. I'm moving it to the end section.}
...
\fix{material moved from earlier to integrate}
