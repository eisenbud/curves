...
$\cH^\circ_{g,3,d}$ parametrizing smooth curves of degree $d$ and genus $g$ in $\PP^3$ has dimension $4d$. While this is not the case in general (Chapter~\ref{HilbertSchemesCounterexamplesChapter}), $4d$ is indeed the ``expected dimension'' from two points of view, explained in Sections~\ref{chi N} and \ref{estimating dim hilb},
...
\subsection{Estimating $\dim \cH^\circ_{g,r,d}$ by the Euler characteristic of the normal bundle}\label{chi N}

Deformation theory suggests why this should be true so often.
...
\section{Estimating $\dim \cH^\circ_{g,r,d}$ using the Brill-Noether theorem}
