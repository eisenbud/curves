...
$X$. \fix{maybe we should change this argument to prove directly that $\sL$ is an invertible sheaf, since that's our standard language}
...
$\sE$ is a vector bundle on $\PP^{1}$. Since the restriction of $\sL$ to each fiber is $\sO_{\PP^{1}}(1)$, which has two global sections, we see that $\sE$ has rank 2 \cite[Theorem ****]{Hartshorne1977}.
...
$$
Since the spaces spanned by $C_{1}$ and $C_{2}$ are complementary, this map of rank 2 vector bundles is an inclusion. Since $\sE'$ has only $a_{1}+a_{2}+2$ independent global sections, and is generated by them, the map is an isomorphism, proving that $\cE \cong \cE'$.

For the last statement, recall from \cite[]{Hartshorne1977} that giving a map $X\to \PP(\cE)$ is
\end{proof}
...
Our interest in scrolls in this book is primarily for the curves that lie on them. The following result tells us what curves there are:

\begin{theorem}\label{where are the curves?}
 Let $X = S(a_{1}, a_{2})$ be a scroll of dimension 2 with $a_{1}\leq a_{2}$. The Picard group of $X$ is $\Pic X \cong \ZZ^2$, freely generated by  the class $F$ of a ruling and the class $H$ of a  hyperplane section. There are reduced irreducible curves in the class $D = pH+qF$ if and only if one of the following holds:
\begin{enumerate}
