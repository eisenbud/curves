...
\blue{global sections}
\index{global section}%
of an 
\blue{invertible sheaf}
...
have \blue{only finitely many inflection points} (this can be proven
...
\marginpar{removed unused equation number}
$$
...
\marginpar{moved Z's name outside of cite and changed tense to past}
Zariski
\index{Zariski!Oskar}%
...
\blue{``flex divisor''}
\index{flex divisor}%
\marginpar{why the quotation marks?}
$\sum w(C, p)p$
...
\marginpar{replaced ``we have'' by colon}
...
(This is how Weierstrass points were characterized when first studied.)
\marginpar{streamlined; but maybe a citation would be relevant?}
...
$$
Applying
\redden{the Riemann--Roch theorem}
...
\marginpar{indexed as Ragnar-Olaf Buchweitz, is that correct?}%
...
\marginpar{\redden{original had hardwired [EH]; is this the intended cite?}}
\cite{EHLGP}
...
\marginpar{added accent to N\'eron}%
...
$$
solving, we see that
\marginpar{undisplayed $w_i \leq i$}
$
w_i \leq i
$
and hence
...
Note that the argument here gives an explicit bound,
\redden{$(g^3-g)!$},
\marginpar{moved/streamlined}
for the size
of $\Aut C$. In fact, we can do much better:
Exercise~\ref{84(g-1)} 
\redden{invites you}
to prove the inequality
$|\!\Aut C| \leq 84(g-1)$, which is 
\marginpar{dropped ``in fact''}
...
Grassmannian 
\index{G!$\GG(r+1, d+1)$}%
$\GG(r+1, H^{0}(\sO_{\PP^{1}}(d))) = \GG(r+1, d+1)$.
\marginpar{$G\to\GG$ for consistency with earlier chapters (unless I'm confused)}
...
is an irreducible subvariety of $\GG(r+1,d+1)$ of codimension 
\marginpar{$G\to\GG$ (further instances in this chapter unmarked)}
...
\marginpar{$\geq \to$ not less than (twice)}
not less than
...
\redden{of dimension~$\ell$}
\marginpar{To allow line break\\[2pt]\redden{There were two propositions with the label `ramification': this one and Proposition \ref{ramification}. Since this label was getting overridden, I removed it, but you should check that none of the refs to  Proposition \ref{ramification} should be to this one.}}
...
\redden{Given a flag} 
\marginpar{``flag'' itself has not been defined}%
...
\cdots \; \subset \;  W \cap V_d \; \subset \;  W \cap V_{e} \,=\, W
.
$$
...
\begin{definition}
A \emph{Schubert index} for $\GG(\ell, e)$ is a sequence ${\bbeta} =
...
V})$ in $\GG(\ell, e)$ is $|{\bbeta}|\colonequals  \sum \beta_i$
...
\index{Chow ring}%
$A^*(\GG(\ell, e))$ of the 
\marginpar{fixed spelling of Gras.}%
\blue{Grassmannian}
\index{Grassmannian}%
is independent
...
the Chow ring $A^*(\GG(\ell, e))$ of an a special Schubert class with an
...
%\begin{fact}[(Pieri's formula)]
\begin{proposition}[Pieri's formula]
\label{Pieri}
%{\it
If $\sigma_\gamma$ is a 
\blue{special Schubert class}
\index{Schubert class!special}%
and $\sigma_{\bbeta}$
\marginpar{removed the CF designation; or else you could remove the Prop designation by unpercenting 4 lines in this block and percenting begin/end\{prop\} (with one xref below to be adjusted)}
...
$\sigma_{\bbeta}$ by 
\marginpar{blued because it's a memorable handle}
\blue{stacks of coins,}
...
\GG(\ell, e) = \ell(e-\ell)$, then
$$
(\sigma_\gamma)^l \cdot \sigma_{\bbeta} \neq 0 \in A^*(\GG(\ell, e), \ZZ).
$$
\end{corollary}
...
\blue{osculating flag}
\index{osculating flag}%
\marginpar{with osc. flag be clear?}
of $C$
...
space associated to $W` `$, in  $\GG(d-r, d+1)$.

\begin{proof}
To keep the notation of 
\redden{page~\pageref{Schubert1},} 
we work with the
affine versions
$\tL, \tsV = (\tV_1, \dots, \tV_{d+1})$ of the projection center and
the osculating flag at $p$, respectively,
so that, for example, $\dim \tL = d-r$. 
\marginpar{dropped ``Note that''}
(The space $\tV_{\!W}$
of the linear series
denoted $(V_{\!W}, \sO_{\PP^1}(d))$
is already a vector space, but for consistency of notation we will write
$\tV_{\tL}$ anyway.)
...
are $\delta$ Schubert indices for $\GG(d-r, d+1)$, the Schubert cycles
$\Sigma_{{\bbeta}^1}({\cal V}^1), \dots, \Sigma_{{\bbeta}^\delta}({\cal
V}^\delta) \subset \GG(d-r, d+1)$ intersect properly; that is, the
...
$\GG(d-r, r+1)$, so that if $\Phi \subset G$ is any subvariety of
...
$$
\sum_{i=1}^\delta |{\bbeta}^i| \; > \; (r+1)(d-r) = \dim \GG(d-r, d+1).
$$
If on the contrary
$$
...
we can deduce:
\marginpar{replaced ``the'' by colon}
...
apply 
\redden{the Riemann--Hurwitz formula}
...
\marginpar{to normalize punctuation}
\begin{enumerate}
\item  Show that $|\bbeta^*| = |\bbeta|$.
\item Show that $(\bbeta^*){\vphantom{\bbeta}}^* = \bbeta$. %align *s
\item Show that the isomorphism $\phi : \GG(k+1, V) \ruto{\,\cong} 
\GG(n-k, V^*)$ carries the Schubert cycle $\Sigma_\bbeta({\cal V})$ to the
...
space associated to $\Lambda$, in  $\GG(d-r, d+1)$. Conclude from
...
\marginpar{\redden{There were two exercises with the label `independent
secants': this one and Exercise \ref{independent secants}. Since this
label was getting overridden, I removed it, but you should check that
none of the refs to  Exercise \ref{independent secants} should be to
this one.}}
...
on $E$, then
\marginpar{dropped repeated ``then''}
