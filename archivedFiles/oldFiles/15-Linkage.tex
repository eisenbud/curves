...
The main theorem on linkage of curves in $\PP^3$ is due to Hartshorne and Rao ~\cite{MR520926}. If $X$ is a purely 1-dimensional projective scheme, then $S_X$ is locally Cohen-Macaulay , and thus $H^1(\sI_X(i)$ is nonzero for only finitely many values of $i\in \ZZ$, so
the vector space
$$
...
\begin{theorem}[Hartshorne-Rao\cite{MR520926}]\label{Hartshorne-Rao}
...
\begin{theorem} [Hartshorne \cite{Hartshorne1977}]\label{Hartshorne}
...
If $X'\subset \PP^{3}$ is another union of 3 disjoint lines then $X$ is linked to $X'$ if and only if $X'\subset Q$ as well. Moreover, $X$ is directly linked to $X'$ if $X'$ is in the opposite linear equivalence class, and evenly linked in two steps
to $X'$ if $X'$ is in the same equivalence class.
\end{proposition}

For more results in this direction, see~\cite{MiglioreSkewLines} from which the argument below is taken.
...
\begin{theorem}(Rao\cite{MR520926}
...
\begin{theorem}[Structure of a linkage class](\cite{MR714753})\label{description of a linkage class}
...
\begin{theorem}[\cite{MR0052828}]\label{Gaeta}
...
Before proving Theorem~\ref{Gaeta} we need one more result from commutative algebra the Hilbert-Burch theorem:

\begin{theorem}[Hilbert-Burch\cite{MR212008}]\label{Hilbert-Burch}
...
The structure present within a given (even) linkage class was illuminated in the work of Lazarsfeld and Rao\cite{MR714753}, which proved a version of a conjecture of Harris; we close this chapter by sketching a result of their paper. But first, an elementary result:
...
Much sharper bounds are known; see for example \cite{MR1073438}.
