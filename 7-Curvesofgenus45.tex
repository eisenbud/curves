...
\subsection{First case: the canonical curve is a complete intersection}\label{non-trigonal genus 5}
...
\subsection{Second case: the canonical curve is not a complete intersection}\label{trigonal genus 5}
...
complex. Moreover, the generators of $I_C$ above may be written as the $4\times 4$ Pfaffians
...
\end{pmatrix}
$$
where $\ell_0,\dots,\ell_3$ are independent linear forms and $g_1, g_2, g_3$ are quadrics. The
 $2\times 2$
...
%
%To do this we will make use of a general position argument, whose conclusion is that the canonical curve has a \emph{simple $(g-2)$-secant $(g-3)$-plane.}
%
...
% 
%\begin{proof}
%By Lemma~\ref{linear general position} the points of a general hyperplane section are in linearly general position.
%\end{proof}
%
...
%curves as well. The reader may check that it applies equally to any locally Gorenstein 1-dimensional scheme, embedded by
%the complete linear series corresponding to its dualizing sheaf---see Chapter~\ref{linkageChapter} for this language.
% 
%   
%\begin{proof} 
