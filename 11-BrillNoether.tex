...
From Theorem~\ref{BN consequences}  we know that every curve $C$ of genus 6 has a $g^{2}_{6}$. For a general curve  $\phi$ maps $C$ birationally onto a plane sextic with 4 nodes. We may ask, ``what could go wrong?", and classify curves of genus 6 accordingly. It is possible to go quite far 
in this direction, but we will only explore *** branches of the tree.
...
Further analysis could be divided as follows:
\begin{enumerate}
 \item $|D|$ has a base point;
 \item $|D|$ is base-point free, but the map $\phi_{D}$
...
$|D|$ has exactly one base point.
When we subtract the base point we get a base-point-free $g^2_5$. 
...
This allows us to describe the other special linear series on $C$. By the adjunction formula, the the canonical series $|K_C|$ is cut on $C$ by conics in the plane. Since any subscheme $\Gamma \subset \PP^2$ of dimension 0 and degree 3 imposes independent conditions on quadrics, it follows that $C$ is not trigonal. As we showed in Chapter~\ref{3b}, the $g^1_4$s on $C$ are exactly the projections from points of $C$, so $W^1_4(C)\cong C$.
...
lies on this surface, and is cut out there by a 3-dimensional family of cubics,  the image of the 3 dimensional family of forms of degree 6 that are multiples of the quintic form defining $C$ in $\PP^2$.
