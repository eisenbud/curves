...
Suppose $f : C \to S$ is a regular map from a smooth curve $C$ to a surface $S$. If $\sL $ is a line bundle on $S$ and $V \subset H^0(\sL )$ a vector space of sections, we can associate to them a linear system on $C$ by taking the pullback linear system $f^*V \subset H^0(f^*\sL )$ on $C$ and subtracting the base points; this is called the \emph{linear series cut out on $C$ by $V$}. 
