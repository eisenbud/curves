...
$$
and the monodromy of the generically finite cover $W^r_d \to M_g$ is transitive.
(\cite{zbMATH04014883}).
...
a general curve, the following theorem of \cite{ELarson2018} gives a comprehensive answer. First, recall that by Corollary~\ref{2L nonspecial}, if $L \in W^r_d(C)$ is a general point, then $h^0(L^m) = md-g+1$ for all $m \geq 2$.
...
$$
has maximal rank; that is, it is injective if $\binom{m+r}{r} \leq md-g+1$ and surjective if $\binom{m+r}{r} \geq md-g+1$.
\end{theorem}
...
By parts~(\ref{rho=0}) and~\ref{Petri} of Theorem~\ref{Wrd omnibus}, $C$ must have 5 distinct $g^1_4$s, and if three of the four nodes of $C_0$ were collinear, the $g^1_4$ cut on $C$ by lines through the fourth node would coincide with the $g^1_4$ cut on $C$ by conics through all four. This would represent a non-reduced point of the scheme $W^1_4(C)$, the subject of Exercise~\ref{nonreduced Wrd} below. 
 ...
 \pict{plane sextic with 4 nodes, indicating the projection from a node and the family of conics through all 4}
 \pict{sextic with 4 nodes indicating the exceptional triangle for the quadratic transformation to another plane model.}
...
A quintic del Pezzo surface $S \subset \PP^5$ contains exactly 10 lines, which (in terms of the description of $S$ as the blow up of $\PP^2$ at four points $p_1,\dots,p_4 \in \PP^2$) are the 4 exceptional divisors and the 6 proper transforms of the lines joining the $p_i$ pairwise. 
 ...
 \pict{dual graph of lines on the quintic del Pezzo}
...
\pict{most of the following could be illustrated (plane curves)}
...
As in the case of the general curve of genus 6, the space of quadratic forms in the ideal of $C$ is 6-dimensional. However, by the geometric Riemann-Roch theorem, the triples of points in the divisors of the $g^1_3$ are colinear,
and the lines they span must all be contained in the quadrics containing $C$. The intersection of the 6 quadratic
form is equal to the union of these line; it is a \emph{rational normal scroll},
