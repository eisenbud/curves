...
In~\cite{Piene-Schlessinger} it is also shown that each component individually is smooth and rational; and the intersection of the two,
which is 11-dimensional with generic point corresponding to a plane nodal cubic  with an embedded point at the node
(see Exercise~\ref{hilb intersection}), is also
smooth and rational. 
...
%\fix{I worry about taking the quotient in cases where the linear series is not invariant.}
...
$$
\end{theorem}
\fix{we get $\dim cH_0\geq -$ easily. But for equality, need to know that $\{C\in \sM_g \mid W^r_d\geq \rho+x\}$
has codim $\geq x$, and, for a general $C$, $\codim W^{r+x}_d\subset W^r_d(C) \geq \dim (Gr(r, \PP^{r+x})$ --
 and even a combination of these! Is there such a result? The trouble is bounded by Martens theorem, of course.}
