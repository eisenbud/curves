...
With our standing assumption of characteristic 0, not every point of $C$ is an inflection point, and indeed
the total number of inflection points, properly counted, is determined by $d, g$ and $r$.
...
\end{equation}
\end{proposition}
A proof of this statement can be found, for example, in~\cite{} (3264?). 

In particular the number of flex points of a given linear series is finite: not every point can be a flex. This may seem obvious---try to imagine a plane curve in which every point is a flex!---but it would be false in characteristic $p$: there exist smooth curves in $\PP^r$ such that every point is an inflection; these are sometimes called ``strange curves'' (see \cite[]{Hartshorne} for an example). 

The Pl\"ucker formula, while extremely useful (as we'll see in the remainder of this chapter), leaves many questions unanswered: we don't know, for example, what combinations of inflection points are possible, or what the behavior of the inflection points on a suitably general curve may be.
