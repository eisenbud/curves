...
\begin{corollary}\label{uninflected curves}
 If $C\subset \PP^r$ is a smooth nondegenerate curve with no inflection points, then $C$ is the rational normal curve of degree $r$. In particular,
\end{corollary}
\begin{proof}
 $(r+1)d + r(r+1)(g-1) = 0$ implies $r(g-1) = -d$, and since $r$ and $d$ are non-negative, $g=0, r=d$. 
\end{proof}

An immediate consequence is the converse of proposition~\ref{Veronese is projectively homogeneous}: Recall that $C\subset \PP^r$ is \emph{projectively homogeneous} if the automorphisms of $\PP^r$ fixing
$C$ act transitively on $C$.
\begin{corollary}~\ref{no more projectively homogeneous curves}
If $C\subset \PP^r$ is a nondegenerate reduced irreducible curve in $\PP^r$ that is projectively homogeneous then $C$ is a rational normal curve.
\end{corollary}


We can now prove the converse of Proposition~\ref{independence on rnc}
\begin{corollary}
If a smooth curve $C\subset \PP^r$ has the property 
that no subscheme $\Gamma\subset C$  of length $\ell \leq r+1$,  lies on a plane of dimension $<\ell$ then $C$ is a rational normal curve of degree $r$.
\end{corollary}
\begin{proof}
If $C$ were not a rational normal curve then it would have an inflection point $p$, and by definition $(r+1)p$ lies on a hyperplane.
\end{proof}
