...
In this concluding chapter, we want to introduce one more aspect of the geometry of linear series on curves, the \emph{inflection points} of a linear system, and use it to give a proof of one implication in the statement Brill-Noether theorem.\fix{I changed from "classical B-N" -- ok?}
...
Let $C$ be a smooth projective curve of genus $g$, and $\cD = (\cL,V)$ a $g^r_d$ on $C$; that is, $\cL \in \Pic^d(C)$ is a line bundle of degree $d$ on $C$ and $V \subset H^0(\cL)$ is an $(r+1)$-dimensional vector space of sections. \fix{maybe not repeat this here?}
