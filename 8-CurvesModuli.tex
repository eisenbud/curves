...
\marginpar{changed tag to a star since you used that style earlier; the numbering works better when there are more formulas as guideposts}
\begin{equation}\label{formula for j}
j(\lambda) \colonequals  256\frac{(\lambda^2-\lambda + 1)^3}{\lambda^2(\lambda-1)^2},
\tag{$*$}
\end{equation}
known as the
\blue{\emph{$j$-function}.}
\index{$j$-function}%
 As $\lambda$ varies in $\PP^1 \setminus \{0,1,\infty\}$,
\redden{the values of}
\marginpar{to avoid abutting math}%
\blue{$j(\lambda)$}
\index{notation!$j(\lambda)$}%
\redden{range over all of}
$\AA^1` `$.
...
This is slightly weaker than saying that $M_1$ is a
\redden{coarse moduli space,}
\index{coarse moduli space}%
\marginpar{\redden{c.m.s. is not defined until Theorem \ref{unproved1}}; indexed}%
because we are restricting ourselves to families with reduced
bases; in fact,
\redden{the statement is true in general,}
\marginpar{does ``statement'' mean the proposition, and does ``in general'' mean without the restriction to reduced bases?}
but requires more machinery to prove.
...
(see for example \cite[Appendix, Theorems B.5 and B.9]{3264} or, for a
\marginpar{the tone throughout is more formal and impersonal; ``treatment that some of our students find harder to understand'' stood out}
\redden{perhaps more challenging treatment,}
...
\redden{The second way in which}
$M_1$ comes close to being a fine moduli space
\redden{is seen in the next result:}
...
$$
commutes.
\marginpar{I've removed a \string \label\{cubic family\} which duplicated a figure label. The only ref to that string identified it as a figure.}
\end{enumerate}
\end{proposition}
...
\redden{on page~\pageref{formula for j}.}
We have already described a family of curves of genus 1 over the
\blue{\emph{$\lambda$-line}}
\marginpar{replaced quotes by italics}%
...
\redden{tautological isomorphism}
\marginpar{do you mean the identity isomorphism?} 
...
\inprogress
\centerline{\hfilneg
\raise3pt\hbox{\includegraphics[width=1.6in]{main/cubic-la=0.5}}\hfil
\includegraphics[width=1.6in]{main/cubic-la=0.9}\hfil
\raise-1pt\hbox{\includegraphics[width=1.6in]{main/cubic-la=1}}\hfilneg
}
\centerline{\hfilneg
\small
\hbox to 1.6in{\hfil\hfil$\lambda=0.5$\hfil}\hfil
\hbox to 1.6in{\hfil\hfil$\lambda=0.9$\hfil}\hfil
\hbox to 1.6in{\hfil\hfil$\lambda=1$\hfil}\hfilneg
}
\caption{A curve of genus 1 degenerating to a rational curve with a node
\marginparhere{\redden{Should some tickmarks and numbers be preserved? (say $x=1,2$ and $y=  1,-1$)}}
...
\marginpar{dropped quotation marks}
...
\blue{\emph{modular compactification}.}
...
$$
the $j$-function
\marginpar{added hyphen to ``$j$ function'' to agree with earlier usage}
...
\inprogress
\centerline{\hfilneg
\lower14pt\hbox{\includegraphics[width=1.6in,trim=30 70 50 100,clip]{main/cubic-t=1}}\hfil
\includegraphics[width=1.6in,trim=0 50 0 0,clip]{main/cubic-t=0.5}\hfil
\hbox{\includegraphics[width=1.6in,trim=0 50 0 0,clip]{main/cubic-t=0}}\hfilneg
}
\centerline{\hfilneg
\small
\hbox to 1.9in{\hfil\hfil$t=1$\hfil}\hfil
\hbox to 1.6in{\hfil\hfil$t=0.5$\hfil}\hfil
\hbox to 1.6in{\hfil\hfil$t=0$\hfil}\hfilneg
}
...
\blue{\emph{semistable reduction}}
...
\marginparhere{\redden{\vskip-140pt please check if this figure is now how it should be}}
...
\blue{\emph{geometric invariant theory}}
...
\marginpar{it would be better to give a URL in the bib}
\redden{(linked from the book's AMS website)}
...
\marginpar{this sounds like the paper is described, but I think you mean the DM compactification, right?}
\redden{described below.}
...
\redden{matter,}
\marginpar{$\leftarrow$ changed colon to comma (or keep the colon and drop ``and thus'')}
...
\marginpar{extended the italics to ``points'', similarly in (2) and (3)}
...
\blue{compactification of the affine line}
...
subset of $X$ defined in this way. (But there may well be other
\marginpar{parenthetic is now a new sentence}
...
\marginpar{\vskip-20pt assigned a heading and made pre-enumerate statement into first item}%
...
correspondingly
\blue{there does not exist a universal family}
...
\marginpar{indexed PDF had note: ``Add Alper ref''}
\end{enumerate}
...
$M_{g}$,
\marginpar{dropped ``for example'' to avoid break in formula}
...
\marginpar{added comma after 12}
...
\marginpar{dropped parentheses around cite}
\cite{MR2202246}. In genera 15 and 16 Chang and Ran showed 
\redden{the possibly weaker statement} 
that $\ovM_{g}$ has no pluricanonical divisors
\marginpar{reworded to avoid clashing subscript and bar}
{\meshing\par}
...
\blue{discriminant hypersurface.}
...
\marginpar{dropped content-free ``We have then the''}
...
degree $d$ are there 
\redden{for a given curve $C$?} 
To begin with, the 
\blue{$g+1$ theorem}
\index{g plus 1@$g+1$ theorem}%
(Theorem \ref{g+1 theorem}) tells us that
there is one, whence we see that $\pi$ is surjective.
...
\redden{which expresses what happens}
...
\marginpar{changed dashes to commas}
...
\redden{where the conjecture is proved}
...
\marginpar{this def repeats one given in Chapter 4; I made it explicit there and parenthetic here}
\redden{nodal curves (those with only nodes as singularities),}
and compute its dimension.

\def\Vdg{V_{\,g,d}}
\def\Vdgbar{\overkern1{15}{\Vdg}}
\def\Vdgpbar{\overkern1{19}{V_{\smash{\!g'`,d}}}}
...
\marginpar{dropped ``Proposition''; there have been other defs with facts stated and justified as here}
...
\marginpar{dropped repeated ``is''}
is an interesting open
problem to find a simpler partial compactification of $ \Vdg$.

\begin{fact}\label{severi irreducible}
\redden{Corollary~\ref{local geometry of Severi} says that}
\marginpar{to provide xref and avoid echo (cf ``We will see'' in previous para)}
...
\index{Severi, Francesco}%
\index{historical context}%
\redden{A~good proof was finally given in \cite{MR837522}.}
\end{fact}
...
\label{local severi geometry}
 $\Phi$
\marginpar{moved a label from the subsection to this proposition. Is this the right xref for the fact that the family of plane quartics with a node has codimension one? (p.~\pageref{nodal plane quartics})}
...
$$ 
\redden{of bidegree $(1, d)$;}
\marginpar{moved this phrase}
...
\marginpar{or else we can drop ``the point''}
\redden{$[1,0,0]$ is not in $C$}, and that the point $p$ is $[0,0,1]$. 
...
\marginpar{removed repeated ``in'' and nonexistent table number}
the table:
$$
...
\marginpar{node!condition imposed by}%
\marginpar{independent conditions}%
...
\redden{of}
 $C \in \PP^N$
\marginpar{dropped ``Moreover'', added ``of'', changed $g,d'$ to $g',d$ in subscript of $\overkern10 V$}
the variety 
\redden{$\Vdgpbar$}
...
\marginpar{changed $p_{\delta'}$ to $p_\delta$}
\meshing
\end{corollary}
...
\marginparhere{\rightskip=-20pt swapped the two subscripts of $V$ and streamlined so this expression is on the last line\\[-48pt]\hskip-170pt\redden{subscripts still to be swapped}}
$V_{\protect\sbinom{d-1}{2}-2,@d}$ is the 
\redden{transverse} 
...
embedding.
\marginpar{embeddi $\to$ embedding (is there more that got dropped?)}
\end{enumerate}
\end{exercise}
