...
that \index{hyperelliptic curve}%
the canonical
...
points. There is a
   ...
   \in S_d$ satisfying the conditions:
\smallbreak
\begin{enumerate}
...
system $|L|$. Since $\phi_\sL$ is an embedding, we'll also denote the
...
in Figure~\ref{fig28bitangents}, which shows a special case in which the 28 bitangents
are all realized over $\RR$).
...
,
$$
and taking the sum and the difference of these two equations yields theresult.
